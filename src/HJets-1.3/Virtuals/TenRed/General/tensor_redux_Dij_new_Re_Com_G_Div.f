c FC %       subroutine ten_red_D_LU_decom_G(m0,m1,m2,m3,p1sq,p2sq,p3sq,p1p2,p1p3,p2p3, 
        subroutine tens_red4_new_Re_Com_GDiv(m0,m1,m2,m3,p1sq,p2sq,p3sq,p1p2,p1p3,p2p3, 
...
c
        ...
        real*8 tz(3,3),itz(3,3),i(3)
c       real*8 deter,detAbs
c	   Logical Singular
c	   Common Singular
        real*8 cdet
        Complex*16 Dij(70,7)
...
c 33-3 4-35
       ...
       PRr(2)=(Cijr_124(2,1)-Cijr_134(1,1)-Dijr(2,1)*r2r1-Dijr(7,2)*2.d0)
...
c       Dijr(4,2) = Rr(1)
       ...
       PRI(2)=(CijI_124(2,1)-CijI_134(1,1)-DijI(2,1)*r2r1-DijI(7,2)*2.d0)
...
CCCCCC CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       ...
       Dijr(11,3)=0.25d0*(-C0r_234+2d0*m0sq*Dijr(1,1)+Dijr(1,2)*r1r0+Dijr(4,2)*r2r1
     &       +Dijr(5,2)*r3r2)
       
       Dijr(12,3)=0.25d0*(Cijr_234(1,1)+2d0*m0sq*Dijr(2,1)+Dijr(4,2)*r1r0+Dijr(2,2)*r2r1
     &                   +Dijr(6,2)*r3r2)
       Dijr(13,3)=0.25d0*(Cijr_234(2,1)+2d0*m0sq*Dijr(3,1)+Dijr(5,2)*r1r0 + Dijr(6,2)*r2r1
     &                    + Dijr(3,2)*r3r2)
       
        DijI(11,3)=0.25d0*(-C0I_234+2d0*m0sq*DijI(1,1)+DijI(1,2)*r1r0+DijI(4,2)*r2r1
     &   +DijI(5,2)*r3r2)
       
       DijI(12,3)=0.25d0*(CijI_234(1,1)+2d0*m0sq*DijI(2,1)+DijI(4,2)*r1r0+DijI(2,2)*r2r1
     &                   +DijI(6,2)*r3r2)
       DijI(13,3)=0.25d0*(CijI_234(2,1)+2d0*m0sq*DijI(3,1)+DijI(5,2)*r1r0 + DijI(6,2)*r2r1
     &                    + DijI(3,2)*r3r2)
...
c 
      ...
      Dijr(7,1)=(C0r_234+2*m0sq*Dijr(1,2)+Dijr(1,3)*r1r0+Dijr(4,3)*r2r1+Dijr(5,3)*r3r2 )*Iv
c      
      Dijr(8,1)=(Cijr_234(1,2)+2*m0sq*Dijr(2,2)+Dijr(6,3)*r1r0+Dijr(2,3)*r2r1
     - +Dijr(8,3)*r3r2)*Iv
c
      Dijr(9,1)=(Cijr_234(2,2)+2*m0sq*Dijr(3,2)+Dijr(7,3)*r1r0+Dijr(9,3)*r2r1
     - +Dijr(3,3)*r3r2)*Iv
c
      Dijr(10,1)=(-Cijr_234(1,1)+2*m0sq*Dijr(4,2)+Dijr(4,3)*r1r0+Dijr(6,3)*r2r1
     - +Dijr(10,3)*r3r2)*Iv
c      
      Dijr(11,1)=(-Cijr_234(2,1)+2*m0sq*Dijr(5,2)+Dijr(5,3)*r1r0+Dijr(10,3)*r2r1
     - +Dijr(7,3)*r3r2)*Iv
c
      Dijr(12,1)=(Cijr_234(3,2)+2*m0sq*Dijr(6,2)+Dijr(10,3)*r1r0+Dijr(8,3)*r2r1
     - +Dijr(9,3)*r3r2)*Iv
c
      Dijr(13,1)=(Cijr_234(4,2)+2*m0sq*Dijr(7,2)+Dijr(11,3)*r1r0+Dijr(12,3)*r2r1
     -      + Dijr(13,3)*r3r2)*Iv        
c
      DijI(7,1)=(C0I_234+2*m0sq*DijI(1,2)+DijI(1,3)*r1r0+DijI(4,3)*r2r1+DijI(5,3)*r3r2 )*Iv
c      
      DijI(8,1)=(CijI_234(1,2)+2*m0sq*DijI(2,2)+DijI(6,3)*r1r0+DijI(2,3)*r2r1
     - +DijI(8,3)*r3r2)*Iv
c
      DijI(9,1)=(CijI_234(2,2)+2*m0sq*DijI(3,2)+DijI(7,3)*r1r0+DijI(9,3)*r2r1
     -+DijI(3,3)*r3r2)*Iv
c
      DijI(10,1)=(-CijI_234(1,1)+2*m0sq*DijI(4,2)+DijI(4,3)*r1r0+DijI(6,3)*r2r1
     - +DijI(10,3)*r3r2)*Iv
c      
      DijI(11,1)=(-CijI_234(2,1)+2*m0sq*DijI(5,2)+DijI(5,3)*r1r0+DijI(10,3)*r2r1
     - +DijI(7,3)*r3r2)*Iv
c
      DijI(12,1)=(CijI_234(3,2)+2*m0sq*DijI(6,2)+DijI(10,3)*r1r0+DijI(8,3)*r2r1
     - +DijI(9,3)*r3r2)*Iv
c
      DijI(13,1)=(CijI_234(4,2)+2*m0sq*DijI(7,2)+DijI(11,3)*r1r0+DijI(12,3)*r2r1
     -      + DijI(13,3)*r3r2)*Iv 
