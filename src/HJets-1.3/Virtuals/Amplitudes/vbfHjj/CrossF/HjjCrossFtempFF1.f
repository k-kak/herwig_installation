       subroutine HjjCrossFFFhl1(F1)
       IMPLICIT NONE
       Real*8 P(5),FRe(1:5),FIm(1:5)
       Complex*16 F1(1:5)
       Real*8   p1sq 
       Real*8   p2sq 
       Real*8   p3sq 
...
c************************************************************************************
       P(1) = s12+s23-s45
       P(2) = s12-s15+s23-s34-s45
       P(3) = -2*s12+s15-s23+s34+s45
       P(4) = s12-2*s15+s23-s45
       P(5) = s12+s15-s34
       FRe(1) = -2*(D01245R+D01345R-s12*(EE0R+EijR(2,1))+2*EijR(11,2)+
     &   (EijR(3,1)+EijR(6,2)-EijR(10,2))*P(1)-EijR(4,1)*P(2)-EijR(4,
     &   2)*P(3)-EijR(7,2)*P(4)-EijR(9,2)*P(5))
       FIm(1) = -2*(D01245I+D01345I-s12*(EE0I+EijI(2,1))+2*EijI(11,2)+
     &   (EijI(3,1)+EijI(6,2)-EijI(10,2))*P(1)-EijI(4,1)*P(2)-EijI(4,
     &   2)*P(3)-EijI(7,2)*P(4)-EijI(9,2)*P(5))
       F1(1)=DCMPLX(FRe(1),FIm(1))
       FRe(2) = -EijR(4,1)+EijR(4,2)-EijR(9,2)
       FIm(2) = -EijI(4,1)+EijI(4,2)-EijI(9,2)
       F1(2)=DCMPLX(FRe(2),FIm(2))
       FRe(3) = EijR(4,2)+EijR(6,2)-EijR(7,2)-EijR(10,2)
       FIm(3) = EijI(4,2)+EijI(6,2)-EijI(7,2)-EijI(10,2)
       F1(3)=DCMPLX(FRe(3),FIm(3))
       FRe(4) = 2*(EijR(3,1)-EijR(4,1)+2*(EijR(4,2)+EijR(8,2)-EijR(9,2
     &   )-EijR(10,2)))
       FIm(4) = 2*(EijI(3,1)-EijI(4,1)+2*(EijI(4,2)+EijI(8,2)-EijI(9,2
     &   )-EijI(10,2)))
       F1(4)=DCMPLX(FRe(4),FIm(4))
       FRe(5) = 2*(EijR(4,1)-2*(EijR(4,2)-EijR(7,2)))
       FIm(5) = 2*(EijI(4,1)-2*(EijI(4,2)-EijI(7,2)))
