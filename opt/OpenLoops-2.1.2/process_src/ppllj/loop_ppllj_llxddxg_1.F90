...
module ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND



module ol_forced_parameters_ppllj_llxddxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_llxddxg_1_/**/REALKIND


! **********************************************************************
module ol_loop_storage_ppllj_llxddxg_1_/**/REALKIND
! **********************************************************************
  ...
  type(wfun) :: wf4(4,15), wf8(8,14), wf32(32,18)

  ! diagram prefactors
  integer,           save :: fac_status_loop1 = -1, fac_status_loop2 = -1
  complex(REALKIND), save :: f(21), c(13)

  !Vector in helicity and colour space for Born-Loop interference
  type(Hpolcont),   save, allocatable :: M0_col1_helarray(:,:), M0M1_hel_cc(:,:,:)
  complex(REALKIND), save :: M2ctcc(16)

end module ol_loop_storage_ppllj_llxddxg_1_/**/REALKIND
...
! **********************************************************************
module ol_loop_ppllj_llxddxg_1_/**/REALKIND
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND, intkind1, intkind2
  use ol_data_types_/**/REALKIND, only: wfun, Hpolcont
  use ol_loop_storage_ppllj_llxddxg_1_/**/REALKIND
  implicit none
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f( 5) = (CI*countertermnorm*ctGqq*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 6) = CI*countertermnorm*ctGqq*eQED**2*gQCD**3
    f( 7) = (CI*countertermnorm*ctVqq*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 8) = CI*countertermnorm*ctVqq*eQED**2*gQCD**3
    f( 9) = countertermnorm*ctZGG*eQED**2*gQCD**3
    f(10) = (CI*countertermnorm*ctHGG*eQED**2*gQCD**3*ML)/(2._/**/REALKIND*MW*sw)
    f(11) = (CI*eQED**2*gQCD**3*integralnorm*SwB)/3._/**/REALKIND
    f(12) = CI*eQED**2*gQCD**3*integralnorm*SwB
    f(13) = (eQED**2*gQCD**3*integralnorm*SwB)/3._/**/REALKIND
    f(14) = eQED**2*gQCD**3*integralnorm*SwB
    f(15) = (eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f(16) = (2*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f(17) = eQED**2*gQCD**3*integralnorm*SwF
    f(18) = (4*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f(19) = 2*eQED**2*gQCD**3*integralnorm*SwF
    f(20) = (eQED**2*gQCD**3*integralnorm*MB*ML*SwF)/(MW**2*sw**2*4._/**/REALKIND)
    f(21) = (eQED**2*gQCD**3*integralnorm*ML*MT*SwF)/(MW**2*sw**2*4._/**/REALKIND)

  c = [ 9*CI*f(11), 9*CI*f(12), f(13), 8*f(13), f(14), 8*f(14), 3*f(15), 3*f(16), 3*f(17), 3*f(18), 3*f(19), 3*f(20), 3*f(21) ]
  c = (1._/**/REALKIND / 6) * c
...
! **********************************************************************
#ifndef PRECISION_dp
  use ol_loop_storage_ppllj_llxddxg_1_/**/DREALKIND, only: hel_states
#endif
  implicit none
  ...
  allocate(A(hel_states,18))
  allocate(M1helarray(1,hel_states))
  allocate(M1helarray_ct(1,hel_states))
  allocate(M0_col1_helarray(1,hel_states))
  A(1:hel_states,1:18)%j = 0
  A(1:hel_states,1:18)%e = 0
  A(1:hel_states,1:18)%hf = 0
  A(1:hel_states,1:18)%s = 0
#if 1 > 0
...
#else
recursive subroutine amp2(P_scatt, M02, qp_kinematics, M2ct, M2colint)
  use ol_loop_storage_ppllj_llxddxg_1_/**/DREALKIND, only: &
  nhel, Hel, hel_states
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for tau- tau+ down anti-down glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND !, only: ci, parameters_status, ZERO, scalefactor, >masses<
  use ol_parameters_init_/**/REALKIND, only: ensure_mp_init, ensure_mp_loop_init
  use ol_kinematics_/**/REALKIND, only: init_kinematics
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_settings_ppllj_llxddxg_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_external_ppllj_llxddxg_1, only: &
    & external_perm_ppllj_llxddxg_1, &
    & external_perm_inv_ppllj_llxddxg_1, &
    & extcomb_perm_ppllj_llxddxg_1, &
    & average_factor_ppllj_llxddxg_1
  use ol_external_ppllj_llxddxg_1, only: &
    & H, hel_not_initialised, hel_init, POLSEL 
  use ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND, only: &
    & colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllj_llxddxg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_llxddxg_1
  ...
  extmasses2 = [ nML, nML, 0, 0, 0 ]
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0) and compute
  ! internal-propagator momenta in light-cone representation
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_llxddxg_1, 5, qp_kinematics)
  ...
  call vert_AQ_S(gH,ntry, ex2(:), ex1(:), wf4(:,8), n3(:,9), t3x4(:,:,5))
  call vert_QA_V(ntry, ex3(:), ex4(:), wf4(:,9), n3(:,10), t3x4(:,:,6))
  call counter_SG_G(ntry, wf4(:,8), ex5(:), wf8(:,5), n3(:,11), t3x8(:,:,5))
  call counter_VG_G(ntry, wf4(:,5), ex5(:), 16, wf8(:,6), 19, n3(:,12), t3x8(:,:,6))
  call counter_AV_Q(ntry, ex4(:), wf4(:,1), wf8(:,7), n3(:,13), t3x8(:,:,7))
  call counter_AZ_Q(gZd,ntry, ex4(:), wf4(:,5), wf8(:,8), n3(:,14), t3x8(:,:,8))
  call counter_AV_Q(ntry, ex4(:), ex5(:), wf4(:,10), n3(:,15), t3x4(:,:,7))
  call prop_A_Q(ntry, wf4(:,10), 24, ZERO, 0_intkind1, wf4(:,11), n2(4))
  call counter_VQ_A(ntry, wf4(:,1), ex3(:), wf8(:,9), n3(:,16), t3x8(:,:,9))
  call counter_ZQ_A(gZd,ntry, wf4(:,5), ex3(:), wf8(:,10), n3(:,17), t3x8(:,:,10))
  call counter_VQ_A(ntry, ex5(:), ex3(:), wf4(:,12), n3(:,18), t3x4(:,:,8))
  call prop_Q_A(ntry, wf4(:,12), 20, ZERO, 0_intkind1, wf4(:,13), n2(5))
  call counter_Q_A(ctqq,1,ntry, wf4(:,3), 20, wf4(:,14), n2(6))
  call prop_A_Q(ntry, wf8(:,1), 11, ZERO, 0_intkind1, wf8(:,11), n2(7))
  call prop_A_Q(ntry, wf8(:,2), 11, ZERO, 0_intkind1, wf8(:,12), n2(8))
  call counter_A_Q(ctqq,1,ntry, wf4(:,7), 24, wf4(:,15), n2(9))
  call prop_Q_A(ntry, wf8(:,3), 7, ZERO, 0_intkind1, wf8(:,13), n2(10))
  call prop_Q_A(ntry, wf8(:,4), 7, ZERO, 0_intkind1, wf8(:,14), n2(11))
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call Hcont_QA(nsync, wf4(:,3), wf8(:,1), A(:,1), n3(:,19), t3x32(:,:,1), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,3), wf8(:,2), A(:,2), n3(:,20), t3x32(:,:,2), nhel, den(5))
    call Hcont_QA(nsync, wf4(:,7), wf8(:,3), A(:,3), n3(:,21), t3x32(:,:,3), nhel, den(7))
    call Hcont_QA(nsync, wf4(:,7), wf8(:,4), A(:,4), n3(:,22), t3x32(:,:,4), nhel, den(8))

    call Hcont_VV(nsync, wf4(:,9), wf8(:,5), A(:,5), n3(:,23), t3x32(:,:,5), nhel, den(11))
    call Hcont_VV(nsync, wf4(:,9), wf8(:,6), A(:,6), n3(:,24), t3x32(:,:,6), nhel, den(12))
    call Hcont_QA(nsync, wf4(:,3), wf8(:,7), A(:,7), n3(:,25), t3x32(:,:,7), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,3), wf8(:,8), A(:,8), n3(:,26), t3x32(:,:,8), nhel, den(5))
    call Hcont_QA(nsync, wf8(:,3), wf4(:,11), A(:,9), n3(:,27), t3x32(:,:,9), nhel, den(7))
    call Hcont_QA(nsync, wf8(:,4), wf4(:,11), A(:,10), n3(:,28), t3x32(:,:,10), nhel, den(8))
    call Hcont_QA(nsync, wf4(:,7), wf8(:,9), A(:,11), n3(:,29), t3x32(:,:,11), nhel, den(7))
    call Hcont_QA(nsync, wf4(:,7), wf8(:,10), A(:,12), n3(:,30), t3x32(:,:,12), nhel, den(8))
    call Hcont_QA(nsync, wf8(:,1), wf4(:,13), A(:,13), n3(:,31), t3x32(:,:,13), nhel, den(3))
    call Hcont_QA(nsync, wf8(:,2), wf4(:,13), A(:,14), n3(:,32), t3x32(:,:,14), nhel, den(5))
    call Hcont_QA(nsync, wf4(:,14), wf8(:,11), A(:,15), n3(:,33), t3x32(:,:,15), nhel, den(15))
    call Hcont_QA(nsync, wf4(:,14), wf8(:,12), A(:,16), n3(:,34), t3x32(:,:,16), nhel, den(17))
    call Hcont_QA(nsync, wf4(:,15), wf8(:,13), A(:,17), n3(:,35), t3x32(:,:,17), nhel, den(20))
    call Hcont_QA(nsync, wf4(:,15), wf8(:,14), A(:,18), n3(:,36), t3x32(:,:,18), nhel, den(22))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M2(1)%j = ((-A(j,15)%j-A(j,17)%j)*f(3)+(-A(j,16)%j-A(j,18)%j)*f(4)+(A(j,9)%j+A(j,13)%j)*f(5)+(A(j,10)%j+A(j,14)%j)*f(6) &
       +(A(j,7)%j+A(j,11)%j)*f(7)+(A(j,8)%j+A(j,12)%j)*f(8)-A(j,6)%j*f(9)-A(j,5)%j*f(10))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_llxddxg_1_/**/REALKIND, only: KL
  implicit none
...
#endif
! M2tree = helicity-summed squared tree matrix element for tau- tau+ down anti-down glue -> 0
! M2hsf  = formula (66) of 1011.3918. Tree level
! spin-correlated hard scattering factor for tau- tau+ down anti-down glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND
  implicit none
  real(DREALKIND), intent(in) :: P_scatt(0:3,5)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_llxddxg_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 5
    r = external_perm_inv_ppllj_llxddxg_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_llxddxg_1_/**/REALKIND
