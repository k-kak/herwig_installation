...
module ol_colourmatrix_ppllj_nenexccxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_nenexccxg_1_/**/REALKIND



module ol_forced_parameters_ppllj_nenexccxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_nenexccxg_1_/**/REALKIND


! **********************************************************************
module ol_loop_storage_ppllj_nenexccxg_1_/**/REALKIND
! **********************************************************************
  ...
  type(wfun) :: wf4(4,13), wf8(8,7), wf32(32,9)
...
end module ol_loop_storage_ppllj_nenexccxg_1_/**/REALKIND
...
! **********************************************************************
module ol_loop_ppllj_nenexccxg_1_/**/REALKIND
! **********************************************************************
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(3) = CI*countertermnorm*ctGcc*eQED**2*gQCD**3
    f(4) = CI*countertermnorm*ctVcc*eQED**2*gQCD**3
...
subroutine denominators()
  ...
  den(4) = 1 /((L(5,24) - MC2)+L(6,24))
  den(6) = 1 /((L(5,12))+L(6,12))
  den(8) = 1 /((L(5,11) - MC2)+L(6,11))
  den(11) = 1 /((L(5,7) - MC2)+L(6,7))
...
! **********************************************************************
#ifndef PRECISION_dp
  use ol_loop_storage_ppllj_nenexccxg_1_/**/DREALKIND, only: hel_states
#endif
  implicit none
  ...
  allocate(A(hel_states,9))
  allocate(M1helarray(1,hel_states))
  allocate(M1helarray_ct(1,hel_states))
  allocate(M0_col1_helarray(1,hel_states))
  A(1:hel_states,1:9)%j = 0
  A(1:hel_states,1:9)%e = 0
  A(1:hel_states,1:9)%hf = 0
  A(1:hel_states,1:9)%s = 0
#if 1 > 0
...
! M2  = helicity-summed squared matrix element for nu_e anti-nu_e charm anti-charm glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND !, only: ci, parameters_status, ZERO, scalefactor, >masses<
  use ol_parameters_init_/**/REALKIND, only: ensure_mp_init, ensure_mp_loop_init
  use ol_kinematics_/**/REALKIND, only: init_kinematics
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_settings_ppllj_nenexccxg_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_forced_parameters_ppllj_nenexccxg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_nenexccxg_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_nenexccxg_1, 5, qp_kinematics)
  ...
  call pol_wf_A(P(:,4), rMC, H4, ex4, POLSEL(4),4)
  ...
  call prop_Q_A(ntry, wf4(:,2), 20, MC, 1_intkind1, wf4(:,4), n2(2))
  call vert_AZ_Q(gZu,ntry, ex4(:), wf4(:,3), wf8(:,1), n3(:,3), t3x8(:,:,1))
  call vert_AV_Q(ntry, ex4(:), ex5(:), wf4(:,5), n3(:,4), t3x4(:,:,3))
  call prop_A_Q(ntry, wf4(:,5), 24, MC, 1_intkind1, wf4(:,6), n2(3))
  call vert_ZQ_A(gZu,ntry, wf4(:,3), ex3(:), wf8(:,2), n3(:,5), t3x8(:,:,2))
  call vert_QA_V(ntry, ex3(:), ex4(:), wf4(:,7), n3(:,6), t3x4(:,:,4))
  call counter_VG_G(ntry, wf4(:,3), ex5(:), 16, wf8(:,3), 19, n3(:,7), t3x8(:,:,3))
  call counter_AZ_Q(gZu,ntry, ex4(:), wf4(:,3), wf8(:,4), n3(:,8), t3x8(:,:,4))
  call counter_AV_Q(ntry, ex4(:), ex5(:), wf4(:,8), n3(:,9), t3x4(:,:,5))
  call prop_A_Q(ntry, wf4(:,8), 24, MC, 1_intkind1, wf4(:,9), n2(4))
  call counter_ZQ_A(gZu,ntry, wf4(:,3), ex3(:), wf8(:,5), n3(:,10), t3x8(:,:,5))
  call counter_VQ_A(ntry, ex5(:), ex3(:), wf4(:,10), n3(:,11), t3x4(:,:,6))
  call prop_Q_A(ntry, wf4(:,10), 20, MC, 1_intkind1, wf4(:,11), n2(5))
  call counter_Q_A(ctcc,4,ntry, wf4(:,4), 20, wf4(:,12), n2(6))
  call prop_A_Q(ntry, wf8(:,1), 11, MC, 1_intkind1, wf8(:,6), n2(7))
  call counter_A_Q(ctcc,4,ntry, wf4(:,6), 24, wf4(:,13), n2(8))
  call prop_Q_A(ntry, wf8(:,2), 7, MC, 1_intkind1, wf8(:,7), n2(9))
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call Hcont_QA(nsync, wf4(:,4), wf8(:,1), A(:,1), n3(:,12), t3x32(:,:,1), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,6), wf8(:,2), A(:,2), n3(:,13), t3x32(:,:,2), nhel, den(5))

    call Hcont_VV(nsync, wf4(:,7), wf8(:,3), A(:,3), n3(:,14), t3x32(:,:,3), nhel, den(7))
    call Hcont_QA(nsync, wf4(:,4), wf8(:,4), A(:,4), n3(:,15), t3x32(:,:,4), nhel, den(3))
    call Hcont_QA(nsync, wf8(:,2), wf4(:,9), A(:,5), n3(:,16), t3x32(:,:,5), nhel, den(5))
    call Hcont_QA(nsync, wf4(:,6), wf8(:,5), A(:,6), n3(:,17), t3x32(:,:,6), nhel, den(5))
    call Hcont_QA(nsync, wf8(:,1), wf4(:,11), A(:,7), n3(:,18), t3x32(:,:,7), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,12), wf8(:,6), A(:,8), n3(:,19), t3x32(:,:,8), nhel, den(10))
    call Hcont_QA(nsync, wf4(:,13), wf8(:,7), A(:,9), n3(:,20), t3x32(:,:,9), nhel, den(13))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M1(1)%j = (A(j,1)%j+A(j,2)%j)*f(1)

  M2(1)%j = ((-A(j,8)%j-A(j,9)%j)*f(2)+(A(j,5)%j+A(j,7)%j)*f(3)+(A(j,4)%j+A(j,6)%j)*f(4)-A(j,3)%j*f(5))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_nenexccxg_1_/**/REALKIND, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_nenexccxg_1_/**/REALKIND, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_nenexccxg_1_/**/REALKIND, only: KL
  implicit none
...
! spin-correlated hard scattering factor for nu_e anti-nu_e charm anti-charm glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND
  implicit none
  real(DREALKIND), intent(in) :: P_scatt(0:3,5)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_nenexccxg_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 5
    r = external_perm_inv_ppllj_nenexccxg_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_nenexccxg_1_/**/REALKIND
