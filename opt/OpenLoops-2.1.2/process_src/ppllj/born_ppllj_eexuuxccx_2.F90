...
module ol_colourmatrix_ppllj_eexuuxccx_2_/**/REALKIND
...
end module ol_colourmatrix_ppllj_eexuuxccx_2_/**/REALKIND



module ol_forced_parameters_ppllj_eexuuxccx_2_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_eexuuxccx_2_/**/REALKIND

module ol_tree_ppllj_eexuuxccx_2_/**/REALKIND
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllj_eexuuxccx_2")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppllj_eexuuxccx_2_/**/DREALKIND, only: &
    & ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for e- e+ up anti-up charm anti-charm -> 0
...
! 
  ...
  use ol_external_ppllj_eexuuxccx_2, only: &
    & external_perm_ppllj_eexuuxccx_2, &
    & external_perm_inv_ppllj_eexuuxccx_2, &
    & extcomb_perm_ppllj_eexuuxccx_2, &
    & average_factor_ppllj_eexuuxccx_2, &
    & photonid_ppllj_eexuuxccx_2
  use ol_external_ppllj_eexuuxccx_2, only: &
    & H, hel_not_initialised, hel_init, POLSEL 
  use ol_colourmatrix_ppllj_eexuuxccx_2_/**/REALKIND, only: &
    & colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllj_eexuuxccx_2_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_ppllj_eexuuxccx_2
  ...
  type(wfun) :: wf4(4,5), wf8(8,18), wf64(64,8)
  ...
  extmasses2 = [ 0, 0, 0, 0, nMC, nMC ]
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0) and compute
  ! internal-propagator momenta in light-cone representation
  call init_kinematics(P_scatt, extmasses2, P, &
      &  external_perm_inv_ppllj_eexuuxccx_2, 6,.false.)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllj_eexuuxccx_2(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllj_eexuuxccx_2(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = &
       & extcomb_perm_ppllj_eexuuxccx_2(extcombs(k))
  end do
  ...
  call pol_wf_Q(P(:,5), rMC, H5, ex5, POLSEL(5))
  ...
  if (ntry == 1) then
    ...
    if (any(POLSEL /= 0)) then
      ...
      call pol_wf_Q(P(:,5), rMC, H5, ex5, 0)
  ...
  call prop_Q_A(ntry, wf8(:,1), 19, MC, 1_intkind1, wf8(:,3), n2(1))
  call vert_QA_Z(gZl,ntry, ex1, ex2, wf4(:,3), n3(:,5), t3x4(:,:,3))
  call prop_W_W(ntry, wf4(:,3), 3, MZ, 1_intkind1, wf4(:,4), n2(2))
  call vert_ZQ_A(gZu,ntry, wf4(:,4), ex5, wf8(:,4), n3(:,6), t3x8(:,:,3))
  call prop_Q_A(ntry, wf8(:,4), 19, MC, 1_intkind1, wf8(:,5), n2(3))
  ...
  call prop_Q_A(ntry, wf8(:,15), 52, ZERO, 0_intkind1, wf8(:,17), n2(7))
  ...
  if ( JBmunu /= 0 ) then ! POWHEGs Bmunu helicity correlation
    ...
    M2munu = M2munu / average_factor_ppllj_eexuuxccx_2
  end if

  ! Colour and helicity average and symmetry factor of outgoing particles
  M2add = M2 / average_factor_ppllj_eexuuxccx_2

  do k = 0, 23-1
    M2(k) = M2add(extcomb_perm_ppllj_eexuuxccx_2(k))
  end do

  ! photon-factors
  call photon_factors(photonid_ppllj_eexuuxccx_2, 0, bornphotonfactor)
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_QA(nsync, wf8(:,2), wf8(:,3), A(:,1), n3(:,17), t3x64(:,:,1), nhel, den(5))
    call cont_QA(nsync, wf8(:,2), wf8(:,5), A(:,2), n3(:,18), t3x64(:,:,2), nhel, den(8))
    call cont_QA(nsync, wf8(:,7), wf8(:,8), A(:,3), n3(:,19), t3x64(:,:,3), nhel, den(11))
    call cont_QA(nsync, wf8(:,8), wf8(:,9), A(:,4), n3(:,20), t3x64(:,:,4), nhel, den(12))
    call cont_QA(nsync, wf8(:,11), wf8(:,12), A(:,5), n3(:,21), t3x64(:,:,5), nhel, den(16))
    call cont_QA(nsync, wf8(:,11), wf8(:,14), A(:,6), n3(:,22), t3x64(:,:,6), nhel, den(18))
    call cont_QA(nsync, wf8(:,16), wf8(:,17), A(:,7), n3(:,23), t3x64(:,:,7), nhel, den(21))
    call cont_QA(nsync, wf8(:,17), wf8(:,18), A(:,8), n3(:,24), t3x64(:,:,8), nhel, den(22))
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = (((-A(j,1)%j-A(j,3)%j-A(j,5)%j-A(j,7)%j)*f(1))/2._/**/REALKIND+((A(j,2)%j+A(j,4)%j+A(j,6)%j &
       +A(j,8)%j)*f(2))/2._/**/REALKIND)
  M1(2) = (((A(j,1)%j+A(j,3)%j+A(j,5)%j+A(j,7)%j)*f(1))/6._/**/REALKIND+((-A(j,2)%j-A(j,4)%j-A(j,6)%j &
       -A(j,8)%j)*f(2))/6._/**/REALKIND)
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppllj_eexuuxccx_2_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllj_eexuuxccx_2")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllj_eexuuxccx_2")
#else
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_qp_ppllj_eexuuxccx_2")
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllj_eexuuxccx_2")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllj_eexuuxccx_2")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllj_eexuuxccx_2")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllj_eexuuxccx_2")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllj_eexuuxccx_2")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllj_eexuuxccx_2")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllj_eexuuxccx_2")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllj_eexuuxccx_2")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllj_eexuuxccx_2")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
...
end module ol_tree_ppllj_eexuuxccx_2_/**/REALKIND
