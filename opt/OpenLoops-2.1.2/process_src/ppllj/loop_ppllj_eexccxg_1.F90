...
module ol_colourmatrix_ppllj_eexccxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_eexccxg_1_/**/REALKIND



module ol_forced_parameters_ppllj_eexccxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_eexccxg_1_/**/REALKIND


! **********************************************************************
module ol_loop_storage_ppllj_eexccxg_1_/**/REALKIND
! **********************************************************************
...
! **********************************************************************
module ol_loop_ppllj_eexccxg_1_/**/REALKIND
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND, intkind1, intkind2
  use ol_data_types_/**/REALKIND, only: wfun, Hpolcont
  use ol_loop_storage_ppllj_eexccxg_1_/**/REALKIND
  implicit none
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f( 5) = (2*CI*countertermnorm*ctGcc*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 6) = CI*countertermnorm*ctGcc*eQED**2*gQCD**3
    f( 7) = (2*CI*countertermnorm*ctVcc*eQED**2*gQCD**3)/3._/**/REALKIND
    f( 8) = CI*countertermnorm*ctVcc*eQED**2*gQCD**3
    ...
    f(14) = (eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
    f(15) = (2*eQED**2*gQCD**3*integralnorm*SwF)/3._/**/REALKIND
...
subroutine denominators()
  ...
  den(6) = 1 /((L(5,24) - MC2)+L(6,24))
  den(9) = 1 /((L(5,12))+L(6,12))
  den(11) = 1 /((L(5,11) - MC2)+L(6,11))
  den(16) = 1 /((L(5,7) - MC2)+L(6,7))
...
! **********************************************************************
#ifndef PRECISION_dp
  use ol_loop_storage_ppllj_eexccxg_1_/**/DREALKIND, only: hel_states
#endif
  implicit none
...
! M2  = helicity-summed squared matrix element for e- e+ charm anti-charm glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND !, only: ci, parameters_status, ZERO, scalefactor, >masses<
  use ol_parameters_init_/**/REALKIND, only: ensure_mp_init, ensure_mp_loop_init
  use ol_kinematics_/**/REALKIND, only: init_kinematics
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_settings_ppllj_eexccxg_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_forced_parameters_ppllj_eexccxg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_eexccxg_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_eexccxg_1, 5, qp_kinematics)
  ...
  call prop_Q_A(ntry, wf4(:,2), 20, MC, 1_intkind1, wf4(:,3), n2(1))
  ...
  call prop_A_Q(ntry, wf4(:,6), 24, MC, 1_intkind1, wf4(:,7), n2(3))
  ...
  call counter_AZ_Q(gZu,ntry, ex4(:), wf4(:,5), wf8(:,7), n3(:,12), t3x8(:,:,7))
  call counter_AV_Q(ntry, ex4(:), ex5(:), wf4(:,9), n3(:,13), t3x4(:,:,6))
  call prop_A_Q(ntry, wf4(:,9), 24, MC, 1_intkind1, wf4(:,10), n2(4))
  call counter_VQ_A(ntry, wf4(:,1), ex3(:), wf8(:,8), n3(:,14), t3x8(:,:,8))
  call counter_ZQ_A(gZu,ntry, wf4(:,5), ex3(:), wf8(:,9), n3(:,15), t3x8(:,:,9))
  call counter_VQ_A(ntry, ex5(:), ex3(:), wf4(:,11), n3(:,16), t3x4(:,:,7))
  call prop_Q_A(ntry, wf4(:,11), 20, MC, 1_intkind1, wf4(:,12), n2(5))
  call counter_Q_A(ctcc,4,ntry, wf4(:,3), 20, wf4(:,13), n2(6))
  call prop_A_Q(ntry, wf8(:,1), 11, MC, 1_intkind1, wf8(:,10), n2(7))
  call prop_A_Q(ntry, wf8(:,2), 11, MC, 1_intkind1, wf8(:,11), n2(8))
  call counter_A_Q(ctcc,4,ntry, wf4(:,7), 24, wf4(:,14), n2(9))
  call prop_Q_A(ntry, wf8(:,3), 7, MC, 1_intkind1, wf8(:,12), n2(10))
  call prop_Q_A(ntry, wf8(:,4), 7, MC, 1_intkind1, wf8(:,13), n2(11))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M2(1)%j = ((A(j,14)%j+A(j,16)%j)*f(3)+(-A(j,15)%j-A(j,17)%j)*f(4)+(-A(j,8)%j-A(j,12)%j)*f(5)+(A(j,9)%j+A(j,13)%j)*f(6)+( &
       -A(j,6)%j-A(j,10)%j)*f(7)+(A(j,7)%j+A(j,11)%j)*f(8)-A(j,5)%j*f(9))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_eexccxg_1_/**/REALKIND, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_eexccxg_1_/**/REALKIND, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_eexccxg_1_/**/REALKIND, only: KL
  implicit none
...
! spin-correlated hard scattering factor for e- e+ charm anti-charm glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND
  implicit none
  real(DREALKIND), intent(in) :: P_scatt(0:3,5)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_eexccxg_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 5
    r = external_perm_inv_ppllj_eexccxg_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_eexccxg_1_/**/REALKIND
