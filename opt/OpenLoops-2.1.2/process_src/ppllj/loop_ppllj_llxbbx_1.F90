...
module ol_colourmatrix_ppllj_llxbbx_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_llxbbx_1_/**/REALKIND



module ol_forced_parameters_ppllj_llxbbx_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_llxbbx_1_/**/REALKIND


! **********************************************************************
module ol_loop_storage_ppllj_llxbbx_1_/**/REALKIND
! **********************************************************************
...
! **********************************************************************
module ol_loop_ppllj_llxbbx_1_/**/REALKIND
! **********************************************************************
  use KIND_TYPES, only: REALKIND, DREALKIND, intkind1, intkind2
  use ol_data_types_/**/REALKIND, only: wfun, Hpolcont
  use ol_loop_storage_ppllj_llxbbx_1_/**/REALKIND
  implicit none
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(2) = CI*eQED**2
    f(3) = (CI*countertermnorm*ctVbb*eQED**2*gQCD**2)/3._/**/REALKIND
    f(4) = CI*countertermnorm*ctVbb*eQED**2*gQCD**2
    f(5) = (CI*eQED**2*MB*ML)/(4._/**/REALKIND*MW**2*sw**2)
    f(6) = (CI*countertermnorm*ctSbb*eQED**2*gQCD**2*MB*ML)/(4._/**/REALKIND*MW**2*sw**2)
    f(7) = (eQED**2*gQCD**2*integralnorm*SwB)/3._/**/REALKIND
    f(8) = eQED**2*gQCD**2*integralnorm*SwB
    f(9) = (eQED**2*gQCD**2*integralnorm*MB*ML*SwB)/(MW**2*sw**2*4._/**/REALKIND)
...
! **********************************************************************
#ifndef PRECISION_dp
  use ol_loop_storage_ppllj_llxbbx_1_/**/DREALKIND, only: hel_states
#endif
  implicit none
  ...
  allocate(A(hel_states,6))
  allocate(M1helarray(1,hel_states))
  allocate(M1helarray_ct(1,hel_states))
  allocate(M0_col1_helarray(1,hel_states))
  A(1:hel_states,1:6)%j = 0
  A(1:hel_states,1:6)%e = 0
  A(1:hel_states,1:6)%hf = 0
  A(1:hel_states,1:6)%s = 0
#if 1 > 0
...
! M2  = helicity-summed squared matrix element for tau- tau+ bottom anti-bottom -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND !, only: ci, parameters_status, ZERO, scalefactor, >masses<
  use ol_parameters_init_/**/REALKIND, only: ensure_mp_init, ensure_mp_loop_init
  use ol_kinematics_/**/REALKIND, only: init_kinematics
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_settings_ppllj_llxbbx_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_forced_parameters_ppllj_llxbbx_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_llxbbx_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_llxbbx_1, 4, qp_kinematics)
  ...
  call vert_QA_V(ntry, ex1(:), ex2(:), wf4(:,3), n3(:,3), t3x4(:,:,3))
  call vert_QA_V(ntry, ex3(:), ex4(:), wf4(:,4), n3(:,4), t3x4(:,:,4))
  call vert_QA_Z(gZl,ntry, ex1(:), ex2(:), wf4(:,5), n3(:,5), t3x4(:,:,5))
  call vert_QA_Z(gZd,ntry, ex3(:), ex4(:), wf4(:,6), n3(:,6), t3x4(:,:,6))
  call prop_W_W(ntry, wf4(:,5), 3, MZ, 1_intkind1, wf4(:,7), n2(1))
  call counter_AQ_S(gH,ntry, ex4(:), ex3(:), wf4(:,8), n3(:,7), t3x4(:,:,7))
  call counter_QA_V(ntry, ex3(:), ex4(:), wf4(:,9), n3(:,8), t3x4(:,:,8))
  call counter_QA_Z(gZd,ntry, ex3(:), ex4(:), wf4(:,10), n3(:,9), t3x4(:,:,9))
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call Hcont_SS(nsync, wf4(:,1), wf4(:,2), A(:,1), n3(:,10), t3x16(:,:,1), nhel, den(1))
    call Hcont_VV(nsync, wf4(:,3), wf4(:,4), A(:,2), n3(:,11), t3x16(:,:,2), nhel, den(2))
    call Hcont_VV(nsync, wf4(:,6), wf4(:,7), A(:,3), n3(:,12), t3x16(:,:,3), nhel, den(3))

    call Hcont_SS(nsync, wf4(:,1), wf4(:,8), A(:,4), n3(:,13), t3x16(:,:,4), nhel, den(1))
    call Hcont_VV(nsync, wf4(:,3), wf4(:,9), A(:,5), n3(:,14), t3x16(:,:,5), nhel, den(2))
    call Hcont_VV(nsync, wf4(:,7), wf4(:,10), A(:,6), n3(:,15), t3x16(:,:,6), nhel, den(3))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M1(1)%j = (A(j,2)%j*f(1)+A(j,3)%j*f(2)-A(j,1)%j*f(5))

  M2(1)%j = (A(j,5)%j*f(3)+A(j,6)%j*f(4)-A(j,4)%j*f(6))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxbbx_1_/**/REALKIND, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxbbx_1_/**/REALKIND, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_llxbbx_1_/**/REALKIND, only: KL
  implicit none
...
! spin-correlated hard scattering factor for tau- tau+ bottom anti-bottom -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND
  implicit none
  real(DREALKIND), intent(in) :: P_scatt(0:3,4)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_llxbbx_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 4
    r = external_perm_inv_ppllj_llxbbx_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_llxbbx_1_/**/REALKIND
