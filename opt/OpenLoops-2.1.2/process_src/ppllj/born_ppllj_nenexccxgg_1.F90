...
module ol_colourmatrix_ppllj_nenexccxgg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_nenexccxgg_1_/**/REALKIND



module ol_forced_parameters_ppllj_nenexccxgg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_nenexccxgg_1_/**/REALKIND

module ol_tree_ppllj_nenexccxgg_1_/**/REALKIND
...
subroutine born_denominators_init()
  ...
  den(3) = 1 /((L(5,40) - MC2)+L(6,40))
  den(6) = 1 /((L(5,11) - MC2)+L(6,11))
  den(9) = 1 /((L(5,36) - MC2)+L(6,36))
  den(10) = 1 /((L(5,24) - MC2)+L(6,24))
  den(13) = 1 /((L(5,7) - MC2)+L(6,7))
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllj_nenexccxgg_1")
#else
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu)
  use ol_tree_ppllj_nenexccxgg_1_/**/DREALKIND, only: &
    & ntry, nhel, Hel, M1helarr
#endif
! P_scatt(0:3,Npart) = incoming external momenta
! M2  = helicity-summed squared matrix element for nu_e anti-nu_e charm anti-charm glue glue -> 0
...
! 
  ...
  use ol_external_ppllj_nenexccxgg_1, only: &
    & external_perm_ppllj_nenexccxgg_1, &
    & external_perm_inv_ppllj_nenexccxgg_1, &
    & extcomb_perm_ppllj_nenexccxgg_1, &
    & average_factor_ppllj_nenexccxgg_1, &
    & photonid_ppllj_nenexccxgg_1
  use ol_external_ppllj_nenexccxgg_1, only: &
    & H, hel_not_initialised, hel_init, POLSEL 
  use ol_colourmatrix_ppllj_nenexccxgg_1_/**/REALKIND, only: &
    & colmat_not_initialised, colourmatrix_init
  use ol_forced_parameters_ppllj_nenexccxgg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_ppllj_nenexccxgg_1
  ...
  extmasses2 = [ 0, 0, nMC, nMC, 0, 0 ]
  ! Convert 2 -> n-2 PS-point to n -> 0 (so that P(1) + ... + P(n) = 0) and compute
  ! internal-propagator momenta in light-cone representation
  call init_kinematics(P_scatt, extmasses2, P, &
      &  external_perm_inv_ppllj_nenexccxgg_1, 6,.false.)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllj_nenexccxgg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllj_nenexccxgg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = &
       & extcomb_perm_ppllj_nenexccxgg_1(extcombs(k))
  end do
  ...
  call prop_W_W(ntry, wf4(:,1), 3, MZ, 1_intkind1, wf4(:,4), n2(1))
  call prop_Q_A(ntry, wf4(:,2), 20, MC, 1_intkind1, wf4(:,5), n2(2))
  call prop_A_Q(ntry, wf4(:,3), 40, MC, 1_intkind1, wf4(:,6), n2(3))
  call vert_ZQ_A(gZu,ntry, wf4(:,4), wf4(:,5), wf16(:,1), n3(:,4), t3x16(:,:,1))
  call vert_AZ_Q(gZu,ntry, ex4, wf4(:,4), wf8(:,1), n3(:,5), t3x8(:,:,1))
  call vert_VQ_A(ntry, ex6, wf4(:,5), wf8(:,2), n3(:,6), t3x8(:,:,2))
  call prop_A_Q(ntry, wf8(:,1), 11, MC, 1_intkind1, wf8(:,3), n2(4))
  call vert_VQ_A(ntry, ex6, ex3, wf4(:,7), n3(:,7), t3x4(:,:,4))
  call vert_AV_Q(ntry, ex4, ex5, wf4(:,8), n3(:,8), t3x4(:,:,5))
  call prop_Q_A(ntry, wf4(:,7), 36, MC, 1_intkind1, wf4(:,9), n2(5))
  call prop_A_Q(ntry, wf4(:,8), 24, MC, 1_intkind1, wf4(:,10), n2(6))
  call vert_ZQ_A(gZu,ntry, wf4(:,4), wf4(:,9), wf16(:,2), n3(:,9), t3x16(:,:,2))
  call vert_ZQ_A(gZu,ntry, wf4(:,4), ex3, wf8(:,4), n3(:,10), t3x8(:,:,3))
  call vert_AV_Q(ntry, wf4(:,10), ex6, wf8(:,5), n3(:,11), t3x8(:,:,4))
  call prop_Q_A(ntry, wf8(:,4), 7, MC, 1_intkind1, wf8(:,6), n2(7))
  call vert_VQ_A(ntry, ex5, wf4(:,9), wf8(:,7), n3(:,12), t3x8(:,:,5))
  call vert_AV_Q(ntry, wf4(:,6), ex5, wf8(:,8), n3(:,13), t3x8(:,:,6))
  call vert_UV_W(ntry, ex5, 16, ex6, 32, wf4(:,11), n3(:,14), t3x4(:,:,6))
  call vert_AV_Q(ntry, ex4, wf4(:,11), wf8(:,9), n3(:,15), t3x8(:,:,7))
  call vert_VQ_A(ntry, wf4(:,11), ex3, wf8(:,10), n3(:,16), t3x8(:,:,8))
  call prop_Q_A(ntry, wf8(:,10), 52, MC, 1_intkind1, wf8(:,11), n2(8))
  ...
  if ( JBmunu /= 0 ) then ! POWHEGs Bmunu helicity correlation
    ...
    M2munu = M2munu / average_factor_ppllj_nenexccxgg_1
  end if

  ! Colour and helicity average and symmetry factor of outgoing particles
  M2add = M2 / average_factor_ppllj_nenexccxgg_1

  do k = 0, 23-1
    M2(k) = M2add(extcomb_perm_ppllj_nenexccxgg_1(k))
  end do

  ! photon-factors
  call photon_factors(photonid_ppllj_nenexccxgg_1, 0, bornphotonfactor)
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((A(j,3)%j+A(j,4)%j+A(j,5)%j)*f(1)+CI*(A(j,7)%j+A(j,8)%j)*f(2))
  M1(2) = ((A(j,1)%j+A(j,2)%j+A(j,6)%j)*f(1)+CI*(-A(j,7)%j-A(j,8)%j)*f(2))
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppllj_nenexccxgg_1_/**/REALKIND, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllj_nenexccxgg_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllj_nenexccxgg_1")
#else
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_qp_ppllj_nenexccxgg_1")
#endif
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllj_nenexccxgg_1")
#else
subroutine amp2ccone(P, M2, I, J)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllj_nenexccxgg_1")
#else
subroutine amp2ccall(P, M2)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllj_nenexccxgg_1")
#else
subroutine amp2hcone(P, M2, I, J, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllj_nenexccxgg_1")
#else
subroutine amp2hcall(P, M2, I, MOM)
#endif
  ! This is an interface for AMP2.
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllj_nenexccxgg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllj_nenexccxgg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllj_nenexccxgg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllj_nenexccxgg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllj_nenexccxgg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,6)
...
end module ol_tree_ppllj_nenexccxgg_1_/**/REALKIND
