...
module ol_colourmatrix_ppllj_nlxludxg_1_/**/REALKIND
...
end module ol_colourmatrix_ppllj_nlxludxg_1_/**/REALKIND



module ol_forced_parameters_ppllj_nlxludxg_1_/**/REALKIND
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_nlxludxg_1_/**/REALKIND


! **********************************************************************
module ol_loop_storage_ppllj_nlxludxg_1_/**/REALKIND
! **********************************************************************
  ...
  type(wfun) :: wf4(4,12), wf8(8,6), wf32(32,8)

  ! diagram prefactors
  integer,           save :: fac_status_loop1 = -1, fac_status_loop2 = -1
  complex(REALKIND), save :: f(6), c(3)

  !Vector in helicity and colour space for Born-Loop interference
  type(Hpolcont),   save, allocatable :: M0_col1_helarray(:,:), M0M1_hel_cc(:,:,:)
  complex(REALKIND), save :: M2ctcc(16)

end module ol_loop_storage_ppllj_nlxludxg_1_/**/REALKIND
...
! **********************************************************************
module ol_loop_ppllj_nlxludxg_1_/**/REALKIND
! **********************************************************************
...
!  use ol_loop_parameters_decl_/**/DREALKIND, only: DOI
  ...
  ! factors of the diagrams
    ...
    f(3) = (CI*countertermnorm*ctGqq*eQED**2*gQCD**3)/(2._/**/REALKIND*sw**2)
    f(4) = (CI*countertermnorm*ctVqq*eQED**2*gQCD**3)/(2._/**/REALKIND*sw**2)
    f(5) = (CI*eQED**2*gQCD**3*integralnorm*SwB)/(2._/**/REALKIND*sw**2)
    f(6) = (eQED**2*gQCD**3*integralnorm*SwB)/(sw**2*2._/**/REALKIND)

  c = [ 9*CI*f(5), f(6), 8*f(6) ]
  c = (1._/**/REALKIND / 6) * c
...
! **********************************************************************
#ifndef PRECISION_dp
  use ol_loop_storage_ppllj_nlxludxg_1_/**/DREALKIND, only: hel_states
#endif
  implicit none
  ...
  allocate(A(hel_states,8))
  allocate(M1helarray(1,hel_states))
  allocate(M1helarray_ct(1,hel_states))
  allocate(M0_col1_helarray(1,hel_states))
  A(1:hel_states,1:8)%j = 0
  A(1:hel_states,1:8)%e = 0
  A(1:hel_states,1:8)%hf = 0
  A(1:hel_states,1:8)%s = 0
#if 1 > 0
...
! M2  = helicity-summed squared matrix element for anti-nu_tau tau- up anti-down glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND !, only: ci, parameters_status, ZERO, scalefactor, >masses<
  use ol_parameters_init_/**/REALKIND, only: ensure_mp_init, ensure_mp_loop_init
  use ol_kinematics_/**/REALKIND, only: init_kinematics
  use ol_momenta_decl_/**/DREALKIND, only: momenta_nan_check
  use ol_settings_ppllj_nlxludxg_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_forced_parameters_ppllj_nlxludxg_1_/**/REALKIND, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_nlxludxg_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_nlxludxg_1, 5, qp_kinematics)
  ...
  call pol_wf_Q(P(:,2), rML, H2, ex2, POLSEL(2),2)
  ...
  call prop_Q_A(ntry, wf4(:,2), 20, ZERO, 0_intkind1, wf4(:,4), n2(2))
  call vert_AW_Q(ntry, ex4(:), wf4(:,3), wf8(:,1), n3(:,3), t3x8(:,:,1))
  call vert_AV_Q(ntry, ex4(:), ex5(:), wf4(:,5), n3(:,4), t3x4(:,:,3))
  call prop_A_Q(ntry, wf4(:,5), 24, ZERO, 0_intkind1, wf4(:,6), n2(3))
  call vert_WQ_A(ntry, wf4(:,3), ex3(:), wf8(:,2), n3(:,5), t3x8(:,:,2))
  call counter_AW_Q(ntry, ex4(:), wf4(:,3), wf8(:,3), n3(:,6), t3x8(:,:,3))
  call counter_AV_Q(ntry, ex4(:), ex5(:), wf4(:,7), n3(:,7), t3x4(:,:,4))
  call prop_A_Q(ntry, wf4(:,7), 24, ZERO, 0_intkind1, wf4(:,8), n2(4))
  call counter_WQ_A(ntry, wf4(:,3), ex3(:), wf8(:,4), n3(:,8), t3x8(:,:,4))
  call counter_VQ_A(ntry, ex5(:), ex3(:), wf4(:,9), n3(:,9), t3x4(:,:,5))
  call prop_Q_A(ntry, wf4(:,9), 20, ZERO, 0_intkind1, wf4(:,10), n2(5))
  call counter_Q_A(ctqq,2,ntry, wf4(:,4), 20, wf4(:,11), n2(6))
  call prop_A_Q(ntry, wf8(:,1), 11, ZERO, 0_intkind1, wf8(:,5), n2(7))
  call counter_A_Q(ctqq,1,ntry, wf4(:,6), 24, wf4(:,12), n2(8))
  call prop_Q_A(ntry, wf8(:,2), 7, ZERO, 0_intkind1, wf8(:,6), n2(9))
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call Hcont_QA(nsync, wf4(:,4), wf8(:,1), A(:,1), n3(:,10), t3x32(:,:,1), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,6), wf8(:,2), A(:,2), n3(:,11), t3x32(:,:,2), nhel, den(5))

    call Hcont_QA(nsync, wf4(:,4), wf8(:,3), A(:,3), n3(:,12), t3x32(:,:,3), nhel, den(3))
    call Hcont_QA(nsync, wf8(:,2), wf4(:,8), A(:,4), n3(:,13), t3x32(:,:,4), nhel, den(5))
    call Hcont_QA(nsync, wf4(:,6), wf8(:,4), A(:,5), n3(:,14), t3x32(:,:,5), nhel, den(5))
    call Hcont_QA(nsync, wf8(:,1), wf4(:,10), A(:,6), n3(:,15), t3x32(:,:,6), nhel, den(3))
    call Hcont_QA(nsync, wf4(:,11), wf8(:,5), A(:,7), n3(:,16), t3x32(:,:,7), nhel, den(8))
    call Hcont_QA(nsync, wf4(:,12), wf8(:,6), A(:,8), n3(:,17), t3x32(:,:,8), nhel, den(11))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M2(1)%j = ((A(j,7)%j+A(j,8)%j)*f(2)+(-A(j,4)%j-A(j,6)%j)*f(3)+(-A(j,3)%j-A(j,5)%j)*f(4))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_nlxludxg_1_/**/REALKIND, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_nlxludxg_1_/**/REALKIND, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_nlxludxg_1_/**/REALKIND, only: KL
  implicit none
...
! spin-correlated hard scattering factor for anti-nu_tau tau- up anti-down glue -> 0
! **********************************************************************
  use ol_parameters_decl_/**/REALKIND
  implicit none
  real(DREALKIND), intent(in) :: P_scatt(0:3,5)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_nlxludxg_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 5
    r = external_perm_inv_ppllj_nlxludxg_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_nlxludxg_1_/**/REALKIND
