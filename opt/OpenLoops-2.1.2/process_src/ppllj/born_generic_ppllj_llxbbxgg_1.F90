module ol_external_ppllj_llxbbxgg_1
! 
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_ppllj_llxbbxgg_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: external_perm_inv_ppllj_llxbbxgg_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: extcomb_perm_ppllj_llxbbxgg_1(0:22) = &
                     [ (dummy_counter, dummy_counter = 0, 22) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_ppllj_llxbbxgg_1(6) = &
                     [ 1, 2, 3, 4, 5, 5 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_ppllj_llxbbxgg_1(6) = &
                     [ 2, 2, 6, 6, 16, 16 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_ppllj_llxbbxgg_1 = &
                     8
  integer, save :: channel_number_ppllj_llxbbxgg_1 = -1
  ...
  integer, save :: photonid_ppllj_llxbbxgg_1(6) = 0

  
  contains

  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_ppllj_llxbbxgg_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_ppllj_llxbbxgg_1")
  ...
  subroutine averagefactor_c(avgf) &
      & bind(c,name="ol_averagefactor_ppllj_llxbbxgg_1")
    use KIND_TYPES, only: DREALKIND
    implicit none
    real(DREALKIND), intent(out) :: avgf
    avgf = average_factor_ppllj_llxbbxgg_1
  end subroutine averagefactor_c

  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_ppllj_llxbbxgg_1")
    ...
    integer :: i, j, ii, jj
    integer :: particle_types_perm_ppllj_llxbbxgg_1(6)
    external_perm_ppllj_llxbbxgg_1 = perm
    do i = 1, 6
      external_perm_inv_ppllj_llxbbxgg_1( &
        external_perm_ppllj_llxbbxgg_1(i)) = i
      particle_types_perm_ppllj_llxbbxgg_1(i) = &
        particle_types_ppllj_llxbbxgg_1( &
        external_perm_ppllj_llxbbxgg_1(i))
    end do
    do i = 1, 6
      do j = 1, i
        if (external_perm_ppllj_llxbbxgg_1(i) >= &
          external_perm_ppllj_llxbbxgg_1(j)) then
          ii = external_perm_ppllj_llxbbxgg_1(i)
          jj = external_perm_ppllj_llxbbxgg_1(j)
        else
          ii = external_perm_ppllj_llxbbxgg_1(j)
          jj = external_perm_ppllj_llxbbxgg_1(i)
        end if
        extcomb_perm_ppllj_llxbbxgg_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_ppllj_llxbbxgg_1 = 1
    do i = 1, n_scatt
      average_factor_ppllj_llxbbxgg_1 = &
        average_factor_ppllj_llxbbxgg_1 &
        * average_factors_ppllj_llxbbxgg_1( &
        external_perm_ppllj_llxbbxgg_1(i))
    end do
    ...
    if (out_symmetry_on /= 0) then
      do i = 1, 6
        average_factor_ppllj_llxbbxgg_1 = &
          average_factor_ppllj_llxbbxgg_1 &
          * factorial(count(particle_types_perm_ppllj_llxbbxgg_1(n_scatt+1:6) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_ppllj_llxbbxgg_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_last_perm(perm) &
      & bind(c,name="ol_f_get_last_perm_ppllj_llxbbxgg_1")
    integer, intent(out) :: perm(6)
    perm = external_perm_ppllj_llxbbxgg_1
  end subroutine get_last_perm


  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_ppllj_llxbbxgg_1")
    ...
    m_ex_orig = [ rML_unscaled, rML_unscaled, rMB_unscaled, rMB_unscaled, rZERO, rZERO ]
    do i = 1, 6
      m_ex(i) = m_ex_orig(external_perm_ppllj_llxbbxgg_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_ppllj_llxbbxgg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine get_types(t_ex) &
      & bind(c,name="ol_f_get_types_ppllj_llxbbxgg_1")
    ...
    integer :: t_ex_orig(6)
    ! External particle masses for in the identity permutation
    t_ex_orig = [3,3,2,2,1,1]
    do i = 1, 6
      t_ex(i) = t_ex_orig(external_perm_ppllj_llxbbxgg_1(i))
    end do
  end subroutine get_types


  subroutine get_types_c(t_ex) &
      & bind(c,name="ol_get_types_ppllj_llxbbxgg_1")
    use, intrinsic :: iso_c_binding, only: c_int
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine get_charges(c_ex) &
      & bind(c,name="ol_f_get_charges_ppllj_llxbbxgg_1")
    ! Return the type of the external particles in the current permutation.
    use KIND_TYPES, only: DREALKIND
    use ol_parameters_decl_/**/DREALKIND
    implicit none
    ...
    ! External particle masses for in the identity permutation
    c_ex_orig = [-3,3,-1,1,0,0]/3._/**/REALKIND
    do i = 1, 6
      c_ex(i) = c_ex_orig(external_perm_ppllj_llxbbxgg_1(i))
    end do
  end subroutine get_charges


  subroutine get_charges_c(c_ex) &
      & bind(c,name="ol_get_charge_ppllj_llxbbxgg_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_ppllj_llxbbxgg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_ppllj_llxbbxgg_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine pol_init(pol) &
      & bind(c,name="ol_f_pol_init_ppllj_llxbbxgg_1")
    implicit none
  ...
  subroutine set_photons(photon_id) &
      & bind(c,name="ol_f_set_photons_ppllj_llxbbxgg_1")
    implicit none
    integer, intent(in) :: photon_id(6)
    photonid_ppllj_llxbbxgg_1 = photon_id
  end subroutine set_photons




end module ol_external_ppllj_llxbbxgg_1


module ol_settings_ppllj_llxbbxgg_1
  implicit none
  ...
  subroutine set_hel_mem_opt(set) &
      & bind(c,name="ol_hel_mem_opt_ppllj_llxbbxgg_1")
    implicit none
  ...
  subroutine set_loopcc(set,was) &
      & bind(c,name="ol_loopcc_ppllj_llxbbxgg_1")
    implicit none
    logical, intent(in) :: set
    logical :: was
    was = loopcc
...
end module ol_settings_ppllj_llxbbxgg_1

module colour_basis_ppllj_llxbbxgg_1
  implicit none
  ! tree colour basis
  integer, save :: extcolours(6) = [0,0,1,1,2,2]
  contains

  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_ppllj_llxbbxgg_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_ppllj_llxbbxgg_1")
    implicit none
  ...
  pure subroutine loop_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_loop_colbasis_dim_ppllj_llxbbxgg_1")
    implicit none
    ! colour representation of external particles: 0=neutral, 1=fundamental, 2=adjoint
    integer, intent(out) :: extcols(6)
  ...
  subroutine loop_colbasis(basis, powers) &
    & bind(c, name="ol_loop_colbasis_ppllj_llxbbxgg_1")
    implicit none
    integer, intent(out) :: powers(1,1)
    integer, intent(out) :: basis(3,2)
#if 1 > 0
...
end module colour_basis_ppllj_llxbbxgg_1

! **********************************************************************
module ol_heltables_ppllj_llxbbxgg_1
! **********************************************************************
  ...
  integer(intkind2), save :: n2(12), n3(3,50)

  ! helicity tables used in the construction of the h helicity states of a wave function (amplitude)
  ! from an v-point vertex (contraction): t<v>x<h>(v-1,h,sz)
  integer(intkind2), save :: t3x16(2,16,6), t3x4(2,4,8), t3x8(2,8,12), t3x64(2,64,24)
...
! **********************************************************************
  ...
  n2(12) = 8
  call heltable([4,16,64], n3(:,27), t3x64(:,:,1))
  call heltable([4,16,64], n3(:,28), t3x64(:,:,2))
  call heltable([4,16,64], n3(:,29), t3x64(:,:,3))
  call heltable([8,8,64], n3(:,30), t3x64(:,:,4))
  call heltable([8,8,64], n3(:,31), t3x64(:,:,5))
  call heltable([8,8,64], n3(:,32), t3x64(:,:,6))
  call heltable([4,16,64], n3(:,33), t3x64(:,:,7))
  call heltable([4,16,64], n3(:,34), t3x64(:,:,8))
  call heltable([4,16,64], n3(:,35), t3x64(:,:,9))
  call heltable([8,8,64], n3(:,36), t3x64(:,:,10))
  call heltable([8,8,64], n3(:,37), t3x64(:,:,11))
  call heltable([8,8,64], n3(:,38), t3x64(:,:,12))
  call heltable([8,8,64], n3(:,39), t3x64(:,:,13))
  call heltable([8,8,64], n3(:,40), t3x64(:,:,14))
  call heltable([8,8,64], n3(:,41), t3x64(:,:,15))
  call heltable([8,8,64], n3(:,42), t3x64(:,:,16))
  call heltable([8,8,64], n3(:,43), t3x64(:,:,17))
  call heltable([8,8,64], n3(:,44), t3x64(:,:,18))
  call heltable([8,8,64], n3(:,45), t3x64(:,:,19))
  call heltable([8,8,64], n3(:,46), t3x64(:,:,20))
  call heltable([8,8,64], n3(:,47), t3x64(:,:,21))
  call heltable([8,8,64], n3(:,48), t3x64(:,:,22))
  call heltable([8,8,64], n3(:,49), t3x64(:,:,23))
  call heltable([8,8,64], n3(:,50), t3x64(:,:,24))

  heltables_not_init = .false.

end subroutine init_heltables

end module ol_heltables_ppllj_llxbbxgg_1
