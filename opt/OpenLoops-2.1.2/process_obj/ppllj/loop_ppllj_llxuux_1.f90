...
module ol_colourmatrix_ppllj_llxuux_1_dp
...
end module ol_colourmatrix_ppllj_llxuux_1_dp



module ol_forced_parameters_ppllj_llxuux_1_dp
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_llxuux_1_dp


! **********************************************************************
module ol_loop_storage_ppllj_llxuux_1_dp
! **********************************************************************
...
! **********************************************************************
module ol_loop_ppllj_llxuux_1_dp
! **********************************************************************
  use kind_types, only: dp, dp, intkind1, intkind2
  use ol_data_types_dp, only: wfun, Hpolcont
  use ol_loop_storage_ppllj_llxuux_1_dp
  implicit none
...
! **********************************************************************
  ...
  ! factors of the diagrams
    ...
    f(2) = CI*eQED**2
    f(3) = (2*CI*countertermnorm*ctVqq*eQED**2*gQCD**2)/3._dp
    f(4) = CI*countertermnorm*ctVqq*eQED**2*gQCD**2
    f(5) = (2*eQED**2*gQCD**2*integralnorm*SwB)/3._dp
...
! **********************************************************************
  ...
  allocate(A(hel_states,4))
  allocate(M1helarray(1,hel_states))
  allocate(M1helarray_ct(1,hel_states))
  allocate(M0_col1_helarray(1,hel_states))
  A(1:hel_states,1:4)%j = 0
  A(1:hel_states,1:4)%e = 0
  A(1:hel_states,1:4)%hf = 0
  A(1:hel_states,1:4)%s = 0
...
! **********************************************************************
  ...
  use ol_settings_ppllj_llxuux_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_heltables_OLR_ppllj_llxuux_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_llxuux_1, 4, qp_kinematics)
  ...
  call counter_QA_V(ntry, ex3(:), ex4(:), wf4(:,6), n3(:,5), t3x4(:,:,5))
  call counter_QA_Z(gZu,ntry, ex3(:), ex4(:), wf4(:,7), n3(:,6), t3x4(:,:,6))
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call Hcont_VV(nsync, wf4(:,1), wf4(:,2), A(:,1), n3(:,7), t3x16(:,:,1), nhel, den(1))
    call Hcont_VV(nsync, wf4(:,4), wf4(:,5), A(:,2), n3(:,8), t3x16(:,:,2), nhel, den(2))

    call Hcont_VV(nsync, wf4(:,1), wf4(:,6), A(:,3), n3(:,9), t3x16(:,:,3), nhel, den(1))
    call Hcont_VV(nsync, wf4(:,5), wf4(:,7), A(:,4), n3(:,10), t3x16(:,:,4), nhel, den(2))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M1(1)%j = (-(A(j,1)%j*f(1))+A(j,2)%j*f(2))

  M2(1)%j = (-(A(j,3)%j*f(3))+A(j,4)%j*f(4))
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxuux_1_dp, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_llxuux_1_dp, only: KL
  implicit none
...
! spin-correlated hard scattering factor for tau- tau+ up anti-up -> 0
! **********************************************************************
  use ol_parameters_decl_dp
  implicit none
  real(dp), intent(in) :: P_scatt(0:3,4)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_llxuux_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 4
    r = external_perm_inv_ppllj_llxuux_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_llxuux_1_dp
