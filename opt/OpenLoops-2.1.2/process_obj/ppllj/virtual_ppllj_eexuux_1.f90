...
module ol_vamp_ppllj_eexuux_1_dp
...
! **********************************************************************
  ...
  use ol_tensor_sum_storage_ppllj_eexuux_1_dp
...
! **********************************************************************
  ...
  use ol_tensor_sum_storage_ppllj_eexuux_1_dp
  use ol_loop_storage_ppllj_eexuux_1_dp, only: &
    merge_step, hel_states

  use ol_vamp_1_ppllj_eexuux_1_dp, only: vamp_1
...
! **********************************************************************
  ...
  use ol_colourmatrix_ppllj_eexuux_1_dp ! colmat_not_initialised, K1, K2, KL, KL2, KL2ct, KL2ct2
  ...
  use ol_tensor_sum_storage_ppllj_eexuux_1_dp, only: &
  ...
  use ol_tables_storage_ppllj_eexuux_1_dp, only: HOL_m3_init
  use ol_loop_ppllj_eexuux_1_dp, only: amp2
  use ol_loop_ppllj_eexuux_1_dp, only: init_merging_tables, update_merging_tables
  use ol_loop_storage_ppllj_eexuux_1_dp, only: &
    & fac_status_loop1, fac_status_loop2, M0_col1_helarray
  use ol_i_operator_dp, only: intdip
  use ol_loop_storage_ppllj_eexuux_1_dp, only: ntryL, nhel, p_switch, &
...
! **********************************************************************
  ...
  use ol_external_ppllj_eexuux_1, only: &
    & photonid_ppllj_eexuux_1
  use ol_colourmatrix_ppllj_eexuux_1_dp, only: &
...
! **********************************************************************
  ...
  use ol_external_ppllj_eexuux_1, only: & 
    ...
    & average_factor_ppllj_eexuux_1, &
    & photonid_ppllj_eexuux_1
  use ol_loop_storage_ppllj_eexuux_1_dp, only: M2ctcc, M0M1_hel_cc
  use ol_tensor_sum_storage_ppllj_eexuux_1_dp, only: reset_tensor_sum, &
    & integrate_tensor_sum
  use ol_debug, only: ol_fatal
  use ol_colourmatrix_ppllj_eexuux_1_dp, only: Cas
...
! **********************************************************************
 ...
 &  photonid_ppllj_eexuux_1
  use ol_momenta_decl_dp, only: momenta_nan_check
  use ol_loop_ppllj_eexuux_1_dp, only: hsfamp2
  use ol_ew_renormalisation_dp, only: photon_factors
  implicit none
  real(dp), intent(in) :: P_scatt(0:3,4)
...
end module ol_vamp_ppllj_eexuux_1_dp



module ol_vamp_ppllj_eexuux_1_qp
  implicit none
  contains
  subroutine vamp2pc(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, mode)
...
end module ol_vamp_ppllj_eexuux_1_qp
...
module ol_vamp_ppllj_eexuux_1
  ...
  subroutine finish_ppllj_eexuux_1()
    ...
    use kind_types, only: dp
!   use ol_data_types_dp, only: Hpolcont
!   use ol_tensor_sum_storage_ppllj_eexuux_1_dp, only: HOL_memory_allocation
    use ol_stability, only: finish_histograms
    use ol_vamp_ppllj_eexuux_1_dp, only: vamp
    use ol_tensor_sum_storage_ppllj_eexuux_1_dp, only: &
...
!   call vamp(Mdummy)
  ...
  subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2) &
    ...
    use ol_vamp_ppllj_eexuux_1_dp, only: vamp2dp => vamp2pc
    use ol_vamp_ppllj_eexuux_1_qp, only: vamp2qp => vamp2pc
  ...
  subroutine ctamp2(P_scatt, M2tree, M2ct) &
    ...
    use kind_types, only: dp
    use ol_vamp_ppllj_eexuux_1_dp, only: ctamp2base
    implicit none
    real(dp), intent(in)  :: P_scatt(0:3,4)
    real(dp), intent(out) :: M2tree, M2ct
  ...
  subroutine iopamp2(P_scatt, M2tree, IRL1) &
    ...
    use kind_types, only: dp
    use ol_vamp_ppllj_eexuux_1_dp, only: iopamp2base
    implicit none
    real(dp), intent(in)  :: P_scatt(0:3,4)
  ...
  subroutine schsfamp2(P_scatt, M2tree, i, j, M2hsf) &
    ...
    use kind_types, only: dp
    use ol_vamp_ppllj_eexuux_1_dp, only: schsfamp2base
    implicit none
    real(dp), intent(in)  :: P_scatt(0:3,4)
    integer, intent(in)  :: i, j
    real(dp), intent(out) :: M2tree, M2hsf
  ...
  subroutine vampcr2(P_scatt, M2L0, M2L1, M2L2, crtype, &
    ...
    use ol_vamp_ppllj_eexuux_1_dp, only: vamp2cache
    use ol_data_types_dp, only: correlator
    implicit none
