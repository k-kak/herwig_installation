...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllj_eexccxg_1")
...
! 
  ...
  use ol_forced_parameters_ppllj_eexccxg_1_dp, only: &
    & check_forced_parameters
  use ol_heltables_ppllj_eexccxg_1
  ...
  type(wfun) :: wf4(4,7), wf8(8,4), wf32(32,4)
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
      &  external_perm_inv_ppllj_eexccxg_1, 5,.false.)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllj_eexccxg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllj_eexccxg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = &
       & extcomb_perm_ppllj_eexccxg_1(extcombs(k))
  end do
  ...
  if ( JBmunu /= 0 ) then ! POWHEGs Bmunu helicity correlation
    ...
    M2munu = M2munu / average_factor_ppllj_eexccxg_1
  end if

  ! Colour and helicity average and symmetry factor of outgoing particles
  M2add = M2 / average_factor_ppllj_eexccxg_1

  do k = 0, 17-1
    M2(k) = M2add(extcomb_perm_ppllj_eexccxg_1(k))
  end do
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_QA(nsync, wf4(:,3), wf8(:,1), A(:,1), n3(:,9), t3x32(:,:,1), nhel, den(3))
    call cont_QA(nsync, wf4(:,3), wf8(:,2), A(:,2), n3(:,10), t3x32(:,:,2), nhel, den(5))
    call cont_QA(nsync, wf4(:,7), wf8(:,3), A(:,3), n3(:,11), t3x32(:,:,3), nhel, den(7))
    call cont_QA(nsync, wf4(:,7), wf8(:,4), A(:,4), n3(:,12), t3x32(:,:,4), nhel, den(8))
...
subroutine colourvector(A, j, M1)
  ...
  M1(1) = ((-A(j,1)%j-A(j,3)%j)*f(1)+(A(j,2)%j+A(j,4)%j)*f(2))
...
! i=j=0 -> 0 means no colour insertion.
! **********************************************************************
  use ol_colourmatrix_ppllj_eexccxg_1_dp, only: K1
  implicit none
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllj_eexccxg_1")
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllj_eexccxg_1")
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllj_eexccxg_1")
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllj_eexccxg_1")
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllj_eexccxg_1")
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllj_eexccxg_1")
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllj_eexccxg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllj_eexccxg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllj_eexccxg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllj_eexccxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllj_eexccxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
...
end module ol_tree_ppllj_eexccxg_1_dp
