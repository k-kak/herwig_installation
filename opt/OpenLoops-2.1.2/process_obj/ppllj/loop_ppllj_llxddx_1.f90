...
module ol_colourmatrix_ppllj_llxddx_1_dp
...
end module ol_colourmatrix_ppllj_llxddx_1_dp



module ol_forced_parameters_ppllj_llxddx_1_dp
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_llxddx_1_dp


! **********************************************************************
module ol_loop_storage_ppllj_llxddx_1_dp
! **********************************************************************
...
end module ol_loop_storage_ppllj_llxddx_1_dp
...
! **********************************************************************
module ol_loop_ppllj_llxddx_1_dp
! **********************************************************************
  use kind_types, only: dp, dp, intkind1, intkind2
  use ol_data_types_dp, only: wfun, Hpolcont
  use ol_loop_storage_ppllj_llxddx_1_dp
  implicit none
...
! **********************************************************************
  ...
  ! factors of the diagrams
    f(1) = (CI*eQED**2)/3._dp
    f(2) = CI*eQED**2
    f(3) = (CI*countertermnorm*ctVqq*eQED**2*gQCD**2)/3._dp
    f(4) = CI*countertermnorm*ctVqq*eQED**2*gQCD**2
    f(5) = (eQED**2*gQCD**2*integralnorm*SwB)/3._dp
...
! **********************************************************************
  ...
  use ol_settings_ppllj_llxddx_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_heltables_OLR_ppllj_llxddx_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_llxddx_1, 4, qp_kinematics)
  ...
  call counter_QA_Z(gZd,ntry, ex3(:), ex4(:), wf4(:,7), n3(:,6), t3x4(:,:,6))
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_llxddx_1_dp, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_llxddx_1_dp, only: KL
  implicit none
...
! spin-correlated hard scattering factor for tau- tau+ down anti-down -> 0
! **********************************************************************
  use ol_parameters_decl_dp
  implicit none
  real(dp), intent(in) :: P_scatt(0:3,4)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_llxddx_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 4
    r = external_perm_inv_ppllj_llxddx_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_llxddx_1_dp
