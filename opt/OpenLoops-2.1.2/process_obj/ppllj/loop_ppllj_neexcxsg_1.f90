...
module ol_colourmatrix_ppllj_neexcxsg_1_dp
...
end module ol_colourmatrix_ppllj_neexcxsg_1_dp



module ol_forced_parameters_ppllj_neexcxsg_1_dp
  implicit none
  contains
...
end module ol_forced_parameters_ppllj_neexcxsg_1_dp


! **********************************************************************
module ol_loop_storage_ppllj_neexcxsg_1_dp
! **********************************************************************
...
! **********************************************************************
module ol_loop_ppllj_neexcxsg_1_dp
! **********************************************************************
...
! **********************************************************************
  ...
  ! factors of the diagrams
    f(1) = (CI*eQED**2*gQCD)/(2._dp*sw**2)
    f(2) = (CI*countertermnorm*eQED**2*gQCD**3)/(2._dp*sw**2)
    f(3) = (CI*countertermnorm*ctGcc*eQED**2*gQCD**3)/(2._dp*sw**2)
    f(4) = (CI*countertermnorm*ctGqq*eQED**2*gQCD**3)/(2._dp*sw**2)
    f(5) = (CI*countertermnorm*ctVsc*eQED**2*gQCD**3)/(2._dp*sw**2)
    f(6) = (CI*eQED**2*gQCD**3*integralnorm*SwB)/(2._dp*sw**2)
    f(7) = (eQED**2*gQCD**3*integralnorm*SwB)/(sw**2*2._dp)
...
! M2  = helicity-summed squared matrix element for nu_e e+ anti-charm strange glue -> 0
! **********************************************************************
  ...
  use ol_settings_ppllj_neexcxsg_1, only: hel_mem_opt, loopcc
...
! 
  ...
  use ol_forced_parameters_ppllj_neexcxsg_1_dp, only: &
    & check_forced_parameters
  use ol_heltables_OLR_ppllj_neexcxsg_1
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
     external_perm_inv_ppllj_neexcxsg_1, 5, qp_kinematics)
  ...
  call pol_wf_A(P(:,3), rMC, H3, ex3, POLSEL(3),3)
  ...
  call prop_A_Q(ntry, wf4(:,2), 20, MC, 1_intkind1, wf4(:,4), n2(2))
  ...
  call prop_A_Q(ntry, wf4(:,9), 20, MC, 1_intkind1, wf4(:,10), n2(5))
  call counter_A_Q(ctcc,4,ntry, wf4(:,4), 20, wf4(:,11), n2(6))
  call prop_Q_A(ntry, wf8(:,1), 11, MC, 1_intkind1, wf8(:,5), n2(7))
  call counter_Q_A(ctqq,3,ntry, wf4(:,6), 24, wf4(:,12), n2(8))
...
subroutine colourvectors(A, j, M1, M2)
  ...
  M2(1)%j = ((A(j,7)%j+A(j,8)%j)*f(2)-A(j,6)%j*f(3)-A(j,4)%j*f(4)+(-A(j,3)%j-A(j,5)%j)*f(5))
...
! K1(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_neexcxsg_1_dp, only: K1
  implicit none
...
! K2(i,j) = <Ci|Cj>
! **********************************************************************
  use ol_colourmatrix_ppllj_neexcxsg_1_dp, only: K2
  implicit none
...
! **********************************************************************
  use ol_colourmatrix_ppllj_neexcxsg_1_dp, only: KL
  implicit none
...
! spin-correlated hard scattering factor for nu_e e+ anti-charm strange glue -> 0
! **********************************************************************
  use ol_parameters_decl_dp
  implicit none
  real(dp), intent(in) :: P_scatt(0:3,5)
...
subroutine assign_helicity_flip(ext_wf_hf,ext_wf_t,m)
  ...
  if(size(ext_wf_hf) == size(hflip,1)) then
    ...
    call ol_fatal("Spin-correlated HSF for ppllj_neexcxsg_1:" &
...
subroutine init_flipped_heltables(i,j)
  ...
  do k = 1, 5
    r = external_perm_inv_ppllj_neexcxsg_1(k)
    if(r == i .or. r == j) then
      l = l+1
      select case(k)
        case(1)
...
end module ol_loop_ppllj_neexcxsg_1_dp
