module ol_external_ppllj_eexuuxg_2
! 
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_ppllj_eexuuxg_2(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: external_perm_inv_ppllj_eexuuxg_2(5) = &
                     [ (dummy_counter, dummy_counter = 1, 5) ]
  integer, save :: extcomb_perm_ppllj_eexuuxg_2(0:16) = &
                     [ (dummy_counter, dummy_counter = 0, 16) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_ppllj_eexuuxg_2(5) = &
                     [ 1, 2, 3, 4, 5 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_ppllj_eexuuxg_2(5) = &
                     [ 2, 2, 6, 6, 16 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_ppllj_eexuuxg_2 = &
                     4
  integer, save :: channel_number_ppllj_eexuuxg_2 = -1
  ...
  integer, save :: photonid_ppllj_eexuuxg_2(5) = 0

  
  contains

  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_ppllj_eexuuxg_2")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_ppllj_eexuuxg_2")
  ...
  subroutine averagefactor_c(avgf) &
      & bind(c,name="ol_averagefactor_ppllj_eexuuxg_2")
    use kind_types, only: dp
    implicit none
    real(dp), intent(out) :: avgf
    avgf = average_factor_ppllj_eexuuxg_2
  end subroutine averagefactor_c

  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_ppllj_eexuuxg_2")
    ...
    integer :: i, j, ii, jj
    integer :: particle_types_perm_ppllj_eexuuxg_2(5)
    external_perm_ppllj_eexuuxg_2 = perm
    do i = 1, 5
      external_perm_inv_ppllj_eexuuxg_2( &
        external_perm_ppllj_eexuuxg_2(i)) = i
      particle_types_perm_ppllj_eexuuxg_2(i) = &
        particle_types_ppllj_eexuuxg_2( &
        external_perm_ppllj_eexuuxg_2(i))
    end do
    do i = 1, 5
      do j = 1, i
        if (external_perm_ppllj_eexuuxg_2(i) >= &
          external_perm_ppllj_eexuuxg_2(j)) then
          ii = external_perm_ppllj_eexuuxg_2(i)
          jj = external_perm_ppllj_eexuuxg_2(j)
        else
          ii = external_perm_ppllj_eexuuxg_2(j)
          jj = external_perm_ppllj_eexuuxg_2(i)
        end if
        extcomb_perm_ppllj_eexuuxg_2((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_ppllj_eexuuxg_2 = 1
    do i = 1, n_scatt
      average_factor_ppllj_eexuuxg_2 = &
        average_factor_ppllj_eexuuxg_2 &
        * average_factors_ppllj_eexuuxg_2( &
        external_perm_ppllj_eexuuxg_2(i))
    end do
    ...
    if (out_symmetry_on /= 0) then
      do i = 1, 5
        average_factor_ppllj_eexuuxg_2 = &
          average_factor_ppllj_eexuuxg_2 &
          * factorial(count(particle_types_perm_ppllj_eexuuxg_2(n_scatt+1:5) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_ppllj_eexuuxg_2")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_last_perm(perm) &
      & bind(c,name="ol_f_get_last_perm_ppllj_eexuuxg_2")
    integer, intent(out) :: perm(5)
    perm = external_perm_ppllj_eexuuxg_2
  end subroutine get_last_perm


  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_ppllj_eexuuxg_2")
    ...
    do i = 1, 5
      m_ex(i) = m_ex_orig(external_perm_ppllj_eexuuxg_2(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_ppllj_eexuuxg_2")
    use kind_types, only: dp
  ...
  subroutine get_types(t_ex) &
      & bind(c,name="ol_f_get_types_ppllj_eexuuxg_2")
    ...
    t_ex_orig = [3,3,2,2,1]
    do i = 1, 5
      t_ex(i) = t_ex_orig(external_perm_ppllj_eexuuxg_2(i))
    end do
  end subroutine get_types


  subroutine get_types_c(t_ex) &
      & bind(c,name="ol_get_types_ppllj_eexuuxg_2")
    use, intrinsic :: iso_c_binding, only: c_int
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine get_charges(c_ex) &
      & bind(c,name="ol_f_get_charges_ppllj_eexuuxg_2")
    ! Return the type of the external particles in the current permutation.
    use kind_types, only: dp
    use ol_parameters_decl_dp
    implicit none
    ...
    do i = 1, 5
      c_ex(i) = c_ex_orig(external_perm_ppllj_eexuuxg_2(i))
    end do
  end subroutine get_charges


  subroutine get_charges_c(c_ex) &
      & bind(c,name="ol_get_charge_ppllj_eexuuxg_2")
    use kind_types, only: dp
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_ppllj_eexuuxg_2")
    use kind_types, only: dp
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_ppllj_eexuuxg_2")
    use kind_types, only: dp
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine pol_init(pol) &
      & bind(c,name="ol_f_pol_init_ppllj_eexuuxg_2")
    implicit none
  ...
  subroutine set_photons(photon_id) &
      & bind(c,name="ol_f_set_photons_ppllj_eexuuxg_2")
    implicit none
    integer, intent(in) :: photon_id(5)
    photonid_ppllj_eexuuxg_2 = photon_id
  end subroutine set_photons




end module ol_external_ppllj_eexuuxg_2


module ol_settings_ppllj_eexuuxg_2
  implicit none
  ...
  subroutine set_hel_mem_opt(set) &
      & bind(c,name="ol_hel_mem_opt_ppllj_eexuuxg_2")
    implicit none
  ...
  subroutine set_loopcc(set,was) &
      & bind(c,name="ol_loopcc_ppllj_eexuuxg_2")
    implicit none
    logical, intent(in) :: set
    logical :: was
    was = loopcc
...
end module ol_settings_ppllj_eexuuxg_2

module colour_basis_ppllj_eexuuxg_2
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_ppllj_eexuuxg_2")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_ppllj_eexuuxg_2")
    implicit none
  ...
  pure subroutine loop_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_loop_colbasis_dim_ppllj_eexuuxg_2")
    implicit none
    ! colour representation of external particles: 0=neutral, 1=fundamental, 2=adjoint
    integer, intent(out) :: extcols(5)
  ...
  subroutine loop_colbasis(basis, powers) &
    & bind(c, name="ol_loop_colbasis_ppllj_eexuuxg_2")
    implicit none
    integer, intent(out) :: powers(1,1)
    integer, intent(out) :: basis(3,1)
...
end module colour_basis_ppllj_eexuuxg_2

! **********************************************************************
module ol_heltables_OLR_ppllj_eexuuxg_2
! **********************************************************************
...
end module ol_heltables_OLR_ppllj_eexuuxg_2
