...
module ol_tree_ppllj_llxbbxg_1_dp
  ...
  complex(dp) :: M1helarr(1,32) ! cache
...
subroutine factors_init()
  ...
  ! factors of the diagrams
    ...
    f(2) = CI*eQED**2*gQCD
    f(3) = (CI*eQED**2*gQCD*MB*ML)/(4._dp*MW**2*sw**2)
...
subroutine amp2(P_scatt, M2, I, MOM, nextcombs, extcombs, M2munu) &
    & bind(c,name="ol_f_amp2_ppllj_llxbbxg_1")
...
! **********************************************************************
  ...
  use ol_helicity_bookkeeping_dp, only: &
    & helbookkeeping_wf, helsync, flip_phase
  use ol_helicity_init, only: helbookkeeping_flip, helsync_flip
  use ol_h_propagators_dp
  use ol_h_wavefunctions_dp
  use ol_wavefunctions_dp, only: wf_V_Std
! 
  use ol_h_vertices_dp
  use ol_h_contractions_dp
  ...
  use ol_heltables_ppllj_llxbbxg_1
  ...
  real(dp)    :: M2add(0:17-1)
  complex(dp) :: M2munuadd(2)
  complex(dp) :: MOM_LC(4), M1(1), M1helarray(1,32)
  real(dp)    :: P_scatt_intern(0:3,5)
  complex(dp) :: epLC(1:4), epStd(1:4,2)
  real(dp), save :: scalebackfactor, old_scalefactor = 0, bornphotonfactor
  ...
  type(wfun) :: wf4(4,8), wf8(8,6), wf32(32,6)
  ...
  call init_kinematics(P_scatt, extmasses2, P, &
      &  external_perm_inv_ppllj_llxbbxg_1, 5,.false.)
  ...
  JBmunu = 0
  if (I > 0) then
    ReplacePol = external_perm_ppllj_llxbbxg_1(I)
  else if (I < 0) then
    JBmunu = external_perm_ppllj_llxbbxg_1(-I)
  end if

  do k = 1, nextcombs
    extcombs_permuted(k) = &
       & extcomb_perm_ppllj_llxbbxg_1(extcombs(k))
  end do
  ...
  if ( JBmunu /= 0 ) then ! POWHEGs Bmunu helicity correlation
    ...
    M2munu = M2munu / average_factor_ppllj_llxbbxg_1
  end if

  ! Colour and helicity average and symmetry factor of outgoing particles
  M2add = M2 / average_factor_ppllj_llxbbxg_1

  do k = 0, 17-1
    M2(k) = M2add(extcomb_perm_ppllj_llxbbxg_1(k))
  end do
...
subroutine diagrams()
  ...
  ! e.g. call cont_VV(nsync, wf3, wf6, A(:,1), n64, t64, nhel, den(5)) ...
    ...
    call cont_QA(nsync, wf4(:,3), wf8(:,1), A(:,1), n3(:,12), t3x32(:,:,1), nhel, den(3))
    call cont_QA(nsync, wf4(:,3), wf8(:,2), A(:,2), n3(:,13), t3x32(:,:,2), nhel, den(5))
    call cont_QA(nsync, wf4(:,3), wf8(:,3), A(:,3), n3(:,14), t3x32(:,:,3), nhel, den(7))
    call cont_QA(nsync, wf4(:,8), wf8(:,4), A(:,4), n3(:,15), t3x32(:,:,4), nhel, den(9))
    call cont_QA(nsync, wf4(:,8), wf8(:,5), A(:,5), n3(:,16), t3x32(:,:,5), nhel, den(10))
    call cont_QA(nsync, wf4(:,8), wf8(:,6), A(:,6), n3(:,17), t3x32(:,:,6), nhel, den(11))
...
subroutine colourvector(A, j, M1)
  ...
  type(polcont) :: A(:,:)
  integer, intent(in) :: j
  complex(dp), intent(out) :: M1(1) ! M1helarray(1,32)
  integer :: empty(0)

  M1(1) = ((A(j,2)%j+A(j,5)%j)*f(1)+(A(j,3)%j+A(j,6)%j)*f(2)+(-A(j,1)%j-A(j,4)%j)*f(3))
...
! **********************************************************************
  ...
  real(dp),    intent(out) :: M2colint(0:17-1)
...
! **********************************************************************
  ...
  complex(dp), intent(in)  :: M1(1), M2(1)
  real(dp),    intent(out) :: M2colint(0:17-1)
  integer,           intent(in)  :: extcombs(:)
  integer :: extcomb, colmatpos, i, j, eco
...
subroutine colourvector(M1out, nhelout) &
  & bind(c,name="ol_tree_colvect_ppllj_llxbbxg_1")
  ! Retrieve M1helarr from cache. Cache must have been filled before by calling amp2().
  ! [out] M1out(:,h): colour vector for each helicity configuration h
  ! [out] nhelout: number of non-vanishing helicity configurations
  implicit none
  complex(dp) :: M1out(1,32)
...
subroutine amp2tree(P, M2) &
    & bind(c,name="ol_f_amp2tree_ppllj_llxbbxg_1")

  ! This is an interface for AMP2.
  ! Calculates the squared matrix element without fuss.
  implicit none
  real(dp), intent(in)  :: P(0:3,5)
  real(dp),  intent(out) :: M2
  real(dp) :: M2tmp(0:17-1)
  real(dp) :: M2munu(4,4)
  call amp2(P, M2tmp, 0, [ 0._dp, 0._dp, 0._dp, 0._dp ], 1, [ 0 ], M2munu)
  M2 = M2tmp(0)
...
subroutine amp2ccone(P, M2, I, J) &
    & bind(c,name="ol_f_amp2ccone_ppllj_llxbbxg_1")
  ...
  call amp2(P, M2tmp, 0, [ 0._dp, 0._dp, 0._dp, 0._dp ], 1, [ extcomb ], M2munu)
  M2 = M2tmp(extcomb)
...
subroutine amp2ccall(P, M2) &
    & bind(c,name="ol_f_amp2ccall_ppllj_llxbbxg_1")
  ...
  real(dp) :: M2munu(4,4)
  integer :: k
  call amp2(P, M2, 0, [ 0._dp, 0._dp, 0._dp, 0._dp ], &
...
subroutine amp2hcone(P, M2, I, J, MOM) &
    & bind(c,name="ol_f_amp2hcone_ppllj_llxbbxg_1")
...
subroutine amp2hcall(P, M2, I, MOM) &
    & bind(c,name="ol_f_amp2hcall_ppllj_llxbbxg_1")
...
subroutine amp2tree_c(p, m2) &
    & bind(c,name="ol_amp2tree_ppllj_llxbbxg_1")
...
subroutine amp2ccone_c(p, m2, i, j) &
    & bind(c,name="ol_amp2ccone_ppllj_llxbbxg_1")
...
subroutine amp2ccall_c(p, m2) &
    & bind(c,name="ol_amp2ccall_ppllj_llxbbxg_1")
  use, intrinsic :: iso_c_binding, only: c_double
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
...
subroutine amp2hcone_c(p, m2, i, j, mom) &
    & bind(c,name="ol_amp2hcone_ppllj_llxbbxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
  real(c_double), intent(out) :: m2
...
subroutine amp2hcall_c(p, m2, i, mom) &
    & bind(c,name="ol_amp2hcall_ppllj_llxbbxg_1")
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  implicit none
  real(c_double), intent(in)  :: p(0:3,5)
...
end module ol_tree_ppllj_llxbbxg_1_dp
