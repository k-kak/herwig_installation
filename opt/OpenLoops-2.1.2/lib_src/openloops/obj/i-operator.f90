...
! **********************************************************************
  ...
  use ol_parameters_decl_dp, only: alpha_QCD, alpha_QED, pi
  use ol_parameters_decl_dp, only: &
       & offshell_photons_lsz, delta_alphamz_dimreg
  use ol_loop_parameters_decl_dp
  use ol_momenta_decl_dp, only: Q, L
  use ol_kinematics_dp
  implicit none
...
! **********************************************************************
  ...
  use ol_loop_parameters_decl_dp, only: pi2_6, tf, ca, mu2_IR => mureg2, Qf2sum
  ...
  if (flavj == 1) then ! emitter j = gluon
    ...
    Kaj = 0
    if (SwF /= 0) then
      Gaj = Gaj - 2._dp/3.*Qf2sum
      Kaj = Kaj - 10._dp/9.*Qf2sum
    end if
    Gj(1) = Gaj
    Gj(0) = 0
...
! **********************************************************************
  ...
  use ol_generic, only: to_string
  use ol_parameters_decl_dp
  use ol_loop_parameters_decl_dp, only: mu2_IR => mureg2, pi2_6, tf, ca, kappa, Qf2sum
  use ol_loop_parameters_decl_dp, only: N_lf, SwF, SwB
  implicit none
...
!    write(*,*) '[OpenLoops] M2k = ', M2k
 ...
 !     write(*,*) '[OpenLoops] M2k =', M2k
  ...
  ! non-singular part of nu_j function for photons
    ...
    ! Gaj = [1/1] - 2/3*sum(Ncf*Qf2)
    Gaj = 0._dp
    if (SwF /= 0) Gaj = Gaj - 2._dp/3.*Qf2sum
    if (M2k == 0) then ! eq. (6.26)
      Nuj_nonsing = 0._dp ! ~ tf/ca
    else if (M2k > 0) then ! eq. (6.24) in N_f=5 scheme
      Nuj_nonsing = Gaj*(log(Sjk/Q2jk) - 2._dp*log((Qjk-Mk)/Qjk)-2._dp*Mk/(Qjk+Mk)) + pi2_6 - spence(Sjk/Q2jk)
