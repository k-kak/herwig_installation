...
subroutine wf_gf_V(P,POL,J_V)
  ...
  real(dp)    :: Pmod, Pgfmod, cos_theta, sin_theta, Pgf(0:3), J_Lor(0:3), P_check(0:3,size(P)) ! Lorentz repr
  integer           :: i, part_i
...
! **********************************************************************
module ol_hel_wavefunctions_dp
! Routines to calculate external wave functions:
! return an array of wave functions, one for each polarisation.
! - wf_S: scalar
! - wf_Q: fermion
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun
  use ol_loop_handling_dp, only: hp_switch
  implicit none
...
end module ol_hel_wavefunctions_dp
