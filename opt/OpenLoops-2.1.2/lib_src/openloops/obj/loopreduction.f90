...
! ******************************************************************************
  ...
  use ofred_reduction_dp, only: twopoint_reduction
  use ofred_reduction_qp, only: twopoint_reduction_qp=>twopoint_reduction
  use ol_parameters_decl_dp, only: hp_switch,hybrid_qp_mode,hp_alloc_mode
  use ol_data_types_dp, only: hcl, met
  use ol_kinematics_dp, only: get_mass2,get_LC_5

  use ol_loop_handling_dp, only: req_qp_cmp,hcl_alloc_hybrid
  ...
  complex(dp)  :: p(1:5)
  complex(dp) :: redcoeff(1:4)

  complex(qp) :: p_qp(1:5)
  complex(qp) :: redcoeff_qp(1:4)
...
! ******************************************************************************
  ...
  use ofred_reduction_dp, only: otf_3pt_reduction_last
  ...
  use ol_loop_handling_dp, only: req_qp_cmp,upgrade_qp,hcl_alloc_hybrid
  use ol_loop_reduction_qp, only: &
  ...
  use ol_data_types_qp, only: basis_qp=>basis
  use ofred_basis_construction_dp, only: normalize_gamma
  ...
  complex(dp) :: Gout_R1, momenta(5,3),p(0:3)
  complex(qp) :: Gout_R1_qp, momenta_qp(5,3)
  real(dp) :: sdlt,gn
  real(qp) :: sdlt_qp
...
! ******************************************************************************
  ...
  use ol_loop_parameters_decl_dp, only: mureg, zero
  ...
  complex(dp), intent(out) :: mom(5,3)
  real(dp), intent(out) :: sdlt
  logical :: zero_mass(3)
  complex(dp) :: k1(5), k2(5), k12(5), p(0:3)
  real(dp) :: rone
...
! ******************************************************************************
  use kind_types, only: dp
  use ol_momenta_decl_dp, only: L
  use ol_loop_parameters_decl_dp, only: mureg,zero,rone
  use ol_kinematics_dp, only: LC2Std_Rep_cmplx
  implicit none
  integer, intent(in) :: mom1, mom2
  logical, intent(out) :: unstable
  ...
  complex(dp) :: k1(5), k2(5), k12(5), p(0:3)
  real(dp) :: coll, soft1, soft2, sdlt
  complex(dp) :: k1_std(0:3), k2_std(0:3), k12_std(0:3)
...
! ******************************************************************************
  ...
  complex(dp), intent(in) :: msq(0:2), p1(1:5), p2(1:5)
  integer, intent(in) :: r, masses_ind
  real(dp), intent(in) :: dlt, psq
  complex(dp), intent(out) :: redcoeff(1:8)

  complex(dp) :: p12(1:4), psq_delta, psq_delta2
  complex(dp) :: contr_p12, contr_p1

  complex(dp) :: p1p1(6:15), p1p2(6:15), p1p12(6:15)
  complex(dp) :: p1p1p1(16:35), p1_p2(16:35), p1p12p12(16:35), p1p1p12(16:35)
  complex(dp) :: P11, P22, T, U, Gmn, W0, P111, V1, Gp1, Gp2, Gp12, Gm, Gp
  complex(dp) :: T3, T6, U3, V16, P1116, P111z3, P16, V
  complex(dp) :: dlt2, dlt3, dlt4, dlt5, dlt6, zero, one, two, five
  complex(dp) :: z, z0, z01, z1, ct1, ct2, z12, z2, zd, z10, z02, z11, z22
  integer :: i, j, k, n
...
! ******************************************************************************
  ...
  use ol_loop_parameters_decl_dp, only: mu2_UV, mu2_IR
  ...
  complex(dp) :: psq_delta,psq_delta2,dlt2,dlt3,dlt4
  complex(dp) :: contr_p1,contr_p2,contr_P12
  complex(dp) :: contr_P12P12,contr_p1p1,contr_p1P12,contr_g,contr_gp1,contr_gP12
  complex(dp) :: contr_P12P12P12,contr_p1p1p1,contr_p1P12P12,contr_p1p1P12
  complex(dp) :: p1p1(6:15),p2p2(6:15),p1p2(6:15),p1P12(6:15),P12P12(6:15),P12_vec(1:4)
  complex(dp) :: p1p1p1(16:35),p1p1P12(16:35),p1P12P12(16:35),P12P12P12(16:35)
...
! ********************************************************************************
  ...
  real(dp), intent(in) :: sdlt
  complex(dp), intent(inout)  :: Gin(:)
  complex(dp), intent(in) :: msq(0:2), mom(5,3)
  ...
  complex(dp) :: k1(5), k2(5), k12(5), masses(0:2), tadp0(1), tadp1(1), tadp2(1)
...
!******************************************************************************
  ...
  implicit none
  integer, intent(in) :: perm(3)
  complex(dp), intent(in) :: A0_msq(:)
  complex(dp), intent(in) :: masses(0:2)
  complex(dp), intent(in) :: tadp0(1), tadp1(1), tadp2(1)
  complex(dp), optional, intent(out) :: A0_0(1), A0_1(1), A0_2(1)
...
! ------------------------------------------------------------------------------
  use kind_types, only: dp
  use ol_debug, only: ol_error
  use ol_loop_parameters_decl_dp, only: polecheck_is
  use ol_parameters_decl_dp, only: zero
  implicit none
  complex(dp), intent(in)  :: p1(1:5), p2(1:5), msq(0:2)
  real(dp),    intent(in)  :: dlt
  complex(dp), intent(in)  :: Gin_A(:)
  complex(dp), intent(out) :: Gout_A(1), Gout_A0(1), Gout_A1(1), Gout_A2(1)
  complex(dp), intent(out) :: Gout_R1
  complex(dp), intent(out) :: A0_0(1), A0_1(1), A0_2(1)

  real(dp) :: psq
  integer :: m_ind
  logical :: dlt_exp
  complex(dp) :: redcoeff1(1:8), redcoeff2(1:8), redcoeff3(1:8)
...
!************************************************************************************
  ...
  use ol_data_types_dp, only: hcl, met
  use ol_kinematics_dp, only: get_mass2
  use ol_parameters_decl_dp, only: hp_switch,hp_err_thres,hp_step_thres,coli_cache_use

  use ol_parameters_decl_dp, only: a_switch, coli_cache_use
  use ol_kinematics_qp, only: get_mass2_qp=>get_mass2
  use ol_loop_handling_dp, only: req_qp_cmp, upgrade_qp, downgrade_dp
  use ol_parameters_decl_dp, only: hybrid_zero_mode,hp_step_thres, &
                                             hp_err_thres,hp_switch,hybrid_dp_mode
  use ol_loop_reduction_qp, only: avh_olo_interface_qp=>avh_olo_interface
  ...
  if(a_switch==1 .or. a_switch==7) then
    ...
    if (iand(Gsum%mode, hybrid_dp_mode) .ne. 0 .or. coli_cache_use .eq. 1) then
      ...
      if (hp_switch .eq. 1 .and. error > hp_step_thres .and. Gsum%error + error > hp_err_thres) then
        ...
        M2%cmp = M2%cmp + real(M2add,kind=dp)
...
!************************************************************************************
  ...
  use ol_momenta_decl_dp, only: L
  use ol_kinematics_dp, only: LC2Std_Rep_cmplx, collier_invariants

  use collier, only: A_cll,B_cll,C_cll,D_cll

  implicit none
  integer,                  intent(in)  :: momenta(:)
  complex(dp),        intent(in)  :: masses2(:), Gsum(:)
  complex(dp),        intent(out) :: M2add
  real(dp), optional, intent(out) :: loss
  complex(dp) :: TI(size(Gsum)), p(0:3,1:size(momenta)-1)
  complex(dp) :: momenta_TI(0:3,size(momenta)-1)
  complex(dp) :: sc_int(1),UV_part(1)
  real(dp)    :: sc_err(1)
...
!************************************************************************************
  ...
  use ol_loop_parameters_decl_dp, only: mureg
  use ol_loop_parameters_decl_dp, only: de1_UV, de1_IR, de2_i_IR
  implicit none
  complex(dp), intent(in)  :: masses2(:), Gsum(:)
  integer, intent(in) :: momenta(:)
  complex(dp), intent(out) :: M2add
  logical           :: tadpole_check
  complex(dp) :: tadpole_mass, p1_2, p2_2, p3_2, p4_2, p12_2, p23_2
  complex(dp) :: rslt(0:2), zero = 0._dp
  integer :: i, j, k
...
!************************************************************************************
  ...
  complex(dp), intent(in)  :: masses2(4)
  complex(dp), intent(out) :: rslt(0:2)
  real(dp),    intent(out) :: err
  complex(dp) :: p(0:3,3), momenta_TI(0:3,3), sc_box(1), UV_part(1)
  real(dp) :: Derr(1)
  integer :: k
...
!************************************************************************************
  ...
  integer,           intent(in)  :: momenta(3)
  complex(dp), intent(out) :: rslt(0:2)
  complex(dp) :: p1_2, p2_2, p3_2, p4_2, p12_2, p23_2
  integer :: i, j, k
...
! ------------------------------------------------------------------------------
  use kind_types, only: dp,qp
  use ol_momenta_decl_dp, only: L
  use ol_data_types_dp, only: redset4, scalarbox
  use ol_kinematics_dp, only: get_LC_5,get_mass2
  use collier, only: tnten_cll
  use ol_parameters_decl_dp, only: a_switch

  use ol_loop_reduction_qp, only: box_onshell_cut_qp=>box_onshell_cut
  use ol_kinematics_qp, only: get_mass2_qp=>get_mass2
  use ol_kinematics_dp, only: get_LC_5_qp
  ...
  complex(dp) :: masses2(0:3),p(1:5,3),q0_p(5),q0_m(5),q0_cuts(2,5),sc_box(0:2)
  integer           :: mom_box(3)
  real(dp)    :: box_error,cut_error

  complex(qp) :: p_qp(1:5,3),q0_p_qp(5),q0_m_qp(5)
  real(qp)    :: box_error_qp,cut_error_qp
...
contains
  ...
  use ol_parameters_decl_dp, only: hp_switch,hp_check_box,hp_err_thres,hp_qp_kinematics_init_mode
  use ofred_basis_construction_qp, only: &
      construct_RedBasis_qp=>construct_RedBasis, &
      construct_p3scalars_qp=>construct_p3scalars
  use ol_data_types_qp, only: basis_qp=>basis, redset4_qp=>redset4
  use ol_data_types_qp, only: scalarbox_qp=>scalarbox
  use ol_loop_reduction_qp, only: compute_scalar_box_qp=>compute_scalar_box
  use ol_external_decl_dp, only: init_qp
  use ol_kinematics_dp, only: init_qp_kinematics
  complex(qp) :: scalars(0:4),cttmp(0:4)
  type(basis_qp)     :: Redbasis_qp
  real(qp)    :: gd2,gd3
  type(scalarbox_qp) :: box_qp

  if (hp_check_box .eq. 1 .and. hp_switch .eq. 1 &
      !.and. errbox .gt. 10**(-real(15-hp_err_thres,kind=dp))) then
...
! ********************************************************************
  ...
  implicit none
  complex(dp), intent(in)  :: p(1:5,1:3), m2(0:3)
  complex(dp), intent(out) :: q0_p(1:5), q0_m(1:5)
  real(dp), intent(out) :: error
  type(redset4), intent(in) :: RedSet

  complex(dp) :: l(1:4,4), k1(1:5), k2(1:5), k3(1:5)
  complex(dp) :: a1, a2
  complex(dp) :: x1, x2, x3p, x3m, x4p, x4m, k3scalars(1:4)
  complex(dp) :: dd0, dd1, dd2, dd3
  complex(dp) :: c0, b0, A, B, R
  integer :: i, perm(3)
  type(basis) :: bas
  ...
  x1 = (dd2 - dd0*(1._dp-a2) - dd1*a2)/bas%gamma
  x2 = (dd1 - dd0*(1._dp-a1) - dd2*a1)/bas%gamma
  ...
  if (hp_gamma_trig) then
    error = max(log10(max(1._dp, ABS(x1))), log10(max(1._dp, ABS(x2))))
    error = max(error, max(log10(max(1._dp, ABS(x3m))), log10(max(1._dp, ABS(x3p)))))
  else
    error = max(log10(max(1._dp, ABS(x3m))), log10(max(1._dp, ABS(x3p))))
  end if
...
! ********************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in)  :: Gtensor(:), q0(4)
  complex(dp), intent(out) :: N0
...
! ********************************************************************
  ...
  use ol_kinematics_dp, only: cont_LC_cntrv
  implicit none
  complex(dp), intent(in)  :: Gtensor(:)
  complex(dp), intent(in)  :: p_offshell(:,:), m_offshell(:), q0_pm(2,5)
  complex(dp), intent(out) :: box_coeff
  complex(dp) :: q0_p(1:4), q0_m(1:4)
  complex(dp) :: N0p, N0m, Gsum(size(Gtensor))
  complex(dp) :: mom(1:4), prop_m, prop_p, Di_p, Di_m
  integer :: i
...
subroutine upgrade_scalar_box(scbox)
  use kind_types, only: dp, qp
  use ol_data_types_dp, only: scalarbox
  use ol_data_types_qp, only: scalarbox_qp=>scalarbox
  use ol_data_types_qp, only: basis_qp=>basis, redset4_qp=>redset4
...
! ********************************************************************
  ...
  use ol_loop_handling_qp, only: G_TensorShift_otf_qp=>G_TensorShift_otf
  use ol_parameters_decl_dp, only: hp_switch,hp_err_thres,hp_step_thres
  use ol_loop_handling_dp, only: req_dp_cmp, req_qp_cmp, upgrade_qp_hcl, downgrade_dp
  use ol_loop_reduction_qp, only: box_coefficient_qp=>box_coefficient
  use ol_kinematics_qp, only: get_LC_5_qp=>get_LC_5,get_mass2_qp=>get_mass2
  ...
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(inout) :: all_scboxes(:)
  real(dp) :: error,error_step,max_error_opp

  complex(dp) :: offshell_p(1:5,1), offshell_m2(1)
  complex(dp) :: Gsum(size(Gtensor%cmp))
  complex(dp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)

  complex(qp) :: offshell_p_qp(1:5,1), offshell_m2_qp(1)
  complex(qp) :: Gsum_qp(size(Gtensor%cmp))
  complex(qp) :: box_coeff_qp, box_qp, scalar_box_qp(0:2), q0p_q0m_qp(2,5)
...
! ********************************************************************
  ...
  use ol_loop_handling_dp, only: G_TensorShift_otf
  use ol_data_types_dp, only: scalarbox, hcl, met
  use ol_kinematics_dp, only: get_LC_4,get_LC_5,get_mass2
  use ol_loop_parameters_decl_dp, only: de1_IR, de2_i_IR
  ...
  use ol_loop_handling_dp, only: req_dp_cmp, req_qp_cmp, upgrade_qp_hcl, downgrade_dp
  use ol_loop_reduction_qp, only: box_coefficient_qp=>box_coefficient
  use ol_kinematics_qp, only: get_LC_4_qp=>get_LC_4,get_LC_5_qp=>get_LC_5,get_mass2_qp=>get_mass2
  ...
  complex(dp) :: offshell_p(1:5,2), offshell_m2(2)
  complex(dp) :: Gsum(size(Gtensor%cmp))
  complex(dp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)

  complex(qp) :: offshell_p_qp(1:5,2), offshell_m2_qp(2)
  complex(qp) :: Gsum_qp(size(Gtensor%cmp))
  complex(qp) :: box_coeff_qp, box_qp, scalar_box_qp(0:2), q0p_q0m_qp(2,5)
...
! ********************************************************************
  ...
  use ol_loop_handling_qp, only: G_TensorShift_otf_qp=>G_TensorShift_otf
  use ol_parameters_decl_dp, only: hp_switch,hp_err_thres,hp_step_thres
  use ol_loop_handling_dp, only: req_dp_cmp,req_qp_cmp, &
                                           upgrade_qp_hcl,downgrade_dp
  use ol_loop_reduction_qp, only: box_coefficient_qp=>box_coefficient
  use ol_kinematics_qp, only: get_LC_4_qp=>get_LC_4, &
  ...
  complex(dp) :: offshell_p(1:5,3), offshell_m2(3)
  complex(dp) :: Gsum(size(Gtensor%cmp))
  complex(dp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)

  complex(qp) :: offshell_p_qp(1:5,3), offshell_m2_qp(3)
  complex(qp) :: Gsum_qp(size(Gtensor%cmp))
  complex(qp) :: box_coeff_qp, box_qp, scalar_box_qp(0:2), q0p_q0m_qp(2,5)
...
! ********************************************************************
  ...
  complex(dp) :: offshell_p(1:5,4), offshell_m2(4)
  complex(dp) :: Gsum(size(Gtensor%cmp))
  complex(dp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)

  complex(qp) :: offshell_p_qp(1:5,4), offshell_m2_qp(4)
  complex(qp) :: Gsum_qp(size(Gtensor%cmp))
  complex(qp) :: box_coeff_qp, box_qp, scalar_box_qp(0:2), q0p_q0m_qp(2,5)
...
! ********************************************************************
  ...
  complex(dp) :: offshell_p(1:5,5), offshell_m2(5)
  complex(dp) :: Gsum(size(Gtensor%cmp))
  complex(dp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)

  complex(qp) :: offshell_p_qp(1:5,5), offshell_m2_qp(5)
  complex(qp) :: Gsum_qp(size(Gtensor%cmp))
  complex(qp) :: box_coeff_qp, box_qp, scalar_box_qp(0:2), q0p_q0m_qp(2,5)
...
! ********************************************************************
  ...
  implicit none
  integer, intent(in) :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:4), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error

  complex(dp) :: box_p(1:4,0:3,1:5), box_m2(0:3,5)
  complex(dp) :: offshell_p(1:5,1,5), offshell_m2(1,5)
  complex(dp) :: Gsum(size(Gtensor))

  complex(dp) :: box_coeff(5), box(5), scalar_box(0:2), q0p_q0m(2,5)
  integer :: i, j, k
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:5), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error
  ...
  complex(dp) :: box_coeff(size(scboxes)), box(size(scboxes)), scalar_box(0:2)
  complex(dp) :: q0p_q0m(2,5), momshift(5), Gsum(size(Gtensor))
  integer :: i, j, k
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:6), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:7), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:8), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(dp), intent(in)  :: momenta(:,:), masses2(0:9), Gtensor(:)
  complex(dp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(dp), intent(out) :: box_error

  complex(dp) :: offshell_p(1:5,6), offshell_m2(6)
...
end module ol_loop_reduction_dp
