...
!***********************************************************************
  use kind_types, only: dp, dp, qp, intkind1, intkind2
  use ol_data_types_dp, only: hol, Hpolcont, wfun
  use ol_loop_parameters_decl_dp, only: bubble_vertex, ir_hacks
  use ol_loop_routines_dp, only: G0initialisationOLR
  use ol_debug, only: ol_fatal, ol_msg
  use ind_bookkeeping_dp, only : ProjHind
  use ol_parameters_decl_dp, only: hp_switch,hp_ir_trig, &
                                             hp_fake_trig
...
!***********************************************************************************
  ...
  ol_coeff%j(:,:,:,nhel_in+1:size(ol_coeff%hf)) = 0._dp
  ...
  function check_collinear_tr()
    use ol_momenta_decl_dp, only: collconf
  ...
  function check_soft_tr()
    use ol_momenta_decl_dp, only: softconf
...
end module hol_initialisation_dp





module ol_h_vert_interface_dp

  use ol_loop_handling_dp, only: req_qp_cmp
...
function valid_hol_hol(Gin, Gout) result(valid_hol)
  ...
  use ol_parameters_decl_dp, only: hp_switch,hybrid_dp_mode, &
                                              hp_alloc_mode
...
function valid_hol_hcl(Gin, Gout) result(valid_hol)
  use ol_data_types_dp, only: hol, hcl
  use ol_loop_handling_dp, only: hybrid_zero_mode
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use ol_vert_interface_dp, only: loop_AZ_Q
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_AZ_Q_qp => loop_AZ_Q
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp)  :: G_add(size(Gout_A%j,1),size(Gout_A%j,2),size(Gout_A%j,3))
  complex(dp)  :: g_RL(2)

  complex(qp) :: G_add_qp(size(Gout_A%j,1),size(Gout_A%j,2),size(Gout_A%j,3))
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_AQ_Z_qp => loop_AQ_Z
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  integer,           intent(in)    :: ng_RL
  complex(dp) :: G_add(size(Gout_Z%j,1),size(Gout_Z%j,2),size(Gout_Z%j,3))
  complex(dp) :: g_RL(2)
  integer(intkind2) :: h

  complex(qp) :: G_add_qp(size(Gout_Z%j,1),size(Gout_Z%j,2),size(Gout_Z%j,3))
  complex(qp) :: g_RL_qp(2)
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_ZA_Q
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_ZA_Q_qp => loop_ZA_Q
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_QZ_A
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_QZ_A_qp => loop_QZ_A
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  integer,           intent(in)    :: ng_RL
  complex(dp) :: G_add(size(Gout_Q%j,1),size(Gout_Q%j,2),size(Gout_Q%j,3))
  complex(dp) :: g_RL(2)
  integer  :: h

  complex(qp) :: G_add_qp(size(Gout_Q%j,1),size(Gout_Q%j,2),size(Gout_Q%j,3))
  complex(qp) :: g_RL_qp(2)
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_QA_Z_qp => loop_QA_Z
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_ZQ_A
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_ZQ_A_qp => loop_ZQ_A
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_AW_Q

  use ol_vert_interface_qp, only: loop_AW_Q_qp => loop_AW_Q
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_AQ_W_qp => loop_AQ_W
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp)  :: G_add(size(Gout_W%j,1),size(Gout_W%j,2),size(Gout_W%j,3))
  integer :: h

  complex(qp)  :: G_add_qp(size(Gout_W%j,1),size(Gout_W%j,2),size(Gout_W%j,3))
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_WA_Q

  use ol_vert_interface_qp, only: loop_WA_Q_qp => loop_WA_Q
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_QW_A

  use ol_vert_interface_qp, only: loop_QW_A_qp => loop_QW_A
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_QA_W_qp => loop_QA_W
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_WQ_A

  use ol_vert_interface_qp, only: loop_WQ_A_qp => loop_WQ_A
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_AV_Q

  use ol_vert_interface_qp, only: loop_AV_Q_qp => loop_AV_Q
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_AQ_V_qp => loop_AQ_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp)  :: G_add(size(Gout_V%j,1),size(Gout_V%j,2),size(Gout_V%j,3))
  integer  :: h

  complex(qp)  :: G_add_qp(size(Gout_V%j,1),size(Gout_V%j,2),size(Gout_V%j,3))
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_VA_Q

  use ol_vert_interface_qp, only: loop_VA_Q_qp => loop_VA_Q
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_QV_A

  use ol_vert_interface_qp, only: loop_QV_A_qp => loop_QV_A
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_QA_V_qp => loop_QA_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_VQ_A

  use ol_vert_interface_qp, only: loop_VQ_A_qp => loop_VQ_A
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_UV_W_qp => loop_UV_W
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_UW_V
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_UW_V_qp => loop_UW_V
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  use kind_types, only: dp, qp, intkind1, intkind2
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert4
  use ol_vert_interface_dp, only: loop_EV_V

  use ol_vert_interface_qp, only: loop_EV_V_qp => loop_EV_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_VE_V_qp => loop_VE_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_GGG_G_23_qp => loop_GGG_G_23
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_GGG_G_12_qp => loop_GGG_G_12
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_CV_D
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_CV_D_qp => loop_CV_D
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp)  :: G_add(size(Gout_C%j,1),size(Gout_C%j,2),size(Gout_C%j,3))
  integer  :: h

  complex(qp)  :: G_add_qp(size(Gout_C%j,1),size(Gout_C%j,2),size(Gout_C%j,3))
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_DV_C
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_DV_C_qp => loop_DV_C
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp)  :: G_add(size(Gout_D%j,1),size(Gout_D%j,2),size(Gout_D%j,3))
  integer :: h

  complex(qp)  :: G_add_qp(size(Gout_D%j,1),size(Gout_D%j,2),size(Gout_D%j,3))
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_AS_Q
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_AS_Q_qp => loop_AS_Q
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_SA_Q
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_SA_Q_qp => loop_SA_Q
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! ----------------------------------------------------------------------
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_QS_A
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_QS_A_qp => loop_QS_A
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! ----------------------------------------------------------------------
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_SQ_A
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_SQ_A_qp => loop_SQ_A
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_QA_S
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_QA_S_qp => loop_QA_S
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
  ...
  complex(dp) :: G_add(size(Gout_S%j,1),size(Gout_S%j,2),size(Gout_S%j,3))
  complex(dp) :: g_RL(2)
  integer(intkind2) :: h

  complex(qp) :: G_add_qp(size(Gout_S%j,1),size(Gout_S%j,2),size(Gout_S%j,3))
  complex(qp) :: g_RL_qp(2)
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_AQ_S
  use ol_parameters_init_dp, only: get_coupling

  use ol_vert_interface_qp, only: loop_AQ_S_qp => loop_AQ_S
  use ol_parameters_init_qp, only: get_coupling_qp=>get_coupling
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! ----------------------------------------------------------------------
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_VV_S

  use ol_vert_interface_qp, only: loop_VV_S_qp => loop_VV_S
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! ----------------------------------------------------------------------
  ...
  use ol_vert_interface_qp, only: loop_VS_V_qp => loop_VS_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! ----------------------------------------------------------------------
  ...
  use ol_vert_interface_qp, only: loop_SV_V_qp => loop_SV_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_kinematics_dp, only: get_LC_4
  use ol_vert_interface_dp, only: loop_SV_T

  use ol_vert_interface_qp, only: loop_SV_T_qp => loop_SV_T
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_TV_S
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_TV_S_qp => loop_TV_S
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_VS_T
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_VS_T_qp => loop_VS_T
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_VT_S
  use ol_kinematics_dp, only: get_LC_4

  use ol_vert_interface_qp, only: loop_VT_S_qp=>loop_VT_S
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_ST_V_qp=>loop_ST_V
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
!***********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_TS_V_qp=>loop_TS_V
  use ol_kinematics_dp, only: get_LC_4_qp
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert3
  use ol_vert_interface_dp, only: loop_SS_S

  use ol_vert_interface_qp, only: loop_SS_S_qp=>loop_SS_S
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert4
  use ol_vert_interface_dp, only: loop_SSS_S

  use ol_vert_interface_qp, only: loop_SSS_S_qp=>loop_SSS_S
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert4
  use ol_vert_interface_dp, only: loop_SVV_S

  use ol_vert_interface_qp, only: loop_SVV_S_qp=>loop_SVV_S
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_VSS_V_qp=>loop_VSS_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_data_types_dp, only: wfun, hol
  use hel_bookkeeping_dp, only: helbookkeeping_ol_vert4
  use ol_vert_interface_dp, only: loop_VVS_S

  use ol_vert_interface_qp, only: loop_VVS_S_qp=>loop_VVS_S
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_SSV_V_qp=>loop_SSV_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_VWW_V_qp=>loop_VWW_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
! **********************************************************************
  ...
  use ol_vert_interface_qp, only: loop_WWV_V_qp=>loop_WWV_V
  use ol_loop_handling_dp, only: hol_dealloc_hybrid
...
end module ol_h_vert_interface_dp
