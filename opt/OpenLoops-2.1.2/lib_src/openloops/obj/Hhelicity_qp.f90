...
module ol_h_helicity_bookkeeping_qp
  implicit none
  contains
...
! **********************************************************************
  ...
  use ol_generic, only: to_string
  use ol_debug, only: ol_error, ol_fatal
  use ol_data_types_qp, only: wfun, polcont, Hpolcont
  implicit none
...
! **********************************************************************
  ...
  use ol_generic, only: to_string
  use ol_debug, only: ol_error, ol_fatal
  use ol_data_types_qp, only: polcont, Hpolcont
  implicit none
  integer(intkind1), intent(in)    :: nsync
...
! **********************************************************************
  ...
  real(qp),    intent(in)  :: MOM(0:3)
  complex(qp), intent(out) :: omega(2)
  complex(qp) :: eps(4), MOM_LC(4)
  call wf_V_Std(P, 0._qp, pol, eps) ! light-cone polarisation vector
...
end module ol_h_helicity_bookkeeping_qp
