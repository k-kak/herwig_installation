...
module ofred_basis_construction_dp
...
subroutine LC_Contr2Cov(p1ctrv,p1cv)
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in)  :: p1ctrv(1:4)
  complex(dp), intent(out) :: p1cv(1:4)

  p1cv(1) =   p1ctrv(2)*0.5_dp
  p1cv(2) =   p1ctrv(1)*0.5_dp
  p1cv(3) = - p1ctrv(4)*0.5_dp
  p1cv(4) = - p1ctrv(3)*0.5_dp
...
!*************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(inout)  :: li(1:4)
...
!************************************************************************
  use kind_types, only: dp
  use ol_kinematics_dp, only: cont_LC_cntrv
  use ol_parameters_decl_dp, only: CI,cone
  use ol_momenta_decl_dp, only: L
  implicit none
  integer, intent(in) :: mom1, mom2
  complex(dp), intent(out) :: gamma, alpha(2)
  complex(dp) :: sqrt_delta, one_sqrt_delta
  complex(dp), intent(out) :: l1(4), l2(4), r1(4), r2(4)
  real(dp) :: p1p2, p1xp2, x1, x2, delta
  real(dp) :: signp1p2, signp1xp2
  ...
  if (L(3,mom1) .eq. 0 .and. L(3,mom2) .eq. 0) then
    ...
    signp1p2 = sign(1._dp,p1p2)
    signp1xp2 = sign(1._dp,p1xp2)
...
!****************************************************************
  ...
  implicit none
  complex(dp), intent(in)  :: p1(1:5), p2(1:5)
  complex(dp), intent(out) :: gamma, alpha(2)
  complex(dp), intent(out) :: l1(1:4), l2(1:4), r1(1:4), r2(1:4)

  real(dp) :: p1p1, p2p2, p1p2, GramDelta, sqrtDelta
  complex(dp) :: l1l1, l2l2
...
!************************************************************************
  use kind_types, only: dp
  use ol_parameters_decl_dp, only: CI,cone,zero
  implicit none

  complex(dp), intent(in)  :: l1(1:4), l2(1:4)
  complex(dp), intent(out) :: l3(1:4), l4(1:4)
  complex(dp) :: b1, b2, b12, bi12, bi21, c1, c2
  complex(dp) :: OnePlusI, OneMinusI, two
...
subroutine construct_l3l4_3(l1,l2,l3,l4)
  ...
  complex(dp), intent(in)  :: l1(1:4), l2(1:4)
  complex(dp), intent(out) :: l3(1:4), l4(1:4)
  real(dp) :: COST1, COST2, SINT1, SINT2, COST1HALF, COST2HALF, SINT1HALF, SINT2HALF
  real(dp) :: L_T1sq, L_T2sq, L_T1, L_T2, COST1p1, COST2p1, MODL1, MODL2, MODL1L2
  complex(dp) :: COSP1pSINP1, COSP1mSINP1, COSP2pSINP2, COSP2mSINP2
  complex(dp) :: b1, b2, cp1, cm1, cp2, cm2
  ...
  MODL1 = real((l1(1)+l1(2))*0.5_dp)
  MODL2 = real((l2(1)+l2(2))*0.5_dp)
  ...
  if(L_T1sq==0.0) then
    if (COST1p1>1.0) then
      b1 = sqrt(COST1p1*0.5_dp)
      cp1 = 0._dp
      cm1 = 0._dp
    else
      b1  = 0._dp
      cp1 = (1._dp/sqrt(2._dp))*(1._dp + CI)
      cm1 = (1._dp/sqrt(2._dp))*(1._dp - CI)
    end if
  else
    if (COST1p1>1.) then
      SINT1 = L_T1/MODL1
      COST1HALF = sqrt(COST1p1*0.5_dp)
      SINT1HALF = (SINT1/COST1HALF)*0.5_dp
      ...
      SINT1 = L_T1/MODL1
      SINT1HALF = sqrt((2._dp - COST1p1)*0.5_dp)
      COST1HALF = (SINT1/SINT1HALF)*0.5_dp
      COSP1pSINP1 = l1(3)/L_T1
      COSP1mSINP1 = l1(4)/L_T1
      b1  = COST1HALF
      cp1 = SINT1HALF*COSP1pSINP1
  ...
  if(L_T2sq==0.0) then
    if (COST2p1>1.0) then
      b2 = sqrt(COST2p1*0.5_dp)
      cp2 = 0._dp
      cm2 = 0._dp
    else
      b2  = 0._dp
      cp2 = (1._dp/sqrt(2._dp))*(1._dp + CI)
      cm2 = (1._dp/sqrt(2._dp))*(1._dp - CI)
    end if
  else
    if (COST2p1>1.) then
      SINT2 = L_T2/MODL2
      COST2HALF = sqrt(COST2p1*0.5_dp)
      SINT2HALF = (SINT2/COST2HALF)*0.5_dp
      ...
      SINT2 = L_T2/MODL2
      SINT2HALF = sqrt((2._dp - COST2p1)*0.5_dp)
      COST2HALF = (SINT2/SINT2HALF)*0.5_dp
      COSP2pSINP2 = l2(3)/L_T2
      COSP2mSINP2 = l2(4)/L_T2
      b2  = COST2HALF
      cp2 = SINT2HALF*COSP2pSINP2
  ...
  MODL1L2 = sqrt(MODL1*MODL2)*4._dp
...
subroutine construct_l3l4_2(l1,l2,l3,l4)
  ...
  complex(dp), intent(in)  :: l1(0:3), l2(0:3)
  complex(dp), intent(out) :: l3(1:4), l4(1:4)
  real(dp) :: COST1, COST2, SINT1, SINT2, COST1HALF, COST2HALF, SINT1HALF, SINT2HALF
  real(dp) :: L_T1sq, L_T2sq, L_T1, L_T2, COST1p1, COST2p1, MODL1, MODL2, MODL1L2
  complex(dp) :: COSP1pSINP1, COSP1mSINP1, COSP2pSINP2, COSP2mSINP2
  ...
  MODL1 = real((l1(0)+l1(1))*0.5_dp)
  MODL2 = real((l2(0)+l2(1))*0.5_dp)
...
! **********************************************************************************
  ...
  complex(dp), intent(in)  :: vj(1:4)
  complex(dp), intent(out) :: Vjj(6:15)
...
! **********************************************************************************
  ...
  complex(dp), intent(in)  :: vi(1:4), vj(1:4)
  complex(dp), intent(out) :: Vij(6:15)
...
! ******************************************************************************
  ...
  complex(dp) :: p1(5), p2(5), gamma, alphas(2)
  complex(dp) :: l1(1:4), l2(1:4), l3(1:4), l4(1:4), li(4,4), r1(1:4), r2(1:4)
  complex(dp) :: r1_cv(1:4), r2_cv(1:4), l3_cv(1:4), l4_cv(1:4)
  complex(dp) :: L33(6:15), L44(6:15), L34(6:15)
  complex(dp) :: Tb(1:4,1:4), r1Tb(6:15,1:4), r2Tb(6:15,1:4)
...
!******************************************************************
  use kind_types, only: dp
  use ol_kinematics_dp, only: cont_LC_cntrv, get_LC_5
  implicit none

  integer, intent(in) :: mom1, mom2, mom3
  complex(dp), intent(in) :: gamma
  real(dp), intent(out) :: gamma_norm
  complex(dp) :: p1(5), p2(5), p3(5)

  complex(dp) :: p1p1, p2p2, p3p3, p1p2, p1p3, p2p3
  real(dp) :: maxinv
  ...
  gamma_norm = min(1._dp, abs(gamma_norm))
...
! ******************************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: basis
  use ol_kinematics_dp, only: get_LC_5
  implicit none
  integer,     intent(in) :: mom3
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(out) :: p3scalars(0:4)
  real(dp),    intent(out) :: gd2,gd3

  complex(dp) :: p3(1:5), r1(1:4), r2(1:4), l3(1:4), l4(1:4)
  complex(dp) :: p3r1, p3r2, p3l3, p3l4
  complex(dp) :: two = 2._dp
...
!******************************************************************************
  ...
  use ol_data_types_dp, only: basis, redset4
  use ol_parameters_decl_dp, only: hp_switch,hp_err_thres,hp_redset_gd3_thres

  use ol_data_types_qp, only: basis_qp=>basis, redset4_qp=>redset4
  use ofred_basis_construction_qp, only: &
      construct_RedBasis_qp=>construct_RedBasis, &
      construct_p3scalars_qp=>construct_p3scalars
  use ol_external_decl_dp, only: init_qp
  use ol_kinematics_dp, only: init_qp_kinematics
  use ol_parameters_decl_dp, only: hp_qp_kinematics_init_mode

  implicit none
  integer, intent(in)  :: mom1, mom2, mom3
  type(basis), intent(in) :: RedBasis12, RedBasis13, RedBasis23
  type(redset4), intent(out) :: RedSet
  complex(dp) :: p1(1:5), p2(1:5), p3(1:5), scalars(0:4)
  real(dp) :: gd2,gd3
  logical :: b1, b2, b3
  integer :: perm(3)
  ...
  real(qp)    :: gd2_qp,gd3_qp
  complex(qp) :: scalars_qp(0:4)
...
! ******************************************************************************
  use kind_types, only: dp, qp
  use ol_data_types_dp, only: redset4_dp=>redset4
  use ol_data_types_qp, only: basis_qp=>basis, redset4_qp=>redset4
  use ofred_basis_construction_qp, only: construct_RedBasis_qp => construct_RedBasis, &
                                                    construct_p3scalars_qp=>construct_p3scalars
  implicit none
...
! ******************************************************************************
  use kind_types, only: dp, qp
  use ol_data_types_dp, only: redset5_dp=>redset5
  use ol_data_types_qp, only: basis_qp=>basis, redset5_qp=>redset5
  use ofred_basis_construction_qp, only: construct_RedBasis_qp => construct_RedBasis, &
                                                    construct_p3scalars_qp=>construct_p3scalars
  implicit none
...
! ******************************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: redset5
  implicit none
...
!*******************************************************************
  ...
  complex(dp) :: p1(1:5), p2(1:5), p3(1:5)
  real(dp)    :: gamma12(3), inv1, inv2, inv3, p1p2, p1p3, p2p3
...
!*******************************************************************
  ...
  complex(dp) :: p1(1:5), p2(1:5), p3(1:5)
  real(dp)    :: delta12(3), inv1, inv2, inv3, p1p2, p1p3, p2p3
...
!*******************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: basis, redset5
  use ol_kinematics_dp, only: cont_LC_cntrv, get_LC_5
  implicit none
  ...
  complex(dp) :: p1(1:5), p2(1:5), p3(1:5), p4(1:5)
  real(dp) :: gamma12(6), inv12, inv13, inv14, inv23, inv24, inv34, &
...
!*******************************************************************
  ...
  complex(dp) :: p1(1:5), p2(1:5), p3(1:5), p4(1:5)
  real(dp) :: delta12(6), inv12, inv13, inv14, inv23, inv24, inv34, &
                    p1p2, p1p3, p1p4, p2p3, p2p4, p3p4, max_value
  integer :: idx, i
...
!******************************************************************
  ...
  complex(dp), intent(in) :: p1(5), p2(5), p3(5)
  complex(dp), intent(out) :: delta
...
!******************************************************************
  ...
  complex(dp) :: p1(5),p2(5),p3(5)
  real(dp)    :: p1p1,p2p2,p3p3,p1p2,p1p3,p2p3
  real(dp)    :: maxinv
...
! ******************************************************************************
  ...
  complex(dp) :: scalars1(0:4),scalars2(0:4)
  real(dp)    :: alfa_max1,alfa_max2,gd21,gd22,gd31,gd32
...
end module ofred_basis_construction_dp
...
module ofred_reduction_dp
  use kind_types, only: dp
  use ol_debug, only: ol_error
  use ol_parameters_decl_dp, only: hp_switch,hp_alloc_mode
  use ofred_basis_construction_dp, only: gdm2_err_estim_OL, &
                                                   gdm2_err_estim_CL
...
! ******************************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in) :: v_cov(4)
  complex(dp), intent(out) :: v_ctrv(4)
...
! ******************************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in) :: msq(0:2), mass, B0coeff(1:4,0:2)
  complex(dp), intent(out) :: GTad(1)
...
! ******************************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in) :: tens1(2:5,1:4)
  complex(dp), intent(out) :: R2_out(6:15)
...
! ******************************************************************************
  ...
  implicit none
  complex(dp), intent(in)  :: Gin(:)
  complex(dp), intent(in)  :: p(1:5), msq(0:1)
  complex(dp), intent(out) :: B0coeff(1:4)
  complex(dp) :: tempcoeff(1:4)
  real(dp) :: thres_exp
  complex(dp) :: pp(6:15), PPmu, Gmunu, PPmunu, PPmunu_psq, PPmunu_m0, PP_m0m1
  real(dp) :: zero = 0._dp, one = 1._dp, two = 2._dp
  complex(dp) :: psq_m0, xi, f0, f1, f2, f3, f4, f5, z0
  integer :: i, j, n
  ...
  !Massless bubble
  if(msq(0) == msq(1) .AND. msq(0) == 0._dp) then
...
! ******************************************************************************
  ...
  use ol_data_types_dp, only: basis
  implicit none
  complex(dp), intent(in)  :: G_ab(6:15)
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(in) :: m0sq, fk0(2), gammas(2)
  complex(dp), intent(in) :: l1(4), l2(4), w1(4), w2(4)
  complex(dp), intent(out) :: redcoeff(1:5,1:4)

  complex(dp) :: GV34, Gv1Tc(1:4), Gv2Tc(1:4), Gv1Tb(1:4), Gv2Tb(1:4)
  complex(dp) :: Atens(1:2), Btens(1:4,1:4)
  integer :: j
...
! ******************************************************************************
  ...
  use ol_data_types_dp, only: basis
  implicit none
  complex(dp), intent(in)  :: Gin_A(:)
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(in) :: msq(0:2)
  complex(dp), intent(out):: Gout_A(1), Gout_A0(1), Gout_A1(1), Gout_A2(1), Gout_R1
  complex(dp), optional, intent(in) :: A0msq(:)
  complex(dp), optional, intent(out) :: A0_0(1), A0_1(1), A0_2(1)
  complex(dp) :: zero = 0._dp
...
! ******************************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: basis
  use ol_kinematics_dp, only: get_LC_mass2
  complex(dp), intent(in)  :: Gin_A(5)
  type(basis),       intent(in)  :: RedBasis
  complex(dp), intent(in)  :: msq(0:2)
  complex(dp), intent(out) :: Gout_A(1), Gout_A0(1), Gout_A1(1), Gout_A2(1)

  complex(dp) :: fk0(2), v1(4), v2(4), Gv1, Gv2
...
! ******************************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: basis
  use ol_loop_handling_dp, only: G_TensorShift_otf
  use ol_kinematics_dp, only: get_LC_4, get_LC_5, get_LC_mass2
  use ol_debug, only: ol_error, ol_msg
  complex(dp), intent(in)  :: Gin_A(15)
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(in) :: msq(0:2)
  complex(dp), intent(out) :: Gout_A(1), Gout_A0(1), Gout_A1(1), Gout_A2(1)
  complex(dp), intent(out) :: Gout_R1
  complex(dp), optional, intent(in) :: A0msq(:)
  complex(dp), optional, intent(out) :: A0_0(1), A0_1(1), A0_2(1)

  complex(dp) :: fk0(2), gammas(2), l1(4), l2(4), v1(4), v2(4), RedCoeff(5,4)
  complex(dp) :: Gtmp_A(5), Gtmp_A0(5), Gtmp_A1(5), Gtmp_A2(5), Gtmp_A3(5)
  complex(dp) :: B0_0(1), B0_1(1), B0_2(1), k2(5), B0coeff(1:4,0:2)
  complex(dp) :: zero = 0._dp
...
! ******************************************************************************
  ...
  complex(dp) :: G_r2(6:15,4), RedCoeff1(5,4), RedCoeff2(5,4), RedCoeff3(5,4), RedCoeff4(5,4)
  !rank2 subtopologies resulting from rank-3 3-p reduction
  complex(dp), dimension(15) :: Sub_A, Sub_A0, Sub_A1, Sub_A2
  complex(dp) :: tad_0(1), tad_1(1), tad_2(1)
  !various rational contributions
  complex(dp) :: R1_term(5)
  complex(dp) :: fk0(2), gammas(2), l1(4), l2(4), v1(4), v2(4), p12(4)
  complex(dp) :: Gtmp_A(5), Gtmp_A0(5), Gtmp_A1(5), Gtmp_A2(5), Gtmp_A3(5)
  complex(dp) :: B0_0(1), B0_1(1), B0_2(1), k2(5), B0coeff(1:4,0:2)
  integer :: i
...
! ******************************************************************************
  ...
  complex(dp), intent(in)  :: G_ab(6:15)
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(in) :: m0sq, p3scalars(0:4), fk0(3), gammas(2)
  complex(dp), intent(in) :: l1(4), l2(4), w1(4), w2(4)
  complex(dp), dimension(5,5), intent(out) :: redcoeff

  complex(dp) :: p3l3, sc1, sc2, alpha, GL33, GL44, GL34, GTa
  complex(dp) :: Gv1Tb(1:4), Gv2Tb(1:4), Gv1Tc(1:4), Gv2Tc(1:4), A_munu(2), Btens(4,5)
  integer :: k, j
  ...
  A_munu(2) = GTa/(4._dp*gammas(1)) ! D0-pinch contribution
...
! ***************************************************************************************
  ...
  use ol_data_types_dp, only: basis
  implicit none
  complex(dp), intent(in)  :: G_a(4,15)
  type(basis), intent(in) :: RedBasis
  complex(dp), intent(in) :: m0sq, p3scalars(0:4), fk0(3), gammas(2)
  complex(dp), intent(in) :: l1(4), l2(4), w1(4), w2(4)
  complex(dp), intent(out) :: redcoeff(4,5,5)

  complex(dp) :: p3l3, sc1, sc2, alpha, GL33(4), GL44(4), GL34(4), GTa(4)
  complex(dp) :: Gv1Tb(4,4), Gv2Tb(4,4), Gv1Tc(4,4), Gv2Tc(4,4), A_munu(4,2), Btens(4,4,5)
  integer :: k, j
...
! **********************************************************************************
  ...
  use ol_kinematics_dp, only: get_LC_mass2
  implicit none
  complex(dp), dimension(4,15,4), intent(in) :: Gin_A
  type(redset4), intent(in) :: RedSet_4
  complex(dp), intent(in) :: msq_in(0:3)
  complex(dp), dimension(4,5,4), intent(out) :: Gout_A, Gout_A0, Gout_A1, Gout_A2, Gout_A3
  complex(dp), optional, intent(out) :: Gout_R1(4,1,4)

  complex(dp), dimension(4,5,4) :: Gtmp_A1, Gtmp_A2, Gtmp_A3
  type(basis) :: Red_Basis
  complex(dp) :: gammas(2), p3_scalars(0:4), zero = 0._dp
  integer :: a, b, l
  logical :: perm(3), zerocheck
  complex(dp) :: fk0(3), msq(0:3), RedCoeff(4,5,5), l1(4), l2(4), v1(4), v2(4)
...
! ******************************************************************************
  ...
  type(redset4), intent(in) :: RedSet_4
  complex(dp), intent(in) :: msq_in(0:3)
  complex(dp), intent(out) :: Gout_A(:), Gout_A0(:), Gout_A1(:), Gout_A2(:), Gout_A3(:)
  complex(dp), optional, intent(out) :: Gout_R1
...
! **********************************************************************************
  ...
  type(redset4), intent(in) :: RedSet_4
  complex(dp), intent(in) :: msq_in(0:3)
  complex(dp), intent(out), dimension(5) :: Gout_A, Gout_A0, Gout_A1, Gout_A2, Gout_A3
  complex(dp), optional, intent(out) :: Gout_R1

  complex(dp) :: RedCoeff(5,5), fk0(3), p3_scalars(0:4), msq(0:3), gammas(2)
  complex(dp) :: l1(4), l2(4), v1(4), v2(4)
  complex(dp), dimension(5) :: Gtmp_A1, Gtmp_A2, Gtmp_A3
  type(basis) :: Red_Basis
  logical :: perm(3)
...
! **********************************************************************************
  use kind_types, only: dp
  use ol_data_types_dp, only: basis, redset4
  use ol_kinematics_dp, only: get_LC_mass2,cont_LC_cntrv
  implicit none
  complex(dp), intent(in) :: Gin_A(5)
  type(redset4),     intent(in) :: RedSet_4
  complex(dp), intent(in) :: msq_in(0:3)
  complex(dp), dimension(1), intent(out) :: Gout_A, Gout_A0, Gout_A1, Gout_A2, Gout_A3
  complex(dp) :: alpha0,alpha1,alpha2,alpha3,gamma,p3_scalars(0:4)
  complex(dp) :: l1(4),l2(4),l3(4),l4(4),fk0(3),msq(0:3)
  complex(dp) :: Gv1,Gv2,Gl3,Gl4,Gtmp_A1(1),Gtmp_A2(1),Gtmp_A3(1)
  type(basis) :: Red_Basis
  logical :: perm(3)
...
! ******************************************************************************
  ...
  complex(dp),                    intent(in)  :: msq_in(0:4)
  complex(dp), dimension(4,5,4),  intent(out) :: Gout_A,Gout_A0,Gout_A1,Gout_A2,Gout_A3,Gout_A4
  complex(dp), optional,          intent(out) :: Gout_R1(4,1,4)

  complex(dp), dimension(4,5,4) :: Gtmp_A1,Gtmp_A2,Gtmp_A3,Gtmp_A4
  complex(dp) :: gammas(2),p3_scalars(0:4)
  type(basis) :: Red_Basis
  integer :: a,b,l,perm(4)
  logical :: zerocheck
  complex(dp) :: fk0(3),msq(0:4),RedCoeff(4,5,5),l1(4),l2(4),v1(4),v2(4)
...
! ******************************************************************************
  ...
  type(redset5), intent(in) :: RedSet_5
  complex(dp), intent(in) :: msq_in(0:4)
  complex(dp), intent(out) :: Gout_A(:), Gout_A0(:), Gout_A1(:), Gout_A2(:), Gout_A3(:), Gout_A4(:)
  complex(dp), optional, intent(out) :: Gout_R1
...
! **********************************************************************************
  ...
  type(redset5), intent(in) :: RedSet_5
  complex(dp), intent(in) :: msq_in(0:4)
  complex(dp), intent(out), dimension(5) :: Gout_A, Gout_A0, Gout_A1, Gout_A2, Gout_A3, Gout_A4
  complex(dp), optional, intent(out) :: Gout_R1

  complex(dp) :: RedCoeff(5,5), fk0(3), p3_scalars(0:4), msq(0:4), gammas(2)
  complex(dp) :: l1(4), l2(4), v1(4), v2(4)
  complex(dp), dimension(5) :: Gtmp_A1, Gtmp_A2, Gtmp_A3, Gtmp_A4
  type(basis) :: Red_Basis
  integer :: perm(4)
...
subroutine trigger_upgrade_hol(alfa_max,gamman,Gin)
  use ol_parameters_decl_dp, only: hp_step_thres,hp_err_thres
  use ol_data_types_dp, only: hol

  use ol_loop_handling_dp, only: upgrade_qp

  real(dp), intent(in)    :: alfa_max,gamman
...
subroutine err_estim_4pt_hol(RedSet,Gin_A,Gout_A,Gout_A0,Gout_A1,Gout_A2,Gout_A3,Gout_R1)
  use ol_data_types_dp, only: hol,dp,qp,redset4
  use ol_parameters_decl_dp, only: hp_max_err
  use ol_parameters_decl_dp, only: hp_gamma_trig, &
                                              hp_alloc_mode, &
                                              hybrid_dp_mode

  use ol_loop_handling_dp, only: hol_alloc_hybrid
  ...
  type(hol),           intent(inout) :: Gout_A,Gout_A0,Gout_A1,Gout_A2,Gout_A3
  type(hol), optional, intent(inout) :: Gout_R1
  real(dp)    :: alfa_max,alfa(3), gamma_inv
  complex(dp) :: B3coeff
  integer           :: mode,perm(3)

  perm =RedSet%perm
  B3coeff = (RedSet%redbasis%gamma)*(RedSet%p3scalars(0))*2._dp
  alfa = [abs(RedSet%p3scalars(1)), abs(RedSet%p3scalars(2)), abs(B3coeff)]
  alfa = log10(alfa)

  if (hp_gamma_trig) then
    gamma_inv=log10( 1._dp / RedSet%gd2**2)
  else
    gamma_inv=0
  end if
...
subroutine err_estim_4pt_hcl(RedSet,Gin_A,Gout_A,Gout_A0,Gout_A1,Gout_A2,Gout_A3,Gout_R1)
  use ol_data_types_dp, only: hcl,dp,qp,redset4
  use ol_parameters_decl_dp, only: hp_max_err
  use ol_parameters_decl_dp, only: hp_gamma_trig, &
                                              hp_alloc_mode, &
                                              hybrid_dp_mode

  use ol_loop_handling_dp, only: hcl_alloc_hybrid
...
subroutine err_estim_5pt_hol(RedSet,Gin_A,Gout_A,Gout_A0,Gout_A1,Gout_A2, &
                             Gout_A3,Gout_A4,Gout_R1)
  use ol_data_types_dp, only: hol,dp,qp,redset5
  use ol_parameters_decl_dp, only: hp_max_err
  use ol_parameters_decl_dp, only: hp_gamma_trig, &
                                              hp_alloc_mode, &
                                              hybrid_dp_mode
...
subroutine err_estim_5pt_hcl(RedSet,Gin_A,Gout_A,Gout_A0,Gout_A1,Gout_A2, &
                             Gout_A3,Gout_A4,Gout_R1)
  use ol_data_types_dp, only: hcl,dp,qp,redset5
  use ol_parameters_decl_dp, only: hp_max_err
  use ol_parameters_decl_dp, only: hp_gamma_trig, &
                                              hp_alloc_mode, &
                                              hybrid_dp_mode
...
subroutine downgrade_4pt_hol(RedSet,mode_in,Gout_A1,Gout_A2,Gout_A3)
  use ol_data_types_dp, only: hol,dp,redset4
  use ol_parameters_decl_dp, only: hp_step_thres,hybrid_dp_mode
  implicit none
  type(redset4), intent(in)     :: RedSet
  ...
  subroutine downgrade(Gout_P1,Gout_P2,Gout_P3)
    use ol_parameters_decl_dp, only: hp_step_thres,hp_err_thres
    use ol_loop_handling_dp, only: downgrade_dp
...
subroutine downgrade_4pt_hcl(RedSet,mode_in,Gout_A1,Gout_A2,Gout_A3)
  use ol_data_types_dp, only: hcl,dp,redset4
  use ol_parameters_decl_dp, only: hp_step_thres,hybrid_dp_mode
  implicit none
  type(redset4), intent(in)    :: RedSet
  integer,       intent(in)    :: mode_in
...
subroutine downgrade_5pt_hol(RedSet,mode_in,Gout_A1,Gout_A2,Gout_A3,Gout_A4)
  use ol_data_types_dp, only: hol,dp,redset5
  use ol_parameters_decl_dp, only: hp_step_thres,hybrid_dp_mode
  implicit none
  type(redset5), intent(in)    :: RedSet
  integer,       intent(in)    :: mode_in
...
subroutine downgrade_5pt_hcl(RedSet,mode_in,Gout_A1,Gout_A2,Gout_A3,Gout_A4)
  use ol_data_types_dp, only: hcl,dp,redset5
  use ol_parameters_decl_dp, only: hp_step_thres,hybrid_dp_mode
  implicit none
  type(redset5), intent(in)    :: RedSet
  integer,       intent(in)    :: mode_in
...
! ******************************************************************************
  use kind_types, only: dp,qp
  use ol_data_types_dp, only: redset4, hol
  use ofred_basis_construction_dp, only: reconstruct_redset4
  use ol_kinematics_dp, only: get_mass2

  use ol_data_types_qp, only: redset4_qp=>redset4
  use ofred_basis_construction_dp, only: upgrade_redset4
  use ofred_reduction_qp, only: otf_4pt_red_qp=>otf_4pt_red
  use ol_loop_handling_dp, only: req_qp_cmp,hol_dealloc_hybrid
  use ol_kinematics_qp, only: get_mass2_qp=>get_mass2

  use ol_parameters_decl_dp, only: hp_switch,hybrid_qp_mode, &
                                              hp_alloc_mode
  implicit none
  type(redset4), intent(in)    :: RedSet_4
...
! ******************************************************************************
  use kind_types, only: dp,qp
  use ol_data_types_dp, only: redset5,hol
  use ol_parameters_decl_dp, only: rzero
  use ol_kinematics_dp, only: get_mass2

  use ol_data_types_qp, only: redset5_qp=>redset5
  use ofred_basis_construction_dp, only: upgrade_redset5
  use ofred_reduction_qp, only: otf_5pt_red_qp=>otf_5pt_red
  use ol_loop_handling_dp, only: req_qp_cmp,hol_dealloc_hybrid
  use ol_kinematics_qp, only: get_mass2_qp=>get_mass2
...
! ******************************************************************************
  use kind_types, only: dp,qp
  use ol_data_types_dp, only: redset4, hcl
  use ol_kinematics_dp, only: get_mass2

  use ol_data_types_qp, only: redset4_qp=>redset4
  use ofred_basis_construction_dp, only: upgrade_redset4
  use ofred_reduction_qp, only: otf_4pt_reduction_last_qp=>otf_4pt_reduction_last
  use ol_loop_handling_dp, only: req_qp_cmp,hcl_dealloc_hybrid
  use ol_kinematics_qp, only: get_mass2_qp=>get_mass2
...
! ******************************************************************************
  use kind_types, only: dp,qp
  use ol_data_types_dp, only: redset5, hcl
  use ol_parameters_decl_dp, only: rzero
  use ol_kinematics_dp, only: get_mass2
