...
! **********************************************************************
  implicit none
  real(qp),    intent(in)  :: P(0:3), M
  integer,           intent(in)  :: POL
  complex(qp), intent(out) :: J_S(4)
  J_S(1) = 1
  J_S(2:4) = 0
end subroutine wf_S
...
! **********************************************************************
  ...
  use ol_parameters_decl_dp, only: wf_v_select
  implicit none
...
subroutine wf_gf_V(P,POL,J_V)
  use kind_types
  use ol_external_decl_qp, only: gf_array, inverse_crossing
  ...
  integer,           intent(in)  :: POL
  complex(qp), intent(out) :: J_V(4) ! Light-cone repr
  real(qp)    :: Pmod, Pgfmod, cos_theta, sin_theta, Pgf(0:3), J_Lor(0:3), P_check(0:3,size(P)) ! Lorentz repr
  integer           :: i, part_i
...
! **********************************************************************
  use ol_parameters_decl_qp, only: CI, sqrt05
  use ol_debug, only: ol_fatal, ol_error
  implicit none

  real(qp),    intent(in)  :: P(0:3), M
  integer,           intent(in)  :: POL
  complex(qp), intent(out) :: EPS(4)
  integer, optional, intent(in)  :: POLSEL
  real(qp)    :: P2_T, P_T, P_MOD
  real(qp)    :: SIN_THETA, ONEPCOS_THETA , ONEMCOS_THETA
  real(qp)    :: COS_THETA, COS_PHI, SIN_PHI
  complex(qp) :: EPHI_PLUS, EPHI_MINUS
...
! **********************************************************************
  ...
  integer,           intent(in)  :: POL
  integer, optional,  intent(in)  :: POLSEL
  complex(qp), intent(out) :: WF(4)
  real(qp)    :: P2_T, P_T, P_MOD
  real(qp)    :: COST, SINT, COSTHALF, SINTHALF, CHI, COSP, SINP
  real(qp)    :: LAPLUS, LAMINUS
  complex(qp) :: ZETA
...
end module ol_wavefunctions_qp



! **********************************************************************
module ol_s_wavefunctions_qp
...
! **********************************************************************
  use ol_data_types_qp, only: wfun
  use ol_wavefunctions_qp, only: wfIN_Q
  implicit none
  real(qp), intent(in)  :: P(0:3), M
  integer,        intent(in)  :: POL
...
! **********************************************************************
  ...
  use ol_data_types_qp, only: wfun
  use ol_kinematics_qp, only: Std2LC_Rep
  use ol_wavefunctions_qp, only: wf_V_Std
  implicit none
  real(qp), intent(in)  :: P(0:3), M
  integer,        intent(in)  :: POL
...
end module ol_s_wavefunctions_qp



! **********************************************************************
module ol_h_wavefunctions_qp
! Routines to calculate external wave functions:
...
end module ol_h_wavefunctions_qp
...
! **********************************************************************
module ol_hel_wavefunctions_qp
! Routines to calculate external wave functions:
! return an array of wave functions, one for each polarisation.
! - wf_S: scalar
! - wf_Q: fermion
...
! **********************************************************************
  use kind_types, only: qp
  use ol_data_types_qp, only: wfun
  use ol_loop_handling_qp, only: hp_switch
  implicit none
...
end module ol_hel_wavefunctions_qp
