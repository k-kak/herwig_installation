...
module ol_kinematics_dp
  ...
  real(dp) :: collthres = 1.E-4
  real(dp) :: softthres = 1.E-4
...
! **********************************************************************
  ...
  complex(dp), dimension(size(mids)) :: m
  integer :: i
...
! **********************************************************************
  ...
  use ol_parameters_decl_dp
  integer, dimension(:),        intent(in) :: mids
  complex(dp), dimension(size(mids)) :: m2
  integer :: i
...
! **********************************************************************
  ...
  use ol_parameters_decl_dp
  integer, dimension(:),     intent(in) :: mids
  real(dp), dimension(size(mids)) :: m2
  integer :: i
...
! **************************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp) :: cont_LC_cntrv
  complex(dp), intent(in) :: V1(1:4)
  complex(dp), optional, intent(in) :: V2(1:4)
  if(present(V2)) then
...
subroutine rand_sphere(r, v)
  ...
  use ol_parameters_decl_dp, only: pi
...
subroutine decay3(E_in, m, psp)
  ...
  use ol_parameters_decl_dp, only: psp_tolerance
  use ol_debug, only: ol_error, ol_msg, ol_fatal
  implicit none
  real(dp), intent(in) :: E_in, m(3)
  real(dp), intent(out) :: psp(0:3,3)
  real(dp) :: E1, m12, m22, m32, E2a, E3a, p1_abs, p2a_abs, p2a(3), gamma, beta(3), beta_p2a
  if (m(1) <= m(2) + m(3)) then
...
! *********************************************************************
  ...
  real(dp), intent(in)  :: sqrt_s, m_ex(:)
  real(dp), intent(out) :: p_rambo(0:3,size(m_ex))
  real(dp) :: p_scatt(4,size(m_ex)-1), wgt, p1
  integer :: n, k
  n = size(m_ex)
...
! **********************************************************************
  ...
  real(dp)  :: softness, collinearity, hardness(3:n)
...
subroutine init_kinematics_mids(P_scatt, m_ext2, P_in_clean, perm_inv, n, qp_kinematics)
  ...
  use ol_parameters_decl_dp, only: hp_switch,hp_qp_kinematics_init_mode,&
                                             hp_nsi,hp_nsi_qp,hp_ndrs,hp_ndrs_qp, &
                                             hp_nred,hp_nred_qp,hp_max_err
  use ol_kinematics_qp, only: init_kinematics_mids_qp=>init_kinematics_mids

  use ol_external_decl_dp, only: init_qp
  integer,         intent(in)  :: n
  real(dp), intent(in)  :: P_scatt(0:3,n)
  integer,         intent(in)  :: m_ext2(n)
  real(dp),  intent(inout) :: P_in_clean(0:3,n)
  integer,         intent(in)  :: perm_inv(n)
...
subroutine init_qp_kinematics()
  use ol_external_decl_dp, only: nParticles,M_ex,init_qp
  use kind_types, only: qp
  use ol_kinematics_qp, only: conv_mom_scatt2in_cache, &
    internal_momenta_six_qp=>internal_momenta_six
  real(qp) :: P_in_clean_qp(0:3,nParticles)
  real(qp) :: P_qp(0:3,nParticles)
  call conv_mom_scatt2in_cache(P_in_clean_qp, nParticles)
  call internal_momenta_six_qp(P_in_clean_qp, nParticles, M_ex, .false.)
  init_qp = .true.
  call internal_momenta_six(real(P_in_clean_qp,kind=dp), nParticles, M_ex, .true.)
...
! **********************************************************************
  ...
  use ol_parameters_init_dp, only: init_kin_arrays
!#ifdef 1
...
subroutine conv_mom_scatt2in_mids(P_scatt, m_ext, P_in_clean, perm_inv, n)
  use kind_types, only: dp, dp
  use ol_external_decl_dp, only: M_ex
  use ol_external_decl_dp, only: n_scatt
  use ol_parameters_decl_dp, only: scalefactor
  use ol_parameters_init_dp, only: init_kin_arrays
!#ifdef 1
...
! **********************************************************************
  ...
  use ol_momenta_decl_dp, only: L, collconf, softconf

  use ol_parameters_decl_dp, only: hp_switch, hp_mode, sync_qp_kinematics
  use ol_momenta_decl_qp, only: L_qp=>L
...
! **********************************************************************
  use kind_types, only: dp
  use ol_momenta_decl_dp, only: L
  use ol_loop_parameters_decl_dp, only: zero
  implicit none

  integer,        intent(in) :: Npart, Jmax
  real(dp), intent(in) :: ext_masses(Npart)
  real(dp), intent(in) :: P_ex(0:3,Npart)
  logical, intent(in)        :: use_qp_kinematics
  integer :: i1 ! conventional particle numbers
  integer :: l1 ! individual 2^(i-1) particles numbers
...
! **********************************************************************
  ...
  use ol_parameters_decl_dp, only: hp_mode, hp_switch, sync_qp_kinematics
  use ol_momenta_decl_qp, only: L_qp=>L
  use ol_kinematics_qp, only: cont_LC_cntrv_qp=>cont_LC_cntrv
...
! **********************************************************************
  use kind_types, only: dp
  use ol_external_decl_dp, only: binom2
...
! **********************************************************************
  ...
  complex(dp) :: invs(binom2(size(moms)+1))
