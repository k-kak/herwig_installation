...
!   end if
  ...
  if (CT_is_on /= 0) then
    ...
    ctHEFTgggh  = (11+3*dgQCD + 1.5_dp*dZg)
  ...
  if (R2_is_on /= 0) then
    ...
    if (SwB /= 0) then
      ...
      ! HEFT
      ctHEFTggh   = ctHEFTggh + [ 1, 89, 14, -17, -93 ] * (nc/24._dp)
      ctHEFTgggh  = ctHEFTgggh - 15*nc/8._dp
      R2HEFTggggh = 0.125_dp
      R2HEFThqq   = 0.25_dp*(nc-1._dp/nc)
      R2HEFTghqq  = 0.25_dp*(3._dp/nc-5*nc)
...
module ol_qcd_offshell_selfenergiesdp
  use kind_types, only: dp, dp
  use ol_loop_parameters_decl_dp, only: de1_IR, de1_UV, de2_i_IR
  use ol_debug, only: ol_msg, ol_error, ol_fatal
  use ol_generic, only: to_string
...
function gluon_ofsse(p2,pid)
  ...
  complex(dp) :: cc1,gluon_ofsse(3),B01,B02,B03,B04
  complex(dp) :: cc1R1,dZ

  !complex(dp) :: B0_1,B0_2,B1_1,B1_2,B11_1,B11_2,B00_1,B00_2
  real(dp) :: ncc,nlf
  integer :: i
...
function quark_ofsse(p2,pid)
  ...
  complex(dp), intent(in) :: p2
  integer,           intent(in) :: pid
  complex(dp) :: cc1,cc2,cc3,quark_ofsse(2),B01,B02,B03
  complex(dp) :: cc1R1,cc2R1,cc3R1,fac
  complex(dp) :: cc1R2,cc2R2,cc3R2

  complex(dp) :: B0_1,A0_1,dZ,dM,M,M2
...
end module ol_qcd_offshell_selfenergiesdp
