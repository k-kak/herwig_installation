...
! ****************************************************
  ...
  use ol_external_decl_qp, only: nParticles, P_ex, crossing, inverse_crossing
  use ol_tensor_bookkeeping, only: rank_to_size, tensor_size
  use ol_Std2LC_converter_qp, only: lorentz2lc_tensor
  use ol_kinematics_qp, only: LC2Std_Rep_cmplx, momenta_invariants
  ...
  complex(qp), allocatable :: T2dim(:,:)
  complex(qp) :: momenta_TI(0:3,size(momenta,2)), T_Lor(size(TI))
  integer           :: l, r
...
! ****************************************************
  ...
  use ol_kinematics_qp, only: LC2Std_Rep, momenta_invariants
  use collier, only: tnten_cll

  implicit none
  complex(qp), intent(in)  :: momenta(:,:), masses_2(:)

  complex(qp) :: T2dim(1,0:0), tuv(0:0)
  complex(qp) :: momenta_TI(0:3,size(momenta,2))
  real(qp)    :: mom(0:3)
  integer           :: l
  do l = 1, size(momenta,2)
...
!************************************************************************************
  ...
  use ol_parameters_decl_dp, only: a_switch
  use ol_loop_parameters_decl_qp, only: de1_IR, de2_i_IR
  use ol_loop_reduction_qp, only: TI_reduction, OPP_reduction, scalar_MIs

  use ol_parameters_decl_qp, only: hybrid_zero_mode, hp_switch
  implicit none
  ...
  complex(qp) :: TI(size(Gsum_hcl%cmp)), p_std(0:3,1:size(momenta)-1)
  complex(qp) :: M2add, box(0:2)
  logical :: single_box
  ...
  subroutine external_library()
    ...
    call TI_call(rank, cmplx(p(1:4,1:size(momenta)-1),kind=qp), &
...
! *******************************************************************
  ...
  use ol_kinematics_qp, only: get_LC_mass2,get_LC_4,get_mass2
  use ol_debug, only: ol_fatal, ol_msg, ol_error
  implicit none
  integer,           intent(in)    :: qt2power, rank
  !complex(qp), intent(in)    :: momenta(:,:), masses_2(:), Gsum(:)
  complex(qp), intent(in)    :: Gsum(:)
  integer,           intent(in)    :: momenta(:), masses(:)
  real(qp),    intent(inout) :: M2
  complex(qp) :: M2add
  complex(qp) :: p1p1, p12(4), zero
...
! beta  = contravariant (light-cone) "active" index contracted with vertices/props to build the loop
  use kind_types, only: qp
  implicit none
  complex(qp), intent(in) :: preFactor
  complex(qp), intent(out) :: G0(4,1,4)
  G0 = 0
  G0(1,1,1) = preFactor
  G0(2,1,2) = preFactor
