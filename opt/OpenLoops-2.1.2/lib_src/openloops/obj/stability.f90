...
subroutine write_histogram(processname, hist, triggered)
  use ol_parameters_decl_dp, only: pid_string, stability_logdir, max_parameter_length
  implicit none
...
subroutine write_point(processname, psp, mu, alphas, perm, me, hplog, hperr)
  ...
  real(dp), intent(in), optional :: hperr
  character(len=max_parameter_length) :: outfile
...
subroutine stability_trigger(processname, np, hist, &
  ...
  use ol_parameters_decl_dp, only: rONE, a_switch, a_switch_rescue, &
...
subroutine vamp2_dp(vamp2, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, redlib, mode)
  ...
  use ol_parameters_decl_dp, only: a_switch, expert_mode
  implicit none
...
function vamp2_qp_scaled(vamp2, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, redlib)
  use kind_types, only: dp, qp
  use ol_parameters_decl_dp, only: rescalefactor, scaling_mode, rONE
  use ol_loop_parameters_decl_dp, only: deviation_mode, polecheck_is
  use ol_init, only: set_parameter, parameters_flush
  use ol_generic, only: relative_deviation
  implicit none
  ...
  real(qp) :: vamp2_qp_scaled
  real(qp) :: M2L0_scaled, M2L1_scaled(0:2), IRL1_scaled(0:2), M2L2_scaled(0:4), IRL2_scaled(0:4)
  interface
    subroutine vamp2(P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, mode)
      use kind_types, only: dp, qp
      implicit none
...
subroutine vamp2generic(vamp2dp, vamp2qp, processname, P_scatt, M2L0, M2L1, IRL1, M2L2, IRL2, &
  ...
  use ol_parameters_decl_dp, only: alpha_qcd, current_processname, a_switch, &
    & a_switch_rescue, redlib_qp, write_psp, use_me_cache, scaling_mode
  use ol_loop_parameters_decl_dp, only: ratcorr_bad, ratcorr_bad_L2, &
  ...
  use ol_parameters_decl_dp, only: hp_switch, write_hp_log, hp_nsi, &
