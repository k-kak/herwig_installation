...
subroutine init_kin_arrays(Npart)
  use ol_momenta_decl_dp, only: Q, L, QInvariantsMatrix
  use ol_external_decl_dp, only: P_ex, M_ex, binom2, crossing, inverse_crossing, gf_array, Ward_array
  use ol_external_decl_dp, only: allocatedNpart
  implicit none
...
subroutine clean_kin_arrays
  use ol_external_decl_dp, only: allocatedNpart
  use ol_momenta_decl_dp, only: Q, L, QInvariantsMatrix, Q_qp, QInvariantsMatrix_qp, L_qp
  use ol_external_decl_dp, only: P_ex, M_ex, binom2, crossing, inverse_crossing, gf_array, Ward_array
  implicit none
...
subroutine tensorrank_init(rank)
  ...
  use ol_tensor_storage_dp, only: tensor_stored, tensor_storage_maxrank
...
! **********************************************************************
  ...
  use ol_tensor_storage_dp, only: tensor_storage_maxrank
  use ol_parameters_decl_dp
  ...
  real(dp), intent(in), optional :: pole1_UV, pole1_IR, pole2_IR
...
subroutine qcd_parameters_init(is_loop_in)
  use ol_parameters_decl_dp, only: alpha_QCD, G2_QCD, gQCD, pi

  use ol_loop_parameters_decl_dp, only: muren_unscaled, do_qcd_renorm
  use ol_loop_parameters_decl_dp, only: muren, muren2, scalefactor
  use ol_qcd_renormalisation_dp, only: qcd_renormalisation
  implicit none
...
subroutine ensure_mp_loop_init()
  ...
  use ol_parameters_init_qp, only: &
...
subroutine met_to_real(Mout,M)
  ...
  use ol_parameters_decl_dp, only: hp_switch,hp_nsi,hp_nsi_qp, &
hp_ndrs,hp_ndrs_qp,hp_nred,hp_nred_qp
  ...
  if (hp_switch .eq. 1) then
    Mout = Mout + real(M%cmp_qp,kind=dp)
    ! log
