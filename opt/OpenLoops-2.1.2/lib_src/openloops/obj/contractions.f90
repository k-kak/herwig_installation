...
! **********************************************************************
  ...
  implicit none
  complex(dp) :: cont_VV
  complex(dp), intent(in) :: A(4), B(4)
  cont_VV = A(1)*B(2) + A(2)*B(1) - A(3)*B(4) - A(4)*B(3)
  cont_VV = 0.5_dp * cont_VV
...
! **********************************************************************
  ...
  implicit none
  complex(dp) :: cont_V
  complex(dp), intent(in) :: A(4)
  cont_V = A(1)*A(2) + A(2)*A(1) - A(3)*A(4) - A(4)*A(3)
  cont_V = 0.5_dp * cont_V
end function cont_V
...
! **********************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp) :: contsum_VVV
  complex(dp), intent(in) :: A1(4), A2(4), B(4)
  contsum_VVV = (A1(1)+A2(1))*B(2) + (A1(2)+A2(2))*B(1) - (A1(3)+A2(3))*B(4) - (A1(4)+A2(4))*B(3)
  contsum_VVV = 0.5_dp * contsum_VVV
...
! **********************************************************************
  ...
  implicit none
  complex(dp) :: cont_EE
  complex(dp), intent(in) :: A(4), B(4), C(4), D(4)
...
! **********************************************************************
  use kind_types, only: dp
  implicit none
  complex(dp), intent(in)  :: B(4), C(4), D(4)
  complex(dp), intent(out) :: Aout(4)
  complex(dp)              :: C1D4, C1D3, C1D2, C2D3, C2D4, C3D4
  ...
  Aout    = (0._dp, 0.5_dp) * Aout
end subroutine cont_EpVVV

end module ol_contractions_dp



! **********************************************************************
module ol_s_contractions_dp
...
end module ol_s_contractions_dp



! **********************************************************************
module ol_h_contractions_dp
! Routines to contract two wave functions
! - cont_PP: two 4-vectors which are not stored in a wfun object
! - cont_SS: two scalars
! - cont_CD: two ghosts (this is the same as cont_SS)
...
! **********************************************************************
  ...
  use ol_data_types_dp
  use ol_helicity_bookkeeping_dp, only: helbookkeeping_cont
  implicit none
...
end module ol_h_contractions_dp
