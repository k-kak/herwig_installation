...
! ******************************************************************************
  ...
  use ofred_reduction_qp, only: twopoint_reduction
  ...
  use ol_data_types_qp, only: hcl, met
  use ol_kinematics_qp, only: get_mass2,get_LC_5
...
! ******************************************************************************
  ...
  use ofred_reduction_qp, only: otf_3pt_reduction_last
  ...
  complex(qp) :: Gout_R1, momenta(5,3),p(0:3)
...
! ******************************************************************************
  ...
  use ol_loop_parameters_decl_qp, only: mureg, zero
  use ol_kinematics_qp, only: LC2Std_Rep_cmplx
  implicit none
  integer, intent(in) :: mom1, mom2
  logical, intent(out) :: tr_t_top
  integer, intent(out) :: perm(3)
  complex(qp), intent(out) :: mom(5,3)
  real(qp), intent(out) :: sdlt
  logical :: zero_mass(3)
  complex(qp) :: k1(5), k2(5), k12(5), p(0:3)
  real(qp) :: rone
...
! ******************************************************************************
  use kind_types, only: qp
  use ol_momenta_decl_qp, only: L
  use ol_loop_parameters_decl_qp, only: mureg,zero,rone
  use ol_kinematics_qp, only: LC2Std_Rep_cmplx
  implicit none
  integer, intent(in) :: mom1, mom2
  logical, intent(out) :: unstable
  ...
  complex(qp) :: k1(5), k2(5), k12(5), p(0:3)
  real(qp) :: coll, soft1, soft2, sdlt
  complex(qp) :: k1_std(0:3), k2_std(0:3), k12_std(0:3)
...
! ******************************************************************************
  ...
  complex(qp), intent(in) :: msq(0:2), p1(1:5), p2(1:5)
  integer, intent(in) :: r, masses_ind
  real(qp), intent(in) :: dlt, psq
  complex(qp), intent(out) :: redcoeff(1:8)

  complex(qp) :: p12(1:4), psq_delta, psq_delta2
  complex(qp) :: contr_p12, contr_p1

  complex(qp) :: p1p1(6:15), p1p2(6:15), p1p12(6:15)
  complex(qp) :: p1p1p1(16:35), p1_p2(16:35), p1p12p12(16:35), p1p1p12(16:35)
  complex(qp) :: P11, P22, T, U, Gmn, W0, P111, V1, Gp1, Gp2, Gp12, Gm, Gp
  complex(qp) :: T3, T6, U3, V16, P1116, P111z3, P16, V
  complex(qp) :: dlt2, dlt3, dlt4, dlt5, dlt6, zero, one, two, five
  complex(qp) :: z, z0, z01, z1, ct1, ct2, z12, z2, zd, z10, z02, z11, z22
  integer :: i, j, k, n
...
! ******************************************************************************
  ...
  use ol_loop_parameters_decl_qp, only: mu2_UV, mu2_IR
  ...
  complex(qp) :: psq_delta,psq_delta2,dlt2,dlt3,dlt4
  complex(qp) :: contr_p1,contr_p2,contr_P12
  complex(qp) :: contr_P12P12,contr_p1p1,contr_p1P12,contr_g,contr_gp1,contr_gP12
  complex(qp) :: contr_P12P12P12,contr_p1p1p1,contr_p1P12P12,contr_p1p1P12
  complex(qp) :: p1p1(6:15),p2p2(6:15),p1p2(6:15),p1P12(6:15),P12P12(6:15),P12_vec(1:4)
  complex(qp) :: p1p1p1(16:35),p1p1P12(16:35),p1P12P12(16:35),P12P12P12(16:35)
  ...
  call set_duv(real(de1_UV,kind=qp))
  call set_dir(real(de1_IR,kind=qp))
...
! ********************************************************************************
  ...
  real(qp), intent(in) :: sdlt
  complex(qp), intent(inout)  :: Gin(:)
  complex(qp), intent(in) :: msq(0:2), mom(5,3)
  ...
  complex(qp) :: k1(5), k2(5), k12(5), masses(0:2), tadp0(1), tadp1(1), tadp2(1)
...
!******************************************************************************
  ...
  implicit none
  integer, intent(in) :: perm(3)
  complex(qp), intent(in) :: A0_msq(:)
  complex(qp), intent(in) :: masses(0:2)
  complex(qp), intent(in) :: tadp0(1), tadp1(1), tadp2(1)
  complex(qp), optional, intent(out) :: A0_0(1), A0_1(1), A0_2(1)
...
! ------------------------------------------------------------------------------
  ...
  use ol_parameters_decl_qp, only: zero
  implicit none
  complex(qp), intent(in)  :: p1(1:5), p2(1:5), msq(0:2)
  real(qp),    intent(in)  :: dlt
  complex(qp), intent(in)  :: Gin_A(:)
  complex(qp), intent(out) :: Gout_A(1), Gout_A0(1), Gout_A1(1), Gout_A2(1)
  complex(qp), intent(out) :: Gout_R1
  complex(qp), intent(out) :: A0_0(1), A0_1(1), A0_2(1)

  real(qp) :: psq
  integer :: m_ind
  logical :: dlt_exp
  complex(qp) :: redcoeff1(1:8), redcoeff2(1:8), redcoeff3(1:8)
...
!************************************************************************************
  ...
  M2%cmp = M2%cmp + real(M2add,kind=qp)
...
!************************************************************************************
  ...
  use ol_loop_parameters_decl_qp, only: mureg
  use ol_loop_parameters_decl_dp, only: de1_UV, de1_IR, de2_i_IR
  implicit none
  complex(qp), intent(in)  :: masses2(:), Gsum(:)
  integer, intent(in) :: momenta(:)
  complex(qp), intent(out) :: M2add
  logical           :: tadpole_check
  complex(qp) :: tadpole_mass, p1_2, p2_2, p3_2, p4_2, p12_2, p23_2
  complex(qp) :: rslt(0:2), zero = 0._qp
  integer :: i, j, k
...
!************************************************************************************
  ...
  use ol_loop_parameters_decl_qp, only: mureg
  implicit none
  complex(qp), intent(in)  :: masses2(4)
  integer,           intent(in)  :: momenta(3)
  complex(qp), intent(out) :: rslt(0:2)
  complex(qp) :: p1_2, p2_2, p3_2, p4_2, p12_2, p23_2
  integer :: i, j, k
...
! ------------------------------------------------------------------------------
  use kind_types, only: qp,qp
  use ol_momenta_decl_qp, only: L
  use ol_data_types_qp, only: redset4, scalarbox
  use ol_kinematics_qp, only: get_LC_5,get_mass2
  ...
  complex(qp) :: masses2(0:3),p(1:5,3),q0_p(5),q0_m(5),q0_cuts(2,5),sc_box(0:2)
  integer           :: mom_box(3)
  real(qp)    :: box_error,cut_error
...
! ********************************************************************
  ...
  use ol_parameters_decl_qp, only: hp_gamma_trig
  implicit none
  complex(qp), intent(in)  :: p(1:5,1:3), m2(0:3)
  complex(qp), intent(out) :: q0_p(1:5), q0_m(1:5)
  real(qp), intent(out) :: error
  type(redset4), intent(in) :: RedSet

  complex(qp) :: l(1:4,4), k1(1:5), k2(1:5), k3(1:5)
  complex(qp) :: a1, a2
  complex(qp) :: x1, x2, x3p, x3m, x4p, x4m, k3scalars(1:4)
  complex(qp) :: dd0, dd1, dd2, dd3
  complex(qp) :: c0, b0, A, B, R
  integer :: i, perm(3)
  type(basis) :: bas
  ...
  x1 = (dd2 - dd0*(1._qp-a2) - dd1*a2)/bas%gamma
  x2 = (dd1 - dd0*(1._qp-a1) - dd2*a1)/bas%gamma
  ...
  if (hp_gamma_trig) then
    error = max(log10(max(1._qp, ABS(x1))), log10(max(1._qp, ABS(x2))))
    error = max(error, max(log10(max(1._qp, ABS(x3m))), log10(max(1._qp, ABS(x3p)))))
  else
    error = max(log10(max(1._qp, ABS(x3m))), log10(max(1._qp, ABS(x3p))))
  end if
...
! ********************************************************************
  use kind_types, only: qp
  implicit none
  complex(qp), intent(in)  :: Gtensor(:), q0(4)
  complex(qp), intent(out) :: N0
...
! ********************************************************************
  ...
  use ol_kinematics_qp, only: cont_LC_cntrv
  implicit none
  complex(qp), intent(in)  :: Gtensor(:)
  complex(qp), intent(in)  :: p_offshell(:,:), m_offshell(:), q0_pm(2,5)
  complex(qp), intent(out) :: box_coeff
  complex(qp) :: q0_p(1:4), q0_m(1:4)
  complex(qp) :: N0p, N0m, Gsum(size(Gtensor))
  complex(qp) :: mom(1:4), prop_m, prop_p, Di_p, Di_m
  integer :: i
...
!*************************************************************************************
  use kind_types, only: qp
  use ol_data_types_qp, only: scalarbox, hcl, met
...
! ********************************************************************
  ...
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(inout) :: all_scboxes(:)
  real(qp) :: error,error_step,max_error_opp

  complex(qp) :: offshell_p(1:5,1), offshell_m2(1)
  complex(qp) :: Gsum(size(Gtensor%cmp))
  complex(qp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)
...
! ********************************************************************
  ...
  use ol_loop_handling_qp, only: G_TensorShift_otf
  use ol_data_types_qp, only: scalarbox, hcl, met
  use ol_kinematics_qp, only: get_LC_4,get_LC_5,get_mass2
  use ol_loop_parameters_decl_dp, only: de1_IR, de2_i_IR
  ...
  complex(qp) :: offshell_p(1:5,2), offshell_m2(2)
  complex(qp) :: Gsum(size(Gtensor%cmp))
  complex(qp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)
...
! ********************************************************************
  ...
  complex(qp) :: offshell_p(1:5,3), offshell_m2(3)
  complex(qp) :: Gsum(size(Gtensor%cmp))
  complex(qp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)
...
! ********************************************************************
  ...
  complex(qp) :: offshell_p(1:5,4), offshell_m2(4)
  complex(qp) :: Gsum(size(Gtensor%cmp))
  complex(qp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)
...
! ********************************************************************
  ...
  complex(qp) :: offshell_p(1:5,5), offshell_m2(5)
  complex(qp) :: Gsum(size(Gtensor%cmp))
  complex(qp) :: box_coeff, box, scalar_box(0:2), q0p_q0m(2,5)
...
! ********************************************************************
  use kind_types, only: qp
  use ol_loop_handling_qp, only: G_TensorShift_otf
  use ol_data_types_qp, only: scalarbox
  use ol_loop_parameters_decl_dp, only: de1_IR, de2_i_IR
  implicit none
  integer, intent(in) :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:4), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error

  complex(qp) :: box_p(1:4,0:3,1:5), box_m2(0:3,5)
  complex(qp) :: offshell_p(1:5,1,5), offshell_m2(1,5)
  complex(qp) :: Gsum(size(Gtensor))

  complex(qp) :: box_coeff(5), box(5), scalar_box(0:2), q0p_q0m(2,5)
  integer :: i, j, k
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:5), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error
  ...
  complex(qp) :: box_coeff(size(scboxes)), box(size(scboxes)), scalar_box(0:2)
  complex(qp) :: q0p_q0m(2,5), momshift(5), Gsum(size(Gtensor))
  integer :: i, j, k
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:6), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:7), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:8), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error
...
! ********************************************************************
  ...
  implicit none
  integer,           intent(in)  :: rank
  complex(qp), intent(in)  :: momenta(:,:), masses2(0:9), Gtensor(:)
  complex(qp), intent(out) :: M2add
  integer,         intent(in) :: scboxes(:)
  type(scalarbox), intent(in) :: all_scboxes(:)
  real(qp), intent(out) :: box_error

  complex(qp) :: offshell_p(1:5,6), offshell_m2(6)
...
end module ol_loop_reduction_qp
