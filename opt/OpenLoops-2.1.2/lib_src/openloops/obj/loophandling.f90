...
!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine upgrade_qp_hcl(Gin)
    ...
    if (Gin%mode .eq. hybrid_dp_mode) then
      ...
      Gin%cmp_qp(:) = cmplx(Gin%cmp(:),kind=qp)
      Gin%cmp(:) = ZERO
    else if (Gin%mode .eq. hybrid_dp_qp_mode) then
      Gin%mode = hybrid_qp_mode
      Gin%cmp_qp(:) = Gin%cmp_qp(:) + cmplx(Gin%cmp(:),kind=qp)
      Gin%cmp(:) = ZERO
    end if
  ...
  subroutine upgrade_qp_hol(Gin)
    ...
    if (Gin%mode .eq. hybrid_dp_mode) then
      ...
      Gin%j_qp(:,:,:,:) = cmplx(Gin%j(:,:,:,:),kind=qp)
      Gin%j(:,:,:,:) = ZERO
    ! mixed dp.qp mode -> pure qp
    else if (Gin%mode .eq. hybrid_dp_qp_mode) then
      Gin%mode = 2
      Gin%j_qp(:,:,:,:) = Gin%j_qp(:,:,:,:) + cmplx(Gin%j(:,:,:,:),kind=qp)
      Gin%j(:,:,:,:) = ZERO
    end if
  ...
  subroutine downgrade_dp_hcl(Gin)
    ...
    if (Gin%mode .eq. hybrid_qp_mode) then
      ...
      Gin%cmp(:) = cmplx(Gin%cmp_qp(:),kind=dp)
    else if (Gin%mode .eq. hybrid_dp_qp_mode) then
      Gin%mode = hybrid_dp_mode
      Gin%cmp(:) = Gin%cmp(:) + cmplx(Gin%cmp_qp(:),kind=dp)
    end if
  ...
  subroutine downgrade_dp_hol(Gin)
    ...
    if (Gin%mode .eq. hybrid_qp_mode) then
      Gin%mode = hybrid_dp_mode
      Gin%j(:,:,:,:) = cmplx(Gin%j_qp(:,:,:,:),kind=dp)
    ! mixed dp.qp mode -> pure dp
    else if (Gin%mode .eq. hybrid_dp_qp_mode) then
      Gin%mode = hybrid_dp_mode
      Gin%j(:,:,:,:) = Gin%j(:,:,:,:) + cmplx(Gin%j_qp(:,:,:,:),kind=dp)
    end if
...
!******************************************************************************
  ...
  type(hol), intent(inout) :: Gin
  complex(dp) :: Gout(4,r1:r2,4,size(Gin%hf))
  integer :: al ,be, h, phys_hel

  complex(qp) :: Gout_qp(4,r1:r2,4,size(Gin%hf))
...
!******************************************************************************
  use kind_types, only: dp, intkind1, qp
  ...
  complex(dp) :: dQ(4)
  complex(dp) :: G1shift(4,1,4,size(HG1in%hf))

  complex(qp) :: G1shift_qp(4,1,4,size(HG1in%hf))
  complex(qp) :: dQ_qp(4)
...
!******************************************************************************
  ...
  implicit none
  complex(dp), intent(in) :: dQ(4)
  complex(dp), intent(inout) :: G1in(5)
  complex(dp) :: G1shift
...
!******************************************************************************
  ...
  implicit none
  complex(dp), intent(in) :: dQ(4)
  complex(dp), intent(inout) :: Gin(15)
  complex(dp) :: g2q0, g3q1,  g4q2,  g5q3
  complex(dp) :: g6q0, g7q1,  g8q2,  g9q3
  complex(dp) :: g7q0, g10q1, g11q2, g12q3
  complex(dp) :: g8q0, g11q1, g13q2, g14q3
  complex(dp) :: g9q0, g12q1, g14q2, g15q3
  complex(dp) :: g2, g3, g4
...
!******************************************************************************
  ...
  implicit none
  complex(dp), intent(in) :: dQ(4)
  complex(dp), intent(inout) :: Gin(35)
  complex(dp) :: g16q1, g17q2, g18q3, g19q4
  complex(dp) :: g17q1, g20q2, g21q3, g22q4
  complex(dp) :: g18q1, g21q2, g23q3, g24q4
  complex(dp) :: g19q1, g22q2, g24q3, g25q4
  complex(dp) :: g20q1, g26q2, g27q3, g28q4
  complex(dp) :: g21q1, g27q2, g29q3, g30q4
  complex(dp) :: g22q1, g28q2, g30q3, g31q4
  complex(dp) :: g23q1, g29q2, g32q3, g33q4
  complex(dp) :: g24q1, g30q2, g33q3, g34q4
  complex(dp) :: g25q1, g31q2, g34q3, g35q4
  complex(dp) :: g6, g7, g8, g9, g10, g11, g12, g13, g14, g15
  complex(dp) :: Gtmp_r2(1:15)
...
end module ol_loop_handling_dp
