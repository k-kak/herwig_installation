...
! **********************************************************************
  ...
  implicit none
  complex(qp) :: cont_VV
  complex(qp), intent(in) :: A(4), B(4)
  cont_VV = A(1)*B(2) + A(2)*B(1) - A(3)*B(4) - A(4)*B(3)
  cont_VV = 0.5_qp * cont_VV
...
! **********************************************************************
  ...
  implicit none
  complex(qp) :: cont_V
  complex(qp), intent(in) :: A(4)
  cont_V = A(1)*A(2) + A(2)*A(1) - A(3)*A(4) - A(4)*A(3)
  cont_V = 0.5_qp * cont_V
end function cont_V
...
! **********************************************************************
  use kind_types, only: qp
  implicit none
  complex(qp) :: contsum_VVV
  complex(qp), intent(in) :: A1(4), A2(4), B(4)
  contsum_VVV = (A1(1)+A2(1))*B(2) + (A1(2)+A2(2))*B(1) - (A1(3)+A2(3))*B(4) - (A1(4)+A2(4))*B(3)
  contsum_VVV = 0.5_qp * contsum_VVV
...
! **********************************************************************
  ...
  implicit none
  complex(qp) :: cont_EE
  complex(qp), intent(in) :: A(4), B(4), C(4), D(4)
...
! **********************************************************************
  use kind_types, only: qp
  implicit none
  complex(qp), intent(in)  :: B(4), C(4), D(4)
  complex(qp), intent(out) :: Aout(4)
  complex(qp)              :: C1D4, C1D3, C1D2, C2D3, C2D4, C3D4
  ...
  Aout    = (0._qp, 0.5_qp) * Aout
end subroutine cont_EpVVV

end module ol_contractions_qp



! **********************************************************************
module ol_s_contractions_qp
...
end module ol_s_contractions_qp



! **********************************************************************
module ol_h_contractions_qp
! Routines to contract two wave functions
! - cont_PP: two 4-vectors which are not stored in a wfun object
! - cont_SS: two scalars
! - cont_CD: two ghosts (this is the same as cont_SS)
...
! **********************************************************************
  ...
  use ol_data_types_qp
  use ol_helicity_bookkeeping_qp, only: helbookkeeping_cont
  implicit none
...
end module ol_h_contractions_qp
