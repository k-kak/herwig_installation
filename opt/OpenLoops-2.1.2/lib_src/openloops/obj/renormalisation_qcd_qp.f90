...
module ol_qcd_renormalisation_qp
  implicit none
  contains
...
! **********************************************************************
  ...
  use ol_parameters_decl_dp, only: LeadingColour, model
  ...
  real(qp)    :: deC_UV, deC_IR, deB_UV, deB_IR, deT_UV, deT_IR
...
!   end if
  ...
  if (CT_is_on /= 0) then
    ...
    ctHEFTgggh  = (11+3*dgQCD + 1.5_qp*dZg)
  ...
  if (R2_is_on /= 0) then
    ...
    if (SwB /= 0) then
      ...
      ! HEFT
      ctHEFTggh   = ctHEFTggh + [ 1, 89, 14, -17, -93 ] * (nc/24._qp)
      ctHEFTgggh  = ctHEFTgggh - 15*nc/8._qp
      R2HEFTggggh = 0.125_qp
      R2HEFThqq   = 0.25_qp*(nc-1._qp/nc)
      R2HEFTghqq  = 0.25_qp*(3._qp/nc-5*nc)
...
end module ol_qcd_renormalisation_qp


module ol_qcd_offshell_selfenergiesqp
  use kind_types, only: qp, dp
  use ol_loop_parameters_decl_qp, only: de1_IR, de1_UV, de2_i_IR
  use ol_debug, only: ol_msg, ol_error, ol_fatal
  use ol_generic, only: to_string
...
function gluon_ofsse(p2,pid)
  ...
  complex(qp) :: cc1,gluon_ofsse(3),B01,B02,B03,B04
  complex(qp) :: cc1R1,dZ

  !complex(qp) :: B0_1,B0_2,B1_1,B1_2,B11_1,B11_2,B00_1,B00_2
  real(qp) :: ncc,nlf
  integer :: i
...
function quark_ofsse(p2,pid)
  ...
  complex(qp), intent(in) :: p2
  integer,           intent(in) :: pid
  complex(qp) :: cc1,cc2,cc3,quark_ofsse(2),B01,B02,B03
  complex(qp) :: cc1R1,cc2R1,cc3R1,fac
  complex(qp) :: cc1R2,cc2R2,cc3R2

  complex(qp) :: B0_1,A0_1,dZ,dM,M,M2
...
end module ol_qcd_offshell_selfenergiesqp
