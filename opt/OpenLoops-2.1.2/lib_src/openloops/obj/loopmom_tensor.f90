...
module ol_loop_momentum_dp
  ...
  subroutine loop_mom_tens(qloop, qarray)
    ...
    use ol_kinematics_dp, only: Std2LC_cmplx
    use ol_tensor_bookkeeping, only: qproductrules
    implicit none
    complex(dp), intent(in)  :: qloop(0:3)
    complex(dp), intent(out) :: qarray(:)
    complex(dp) :: qloop_lc(4)
    integer :: l
...
end module ol_loop_momentum_dp
