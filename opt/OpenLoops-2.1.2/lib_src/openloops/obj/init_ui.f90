...
module ol_init
  ...
  subroutine set_if_modified_quadcmplx(current, new)
    implicit none
    complex(qp), intent(inout) :: current
    real(qp),    intent(in) :: new
    if (current /= new) then
      current = new
      setparameter_tree_was_called = .true.
...
!    use nll_ppvj, only: nll_order, nll_ll, nll_add
    ...
    select case (to_lowercase(param))
      ...
      case default
        ...
        if (.not. forwarded_init) then
          forwarded_init = .true.
          call setparameter_double(param, real(val, dp))
  ...
  subroutine setparameter_single(param, val, err)
    ...
    real, intent(in) :: val
    integer, intent(out), optional :: err
    call setparameter_double(param, real(val, dp), err)
  ...
  subroutine parameters_flush() bind(c,name="ol_parameters_flush")
    use ol_parameters_init_dp, only: parameters_init, loop_parameters_init
    use ol_parameters_init_qp, only: parameters_init_qp=>parameters_init, &
                                                loop_parameters_init_qp=>loop_parameters_init
    use ol_parameters_init_dp, only: qcd_parameters_init
    implicit none
  ...
  subroutine tree_parameters_flush() bind(c,name="ol_tree_parameters_flush")
    use ol_parameters_init_dp, only: parameters_init
    use ol_parameters_init_dp, only: qcd_parameters_init
    implicit none
  ...
  subroutine setparameter_string_c(param, val) bind(c,name="ol_setparameter_string")
    ...
    use ol_parameters_decl_dp, only: max_parameter_name_length, max_parameter_length
    implicit none
    character(kind=c_char), dimension(*), intent(in) :: param
