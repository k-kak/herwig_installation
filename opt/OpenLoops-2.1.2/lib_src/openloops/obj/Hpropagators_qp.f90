...
! *****************************************************************************
  ...
  use ol_kinematics_qp, only: get_LC_4, get_LC_mass2
  use ol_momenta_decl_qp, only: collconf, softconf
  use ol_parameters_decl_dp, only: ir_hacks
  implicit none
  integer(intkind1), intent(in)    :: ntry
  integer,           intent(in)    :: mom
  integer(intkind2), intent(inout) :: n
  type(wfun),        intent(in)    :: Q(n)
  complex(qp), intent(in)    :: M
...
end module ol_hel_propagators_qp
