...
module openloops
  ...
  use ol_parameters_decl_dp,  only: procname_length, max_parameter_length
  use ol_external_decl_dp, only: n_scatt
  use ol_debug, only: get_error, ol_msg, ol_error, ol_fatal, ol_fatal_update, error
  use ol_parameters_init_dp, only: parameters_write
  implicit none
  private
  ! from module init_iu
  ...
  type process_handle
    ...
    real(dp), allocatable :: last_psp(:,:)
    real(dp) :: last_muren
    real(dp) :: last_alpha_QCD
...
!      call ol_fatal_update()
  ...
  function register_process_lib(libname, proc, content, amptype, n_in, pol, perm, extid, photon_id, qcd_powers)
    ...
    use ol_loop_parameters_decl_dp, only: maxpoint, maxrank, do_pole_checks
    implicit none
    character(len=*), intent(in) :: libname
    character(len=*), intent(in) :: proc
    integer, intent(in) :: content, amptype, n_in
  ...
  use ol_parameters_decl_dp, only: check_collection
  ! loop over library types
    use kind_types, only: dp
    use ol_parameters_decl_dp, only: coupling_qcd, coupling_ew, OLMode
    implicit none
  ...
  ! 1: found, 0: not found, -1: abort
    ...
    use ol_loop_parameters_decl_dp, only: stability_mode,a_switch,a_switch_rescue,redlib_qp, &
    use_bubble_vertex
  ...
  ! checks that proclib folder exists within set install_path
    use ol_parameters_decl_dp, only: install_path
    implicit none
  ...
  subroutine check_parameters(p,amptype,found)
    ...
    use ol_loop_parameters_decl_dp , only: nf, nc
    use ol_version, only: process_api
    implicit none
    integer, intent(in) :: p
    integer, intent(in) :: amptype
  ...
  !       p1 -> 1, p2 -> 2, p3 -> 3  such that  N[p1] > N[p2] > N[p3] */
    use ol_generic, only: to_string
    use ol_parameters_decl_dp, only: rMC, rYC, rMM, rYM, rML, rYL
    implicit none
    type(extparticle), intent(inout) :: ext(:)
  ...
  function write_shop_list(ext, proc)
    use ol_parameters_decl_dp, only: shopping_list, order_ew, order_qcd, &
  ...
  function ID_to_extparticle(id_in)
    ...
    use ol_parameters_decl_dp, only: use_me_cache
  ...
  subroutine phase_space_point(id, sqrt_s, psp)
    implicit none
    integer, intent(in) :: id
    real(dp), intent(in) :: sqrt_s
    real(dp), intent(out) :: psp(:,:)
  ...
  subroutine phase_space_point_c(id, sqrt_s, pp) bind(c,name="ol_phase_space_point")
    ...
    integer :: i
    real(dp) :: f_sqrt_s
    real(dp) :: f_psp(0:3,n_external(int(id)))
  ...
  subroutine evaluate_tree(id, psp, res)
   ...
   ! [out] res: squared tree matrix element
    ...
    real(dp) :: m2cc(0:n_external(id)*(n_external(id)+1)/2+1)
    real(dp) :: resmunu(4,4)
    type(process_handle) :: subprocess
    call stop_invalid_id(id)
    if (error > 1) return
  ...
  subroutine evaluate_tree_colvect_c(id, pp, amp, nhel) bind(c,name="ol_evaluate_tree_colvect")
    ...
    real(c_double) :: res
    complex(dp) :: f_amp(get_tree_colbasis_dim(id),get_nhel(id))
    integer :: f_nhel, k, h
    real(dp) :: f_bornphotonfactor
  ...
  subroutine evaluate_tree_colvect2(id, psp, m2arr)
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2arr(:)
    integer :: nhel
    real(dp) :: res, bornphotonfactor
    complex(dp) :: amp(get_tree_colbasis_dim(id),get_nhel(id))
    call evaluate_tree(id, psp, res) ! fill colour vector cache
    call process_handles(id)%tree_colvect(amp, nhel)
  ...
  subroutine evaluate_loop_colvect(id, psp, M2amp, nhel, acc)
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    complex(dp), intent(out) :: M2amp(:,:)
    real(dp), intent(out), optional :: acc
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), f_acc
    type(correlator) :: Lcc
    integer, intent(out) :: nhel
    real(dp) :: bornphotonfactor
  ...
  subroutine evaluate_loop_colvect_c(id, pp, M2amp, nhel, acc) bind(c,name="ol_evaluate_loop_colvect")
    ...
    real(dp) :: psp(0:4,n_external(id))
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), f_acc
    type(correlator) :: Lcc
    complex(dp) :: f_M2amp(get_tree_colbasis_dim(id),get_loop_nhel(id))
    real(dp) :: f_bornphotonfactor
    integer :: f_nhel, k, h
    if (.not. associated(process_handles(id)%loop_colvect)) then
      call ol_msg("Error: loop colour vector information is not available")
  ...
  subroutine evaluate_loop_colvect2(id, psp, m2arr, acc)
    ...
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), f_acc
    real(dp) :: bornphotonfactor
    complex(dp) :: amp(get_loop_colbasis_dim(id),get_loop_nhel(id))
    type(correlator) :: Lcc
    if (.not. associated(process_handles(id)%loop_colvect)) then
      call ol_msg("Error: loop colour vector information is not available")
      call ol_fatal("       for process " // process_handles(id)%process_name)
  ...
  subroutine evaluate_loop_colvect2_c(id, pp, m2arr, acc) bind(c,name="ol_evaluate_loop_colvect2")
    ...
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), f_acc
    type(correlator) :: Lcc
    complex(dp) :: amp(get_tree_colbasis_dim(id),get_loop_nhel(id))
    real(dp) :: bornphotonfactor
    if (.not. associated(process_handles(id)%loop_colvect)) then
      call ol_msg("Error: loop colour vector information is not available")
      call ol_fatal("       for process " // process_handles(id)%process_name)
  ...
  subroutine evaluate_cc(id, psp, tree, cc, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: tree, cc(:), ewcc
    type(process_handle) :: subprocess
    real(dp) :: m2cc(0:n_external(id)*(n_external(id)+1)/2+1) ! keep +1 for compatibility
    real(dp) :: resmunu(4,4)
    integer  :: n_cc, i, j
    call stop_invalid_id(id)
    if (error > 1) return
  ...
  subroutine evaluate_cc_c(id, pp, tree, cc, ewcc) bind(c,name="ol_evaluate_cc")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_tree, f_cc(rval_size(n_external(id),2)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_cc2(id, psp, res, cc, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: res, cc(:), ewcc
    type(process_handle) :: subprocess
    integer  :: n_cc, i, j
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), acc
    type(correlator) :: Lcc
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_cc2_c(id, pp, res, cc, ewcc) bind(c,name="ol_evaluate_cc2")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_res, f_cc(rval_size(n_external(id),2)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_loopcc(id, psp, m2l0, m2l1, cc, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2), cc(:), ewcc
    type(process_handle) :: subprocess
    integer  :: n_cc, i, j
    real(dp) :: ir1(0:2), m2l2(0:4), ir2(0:4), acc
    type(correlator) :: Lcc
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_loopcc_c(id, pp, m2l0, m2l1, cc, ewcc) bind(c,name="ol_evaluate_loopcc")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_m2l0, f_m2l1(0:2), f_cc(rval_size(n_external(id),2)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccmatrix(id, psp, tree, ccij, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: tree, ccij(:,:), ewcc
    type(process_handle) :: subprocess
    real(dp) :: m2cc(0:n_external(id)*(n_external(id)+1)/2+1)
    integer  :: n_cc, i, j
    real(dp) :: resmunu(4,4)
  ...
  subroutine evaluate_ccmatrix_c(id, pp, tree, ccij, ewcc) bind(c,name="ol_evaluate_ccmatrix")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_tree, f_ccij(n_external(id),n_external(id)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccmatrix2(id, psp, res, ccij, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: res, ccij(:,:), ewcc
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), acc
    type(process_handle) :: subprocess
    integer  :: n_cc, i, j
    type(correlator) :: Lcc
  ...
  subroutine evaluate_ccmatrix2_c(id, pp, res, ccij, ewcc) bind(c,name="ol_evaluate_ccmatrix2")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_res, f_ccij(n_external(id),n_external(id)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_loopccmatrix(id, psp, m2l0, m2l1, ccij, ewcc)
   ...
   ! [out] ewcc: charge correlation for EW i-operator
    ...
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2), ccij(:,:), ewcc
    type(process_handle) :: subprocess
    real(dp) :: m2cc(0:n_external(id)*(n_external(id)+1)/2+1)
    real(dp) :: ir1(0:2), m2l2(0:4), ir2(0:4), acc
    integer  :: n_cc, i, j
  ...
  subroutine evaluate_loopccmatrix_c(id, pp, m2l0, m2l1, ccij, ewcc) bind(c,name="ol_evaluate_loopccmatrix")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_m2l0, f_m2l1(0:2), f_ccij(n_external(id),n_external(id)), f_ewcc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccewmatrix_c(id, pp, tree, ccij) bind(c,name="ol_evaluate_ccewmatrix")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_tree, f_ccij(n_external(id),n_external(id))
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_ccewmatrix2_c(id, pp, res, ccij) bind(c,name="ol_evaluate_ccewmatrix2")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_res, f_ccij(n_external(id),n_external(id))
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_sc(id, psp, emitter, polvect, res)
    ...
    real(dp), intent(in) :: psp(:,:), polvect(4)
    real(dp), intent(out) :: res(:)
    type(process_handle) :: subprocess
    integer :: j, extcombs(n_external(id)), nextcombs
    real(dp) :: m2sc(0:n_external(id)*(n_external(id)+1)/2+1)
    real(dp) :: resmunu(4,4)
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_sc_c(id, pp, emitter, polvect, res) bind(c,name="ol_evaluate_sc")
    ...
    real(dp) :: f_pp(0:4,n_external(id)), f_polvect(4)
    real(dp) :: f_res(n_external(id))
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_scpowheg(id, psp, emitter, res, resmunu)
   ...
   ! B^(mu,nu) = sum_l,k M(l)M(k) epsilon_l^mu* epsilon_k^nu
    ...
    integer, intent(in) :: id, emitter
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: res, resmunu(4,4)
    type(process_handle) :: subprocess
    real(dp) :: m2cc(0:n_external(id)*(n_external(id)+1)/2+1)
    call stop_invalid_id(id)
    if (error > 1) return
  ...
  subroutine evaluate_sctensor(id, psp, emitter, res, resmunu)
   ...
   ! [out] res(N:4:4): array with the spin correlated born matrix element B^(jk)(mu,nu)
    ...
    integer, intent(in) :: id, emitter
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: res, resmunu(n_external(id),4,4)
    call stop_invalid_id(id)
  ...
  subroutine evaluate_loopsctensor(id, psp, emitter, m2l0, m2l1, resmunu)
   ...
   ! [out] res(N:4:4): array with the spin correlated born matrix element B^(jk)(mu,nu)
    ...
    integer, intent(in) :: id, emitter
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2), resmunu(n_external(id),4,4)
    call stop_invalid_id(id)
  ...
  subroutine evaluate_loopsctensor_c(id, pp, emitter, m2l0, m2l1, resmunu) bind(c,name="ol_evaluate_loopsctensor")
    ...
    integer :: f_id, f_emitter
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_m2l0, f_m2l1(0:2), f_resmunu(4,4)
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_loopscpowheg(id, psp, emitter, m2l0, m2l1, resmunu)
   ...
   ! L^(mu,nu) = sum_l,k Mloop(l)Mborn(k) epsilon_l^mu* epsilon_k^nu
    ...
    integer, intent(in) :: id, emitter
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2), resmunu(4,4)
    real(dp) :: ir1(0:2), m2l2(0:4), ir2(0:4), acc
    type(process_handle) :: subprocess
    type(correlator) :: Lmunu
    call stop_invalid_id(id)
  ...
  recursive subroutine evaluate_full(id, psp, m2l0, m2l1, ir1, m2l2, ir2, acc)
    use ol_stability
    use ol_generic, only: to_string
    use ol_parameters_decl_dp, only: add_associated_ew, alpha_QCD
    use ol_loop_parameters_decl_dp, only: IR_is_on, loop_parameters_status, muren_unscaled, CT_is_on, R2_is_on, TP_is_on
    use ol_loop_parameters_decl_dp, only: stability_mode,a_switch,a_switch_rescue,redlib_qp,use_bubble_vertex
    implicit none
    integer, intent(in) :: id
    real(dp), intent(in) :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4)
    real(dp), intent(out) :: acc
    real(dp) :: m2l0ew, m2l1ew(0:2), ir1ew(0:2), m2l2ew(0:4), ir2ew(0:4), accew
    real(dp) :: last_m2ct, new_m2ct, muren_bak
  ...
  subroutine evaluate_full_c(id, pp, m2l0, m2l1, ir1, m2l2, ir2, acc) bind(c,name="ol_evaluate_full")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_m2l0, f_m2l1(0:2), f_ir1(0:2), f_m2l2(0:4), f_ir2(0:4)
    real(dp) :: f_acc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
  ...
  subroutine evaluate_fullcr(id, psp, m2l0, m2l1, ir1, m2l2, ir2, cr, acc)
    ...
    use ol_data_types_dp, only: correlator
    use ol_parameters_decl_dp, only: alpha_QCD
    use ol_loop_parameters_decl_dp, only: muren_unscaled
  ...
  subroutine evaluate_loop(id, psp, m2l0, m2l1, acc)
    ...
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2)
    real(dp), intent(out) :: acc
    real(dp) :: ir1(0:2), m2l2(0:4), ir2(0:4)
  ...
  subroutine evaluate_loopbare(id, psp, m2l0, m2l1, acc)
    use ol_loop_parameters_decl_dp, only: CT_is_on, R2_is_on, IR_is_on, polecheck_is
    implicit none
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2l1(0:2)
    real(dp), intent(out) :: acc
    real(dp) :: ir1(0:2), m2l2(0:4), ir2(0:4), m2ct(0:2)
  ...
  subroutine evaluate_loop2(id, psp, res, acc)
    ...
    real(dp), intent(out) :: res
    real(dp), intent(out) :: acc
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4)
    if (.not. btest(process_handles(id)%content, 2)) then
  ...
  subroutine evaluate_iop2(id, psp, res, resir, acc)
    ...
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: res, resir(0:2)
    real(dp), intent(out) :: acc
    real(dp) :: m2l0, m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4)
    if (.not. btest(process_handles(id)%content, 2)) then
      call ol_fatal('evaluate: loop^2 routine not available for process ' // trim(to_string(id)))
  ...
  subroutine evaluate_iop2_c(id, pp, res, resir, acc) bind(c,name="ol_evaluate_iop2")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_res, f_resir(0:2), f_acc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
    f_pp = reshape(pp, [5,process_handles(id)%n_particles])
  ...
  subroutine evaluate_associated(id, psp, level, m2l0)
    use ol_stability
    use ol_generic, only: to_string
    use ol_parameters_decl_dp, only: add_associated_ew
    use ol_loop_parameters_decl_dp, only: IR_is_on
    implicit none
  ...
  subroutine evaluate_ct(id, psp, m2l0, m2ct)
    use ol_parameters_decl_dp, only: add_associated_ew
    use ol_loop_parameters_decl_dp, only: CT_is_on, R2_is_on, TP_is_on, do_ew_renorm,use_bubble_vertex
    ...
    real(dp), intent(out) :: m2l0, m2ct
    real(dp) :: m2l0ew, m2ctew
    type(process_handle)  :: subprocess, subprocessew
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_loopct(id, psp, m2l0, m2ct)
    use ol_loop_parameters_decl_dp, only: CT_is_on, IR_is_on
    ...
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2ct(0:2)
    real(dp) :: m2ct_000, m2ct_110, polescale
  ...
  subroutine evaluate_iop(id, psp, m2l0, m2iop)
    ...
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2iop(0:2)
    real(dp) :: m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4), acc
    type(process_handle)  :: subprocess
    integer :: IR_is_on_bak
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_schsf(id, psp, m2l0, pind_1, pind_2, m2schsf)
    ...
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2schsf
    type(process_handle)  :: subprocess
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_pt(id, psp, m2l0, m2pt, m2l1)
    ...
    integer, intent(in) :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2pt, m2l1
    type(process_handle)  :: subprocess
    call stop_invalid_id(id)
    if (error > 1) return
    subprocess = process_handles(id)
  ...
  subroutine evaluate_poles(id, psp, m2l0, m2bare, m2ct, m2ir, m2sum)
    ...
    integer, intent(in)  :: id
    real(dp), intent(in)  :: psp(:,:)
    real(dp), intent(out) :: m2l0, m2bare(0:2), m2ct(0:2), m2ir(0:2), m2sum(0:2)
    real(dp) :: m2l1(0:2), ir1(0:2), m2l2(0:4), ir2(0:4)
    real(dp) :: m2l1_100(0:2), m2l1_010(0:2), m2l1_001(0:2), m2l1_000(0:2)
    real(dp) :: m2ct_100, m2ct_010, m2ct_000
    real(dp) :: acc, polescale
    real(dp) :: poleUV1bak, poleIR1bak, poleIR2bak
  ...
  subroutine evaluate_poles_c(id, pp, m2l0, m2bare, m2ct, m2ir, m2sum) bind(c,name="ol_evaluate_poles")
    ...
    integer :: f_id
    real(dp) :: f_pp(0:4,n_external(id))
    real(dp) :: f_m2l0, f_m2bare(0:2), f_m2ct(0:2), f_m2ir(0:2), f_m2sum(0:2)
    real(dp) :: f_acc
    f_id = id
    call stop_invalid_id(f_id) ! needed because of reshape
    if (error > 1) return
  ...
  subroutine check_poles(id, psp_in)
    ...
    implicit none
    integer, intent(in)  :: id
    real(dp), intent(in), optional  :: psp_in(:,:)
    real(dp) :: psp(0:3,n_external(int(id)))
    real(dp) :: m2l0, m2bare(0:2), m2ct(0:2), m2ir(0:2), m2sum(0:2)
    real(dp) :: energy = 2000
